...
\begin{satz}
	...
	\begin{enumerate}
		...
		mit $m(b\otimes c, b'\otimes c') = bb'\otimes cc'$. \label{thm:314:i}
		\item $(B\otimes_A, +, m)$ ist ein Ring (und sogar eine $A$-Algebra). \label{thm:314:ii}
		...
		\begin{figure}[H]
			...
			\begin{tikzcd}[sep=1cm]
				...
				& C \arrow[bend left]{luu}{\psi} \arrow{u}
			\end{tikzcd}
			. % TODO: punkt
...
\begin{proof}
	...
	\begin{enumerate}[label=\ref{thm:314:\roman*}]
		\item Betrachte für $(b,c)\in B\times C$ die $A$-bilineare Abbildung $B\times C\to B\otimes_AC, (b'c')\mapsto bb'\otimes cc$ mit
		\begin{center}
			\begin{tikzcd}
				B\times C\arrow{d}\arrow{r}& B\otimes_AC\\
				B\otimes_AC\arrow[dashrightarrow]{ru}[swap]{\exists!\phi_{(b,c)} \text{ $A$-linear}}
			\end{tikzcd}
			. % TODO: punkt
		\end{center}
		...
		\begin{center}
			\begin{tikzcd}[column sep=2cm]
				...
				B\otimes_AC\arrow[dashrightarrow]{ru}[swap]{\exists!\phi\text{ $A$-linear}}
		...
		\item Nachrechnen mit \ref{thm:314:i} und Rechenregeln für $\otimes$, wobei $1_{B\otimes_AC}= 1\otimes 1$ zu beachten ist.
...
\end{lem}
\begin{proof}
	Sei $x+M_{tor}$. Angenommen, es existiert ein $a\in A, a\neq 0$ mit $x+M_{\tor} = 0+M_{tor}$. Dann folgt $ax\in M_{tor}$, es existiert also ein $b\in A, b\neq 0$ mit $bax = 0$. Da $ba\neq 0$ gilt, muss schon $x\in M_{\tor}$ gelten.
\end{proof}
...
\begin{defi}
	Sei $A$ Hauptidealring, $p\in A$ prim. Definiere
	$$M(p) = \{x\in M|\exists r\geq 0: p^rx = 0\} = \bigcup\limits_{r\geq 0} M_{p^r}$$
	$M(p)\subseteq M$ Untermodul. Falls $M$ endlich erzeugt ist, existiert $r\geq 0$ mit $M_{p^r} = M(p)\Rightarrow p^r\in Ann(M)\Rightarrow Ann(M)\supset (p^r)\Rightarrow \exists s\leq r: Ann(M) = (p^s)$ (da $A$ Hauptidealring).
	
	Dann gilt $M_{p^{s-1}}\subsetneq M_{p^s} = M(p)$.
	
	$M$ heißt primär $\Leftrightarrow \exists p\in A$ prim: $M = M(p)$.
\end{defi}	


\begin{satz}[Primärzerlegung von Torsionsmoduln über Hauptidealringen]
	Sei $A$ Hauptidealring, $M$ endlich erzeugter Torsionsmodul über $A$. Dann existiert $a\neq 0$ mit $Ann(A) = (a)$. Seien $p_1,\dots, p_n\in A$ prim mit $(p_i, p_j) = (1)$ für alle $i\neq j$ und $r_1,\dots, r_n\geq 1$ mit
	$$ a = p_1^{r_1}\dots p_n^{r_n}$$
	Dann gilt:
	\begin{enumerate}
		\item $M = M(p_1)\oplus\dots\oplus M(p_n)$
		\item $Ann(M(p_i)) = (p_i^{r_i})$
	\end{enumerate}
\end{satz}
...
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item $p_1^{r_1},\dots, p_n^{r_n}$ paarweise teilerfremd. $\Rightarrow M = M_{p_1^{r_1}}\oplus\dots \oplus M_{p_n^{r_n}}$ nach Lemma 4.12. Zeige nun noch $M_{p_i^{r_i}} = M(p_i)$:
		\begin{description}
			\item[\glqq$\subseteq$\grqq :] $\checkmark$
			\item[\glqq$\supseteq$\grqq :] Sei $x\in M(p_i)\Rightarrow \exists r\geq 0: p_i^r = 0$.
			
			1. Fall: $r\subseteq r_i\Rightarrow p_i^{r_i}x = 0\Rightarrow x\in M_{p_i^{r_i}}$
			
			2. Fall: $r>r_i\Rightarrow \gcd(p_i^r, a) = p_i^{r_i}\Rightarrow \exists c, d\in A: cp_i^r+da = p_i^{r_i}\Rightarrow p_i^{r_i}x = cp_i^rx+dax = 0$.
		\end{description}
		\item $(p_i^{r_i})\subseteq Ann(M_{p_i^{r_i}}) = Ann(M(p_i))$ nach Teil (i). $\Rightarrow Ann(M(p_i)) = (p_i^{s_i})$ für $s_i\leq r_i$. Da $M = M(p_1)\oplus \dots \oplus M(p_n)\Rightarrow Ann(M) = \bigcap Ann(M(p_i))\ni p_i^{s_i}\dots p_n^{s_n}$, aber es gilt auch $Ann(M) = (p_1^{r_1}\dots p_n^{r_n})$.
		
		Da $(p_i, p_j) = (1)$ für $i\neq j$, folgt $r_i\leq s$.
	\end{enumerate}
\end{proof}

\begin{beme}
	$M(p_i)$ im Satz ist nach Lemma 4.12 von der Form $b_iM$, wobei $b_i = \frac{a}{p_i^{r_i}}$.
\end{beme}

\begin{defi}
	Ein $A$-Modul $M$ heißt zyklisch, wenn $\exists x\in M: M = \<x\>_A$. Übung: $M\cong \fak{A}{Ann(x)}$.
\end{defi}

\begin{satz}[Zerlegung von primären Moduln in zyklische Moduln]
	Sei $A$ Hauptidealring. Sei $M$ primär und endlich erzeugt. Sei $p\in A$ mit $Ann(M) = (p^r)$. Dann $\exists 1\leq s_1\leq\dots \leq s_l = r$ und $\exists M_1,\dots, M_l\subseteq M$ Untermoduln:
	\begin{enumerate}
		\item $M = M_1\oplus\dots\oplus M_l$
...
\begin{proof}
	...
	\begin{description}
		\item[$n = 1$:] $\checkmark$
		\item[$n-1\to n$:] Sei $\{x_1,\dots, x_n\}$ Erzeugendensystem von $M$. Nach Lemma 4.11 ist $(p^r) = Ann(M)  = \bigcap Ann(x_i)$.
		
		$\Rightarrow \exists t_i\leq r: Ann(x_i) = (p^{t_i})\Rightarrow \exists x_i: Ann(x_i) = (p_i^r)$. Setze $U\coloneqq \<x_n\>$ und $N = \fak{M}{U}, \pi\colon M\to N$. Da $\pi$ surjektiv: $Ann(M)\subseteq Ann(N)\Rightarrow \exists s\leq r: Ann(N) = (p^s)$.
		
		Es gilt $N = \<\pi(x_1),\dots, \pi(x_{n-1})\>$. Nach Induktionsvoraussetzugn existieren $N_1,\dots, N_k\subseteq N$ Untermoduln und $1\leq t_1\leq\dots \leq t_k = s$ mit $N = N_1\oplus \dots \oplus N_k$ und $N_j$ zyklisch mit $Ann(N_j) = (p^{t_j})$.
		
		\lecture{7. Mai 2018}
		
		Fixiere $j\in\{1,\dots, k\}$, sei $y\in N_j$ mit $N_j = \<y\>$. Wähle $xÄ\in M$ mit $\pi(x') = y$. Dann gilt $\pi(p^{t_j}x') = p^{t_j}y = 0$, also $x''\eqqcolon p^{t_j}x'\in U$.
		
		Aber $Ann(M) = (p^r) \Rightarrow 0 = p^rx' = p^{r-t_j}x''$
		
		Lemma:
			Sei $L$ zyklischer $A$-Modul, $Ann(L) = (p^r)$, $v\in L$ und $s\leq r$. Falls $p^sv = 0$ gilt, folgt dann $\exists w\in L: v = p^{r-s}w$.
		
		Beweis des Lemmas:
			Sei $z\in L$ mit $L = \<z\> \Rightarrow \exists a \in A: v = az\Rightarrow 0 = p^sv = p^saz\Rightarrow p^sa\in Ann(L) = (p^r)$, also $\exists b\in A: p^sa = p^rb\Rightarrow a = p^{r-s}b\Rightarrow v = p^{r-s}bz$.
			
		Wir wenden nun das Lemma an auf $L = U$, $v = x''$, $s = r-t_j$: Dann folgt $\exists x'''\in U: x'' = p^{l_j}x'''$.
		
		Definiere nun $x\coloneqq x'-x'''$. Dann gilt:
		\begin{equation}
		\pi(x) = \pi(x')-\pi(x'') = y-0\\
		p^{t_j}x = p^{t_j}x'-p^{t_j}x''' = x''-x'' = 0
		\end{equation}
		Definiere $M_j\coloneqq \<x\>$. Dafür gilt $\pi(M_j) = N_j$ und $\ker(\pi|_{M_j}) = \{0\}$. Denn sei $v\in M_j$ mit $\pi(v) = 0$. Es existiert ein $a\in A$ mit $v = ax\Rightarrow 0 = a\pi(x) = ay\Rightarrow a \in Ann(y) = Ann(N_j) = (p^{t_j})\Rightarrow \exists b\in A: a = bp^{t_j}\Rightarrow v = bp^{t_j}x  = 0$, wobei letzteres mit Eigenschaft (2) folgt.
		
		Folglich erhalten wir $\pi|_{M_j}\colon M_j\xrightarrow{\cong} N_j$. Zeige $M_j\cap \sum_{j'\neq j} M_j' = \{0\}$: Sei $v\in M_j\cap\sum_{j'\neq j} M_j'$, also $\pi(v) \in N_j\cap\sum_{j'\neq j} N_{j'} = \{0\}\Rightarrow v = 0$.
		
		Insgesamt haben wir gezeigt: $\sum M_j = \bigoplus M_j \eqqcolon M'$ und $\pi|_{M'}\colon M'\xrightarrow{\cong} N$ (das gilt auf allen direkten Summanden). $\Rightarrow M = M'\oplus U$.
	.
		Wähle $l = k+1, s_j  = t_j (j = 1,\dots, k), s_l = r$ und $M_j = N_j (j = 1,\dots, k), M_l = U$
	\end{description}
	
\end{proof}

Zusammenfassung: Sei $A$ Hauptidealring, $M$ endlich erzeugter $A$-Modul. Dann gibt es $r\geq 0$ sowie $p_1,\dots, p_n\in A$ prim, sodass gilt:
$\forall i \exists 1\leq s_{i_1}\leq\dots\leq s_{i_{l_i}}$ mit
$$M\cong A^r\oplus \bigoplus_{i = 1}^n\bigoplus_{j = 1}^{l_i} \fak{A}{(P_i^{s_{i_j}})}$$

\subsection{Anwendung auf Matrizen}
Sei $k$ Körper, $V$ ein $k$-Vektorraum, $\dim V<\infty$, $\psi\in \End_k(V), M = k[t]$-Modul zu $(V,\psi)$

\begin{beme}
	\leavevmode
	\begin{enumerate}
		\item $M$ ist Torsionsmodul (denn sonst hätte $M$ torsionsfreien Anteil, der insbesondere frei ist über $k[t]$ und damit unendlichdimensional über $k$)
		\item $M$ zyklisch: $\exists u\in V: \<u, \psi(u), \psi^2(u),\dots\>_k = V$
	\end{enumerate}
\end{beme}
\begin{beme}
	$Ann(M) = \{p\in k[t]|p(\psi) = 0$ in $\End(V)\} = (m_{\psi})$, wobei $m_{\psi}$ das Minimalpolynom von $\psi$ ist-
\end{beme}
...
\begin{defi}
	...
	\end{description}
	heißt Begleitmatrix zu $p$ (\glqq companion matrix\grqq)
\end{defi}
\begin{beme}
...
\end{beme}
\begin{lem}
	Falls $M$ zyklischer $k[t]$-Modul ist:
	\begin{enumerate}
		\item $\dim_ V = \deg m_{\psi}$
		\item Sei $u \coloneqq \deg_(m_{\psi})$ und $v\in V$ mit $M = \<v\>_{k[t]}$. Seien $p_0,\dots, p_{n-1}\in k[t]$ mit $\deg p_i<n$ mit $p_0,\dots, p_{n-1}$ linear unabhängig über $k$. Dann ist $(p_0v, \dots, p_{n-1}v)$ Basis von $V$ über $k$. 
	\end{enumerate}
\end{lem}
\begin{proof}
	Sei $m_{\psi}(t) = t^n+a_{n-1}t^{n-1}+\ldots + a_0$.
	\begin{enumerate}
		\item [a)] Zeige $\<v, \psi(v),\dots, \psi^{n-1}(v)\>_k = V$. Denn nach Bemerkung 4.18 gilt $\<v, \psi(v),\psi^2(v),\dots\>_k = V$; Da $m_{\psi}(\psi) = 0$ gilt, ist $\psi^n(v)\in \<v, \psi(v),\dots, \psi^{n-1}(v)\>_k$ und durch Induktion ergibt sich $\psi^{n+i}(v) \in \<v, \psi(v), \dots, \psi^{n-1}(v)\>_k$
		\item[b)] Zeige $(p_0v,\cdots, p_{n-1}v)$ ist linear unabhängig. Seien $b_0,\dots, b_{n-1}\in k$ mit $0 = \sum b_ip_iv \Rightarrow p \coloneqq \sum b_ip_i\in Ann(v) = Ann(M) = (m_{\psi})$.
		
		Da $\deg p_i<\deg m_{\psi}$ ist, folgt $p = 0$ und, da $p_0,\dots, p_{n-1}$ linear unabhängig sind, ist $b_= =\dots = b_{n-1} = 0$.
		
		Aus (a) und (b) folgen nun (i) und (ii).
	\end{enumerate}
Aus (a) und (b) folgen nun (i) und (ii).
\end{proof}

\begin{lem} Sei $M$ zyklischer $k[t]$-Modul.
	\begin{enumerate}
		\item Es gibt eine Basis $B$ von $V$ mit $M_B(\psi) =C(m_{\psi})$.
		\item Falls $m_{\psi}(t) = (t-\lambda)^n \Rightarrow \exists$ Basis $B'$ von $V$ mit 
		$$M_{B'}(\psi) = \left( \begin{array}{cccc}
		\lambda & 1 &  & 0 \\ 
		& \lambda & \ddots &  \\ 
		&  & \ddots & 1 \\ 
		0 &  &  & \lambda
		\end{array} \right) = Jn(\lambda)$$
	\end{enumerate}
\end{lem}
\begin{proof} Sei $M = \<v\>_{k[t]}$.
	\begin{enumerate}
		\item Definiere $v_i = \psiî(v)$ für $i = 0,\dots, n-1$. Aus Lemma 4.22 folgt $(v_0,\dots, v_{n-1})$ ist Basis von $V$. Dabei gilt $\psi(v_{i} = v_{i+1}$ für $i = 0,\dots, n-2$ und $\psi(v_{n-1}) =\psi^n(v) = -\sum_{i = 0}^{k-1}a_i\psiî(v) = -\sum_{i = 0}^{k-1}a_iv_i$, da $m_{\psi}(\psi) = 0$. Dabei ist $m_{\psi}(t) = t^n+a_{n-1}t^{n-1}+\dots + a_0$.
		\item Definiere $v_i'\coloneqq (t-\lambda)^{n-i}\cdot v = (\psi-\lambda)^{n-i}(v)$ für $i = 1,\dots, n$.
		
		Mit Lemma 4.22 folgt nun, dass $(v_1',\dots, v_n')$ eine Basis von $V$ ist.
		
		\begin{eqnarray*}
			\psi(v_i') & = & t\cdot v_i' = t\cdot(t-\lambda)^{n-1}\cdot v\\
			 &= & (t-\lambda)^nv + \lambda (t-\lambda)^{n-1}v\\
			 & =& \lambda v_i'\\
			 \psi(v_i') & =& (t-\lambda)^{n-i+1}v +\lambda(t-\lambda)^{n-i}v = v_{i-1}' + \lambda v_i'
		\end{eqnarray*}
	\end{enumerate}
\end{proof}

\begin{bem}
	Umgekehrt gilt: Falls $\exists B$ Basis von $V$, $\exists p\in k[t]$ normiert: $M_B(\psi) =C(p)\Rightarrow M$ zyklisch.
\end{bem}

\begin{satz}[Rationale Normalform] Sei $V$ ein $k$-Vektorraum, $\dim V <\infty, \psi\in\End(V)$. Sei $m_{\psi} = p_1^{r_1}\cdot\ldots\cdot p_n^{r_n}$ mit $p_i\in k[t]$ irreduzibel (und normiert) mit $p_i\neq p_j$. Dann existieren $1\leq s_{i1}\leq\dots\leq s_{i{l_j}}  = r_i$ und es existiert eine Basis $B$ von $V$, sodass
	
	$$ M_B(\psi) = \left(\begin{array}{cccc}
	C(p_1^{s_{i1}}) &  &  & 0 \\ 
	& C(p_1^{s_{i2}}) &  &  \\ 
	&  & \ddots &  \\ 
	0 &  &  & C(p_n^{s_{i{l_n}}})
	\end{array} \right)$$%TODO: boxen drumrummachen
\end{satz}
\begin{proof}
	Aus den Sätzen 4.14 und 4.17 folgt, dass es eine Zerlegung $M = \bigoplus_{i = 1}^n\bigoplus_{j = 1}^{l_n} M_{ij}$, wobei $M_{ij}$ zyklisch mit $ann(M_{ij})  = (p_i^{s_{ij}})$. $M_{ij}$ ist Untermodul von $M\Rightarrow {}_k(M_{ij}) \eqqcolon V_{in}\subseteq V$ ist $k$-Untervektorraum und $\psi(V_{ij})\subseteq V_{ij}$
	
	Mit Lemma 4.23 folgt daraus, dass $M_{ij}$ zyklisch ist, dass eine Basis $B_{ij}$ von $V_{ij}$ existiert mit $M_{B_{ij}}(\psi|_{V_{ij}}) = C(p_i^{s_{ij}})$. Die Basis $B = B_{11}\cup B_{12}\cup \dots \cup B_{nl_n}$ ist geeignet.
\end{proof}

\begin{kor} Sei $m_{\psi} = p_1^{r_1}\cdot\ldots\cdot p_n^{r_n}$ mit $p_i\in k[t]$ normiert, irreduzibel, $p_i\neq p_j$ für $i\neq j$. Dann folgt $\exists s_i\geq r_i: \chi_{\psi} = p_1^{s_1}\cdot\ldots\cdot p_n^{s_n}$.
\end{kor}
...
\begin{satz}[Jordansche Normalform]
	Sei $V$ ein $k$-Vektorraum, $\dim V<\infty$, $\psi\in\End(V)$. Dann sind äquivalent:
	\begin{enumerate}
		\item $m_{\psi}$ zerfällt über $k$
		\item $\chi_{\psi}$ zerfällt über $k$
		\item $\exists \lambda_1,\dots, \lambda_n\in k \exists 1\leq s_{i1}\leq\dots\leq s_{il_i}$ und Basis $B'$ von $V$ mit
		$$M_{B'}(\psi) = \left( \begin{array}{cccc}
		J_{s_11}(\lambda_1) &  &  & 0 \\ 
		& J_{s_12}(\lambda_1) &  &  \\ 
		&  & \ddots &  \\ 
		0 &  &  & J_{s_{nl_n}}(\lambda_n)
		\end{array} \right)$$
	\end{enumerate}
\end{satz}
\begin{proof}
	\begin{description}
		\item[i$\Leftrightarrow$ ii:] Folgt aus Korollar 4.25
		\item[iii$\Rightarrow$ii:] $\checkmark$
		\item[i$\Rightarrow$ iii:] Beweis wie Beweis von Rationaler Normalform, statt Lemma 4.23 (i) nutze Lemma 4.23 (ii)
	\end{description}
\end{proof}
