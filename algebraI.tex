...
Nachtrag zu Bemerkung \ref{bem:2.17}: Sei $A = \IZ[T_1,\dots]$, $\phi = \ev_{T_i = 0}\colon A\to \IZ$, $M = {}_A\IZ$. Das heißt für $p\in A, z\in\IZ, p\cdot z = p(0,\dots)\cdot z$. Behauptung: $M$ ist endlich erzeugt, aber nicht präsentiert (als $A$-Modul).
\begin{proof}
	$M$ ist endlich erzeugt. $\checkmark$
	
	Angenommen $M$ ist endlich präsentiert. Dann: $\exists f\colon A^n \to M$ ($A$-linear und surjektiv), sodass $\ker f$ endlich erzeugt ist. %TODO: ersetze pfeil durch surjektionspfeil
	Sei $e_j = (0,\dots, 1,\dots, 0)\in A^n$, wobei die $1$ im $j$-ten Eintrag steht. Definiere nun $z_j = f(e_j)\in\IZ$.Dann ist $\ker f = \{\underline{p} = (p_1,\dots, p_n)\in A^n|\sum p_j(0)z_j = 0\}$.
	
	Aus der Annahme folgt nun: $\exists \underline{p^{(1)}},\dots, \underline{p^{(m)}}\in\ker f$ sodass $\ker f = \<\underline{p^{(1)}},\dots, \underline{p^{(m)}}\>_A$. Schreibe $\underline{p^{(i)}} = (p_1^{(i)}),\dots,p_n^{(i)})$. Wähle $N>0$, sodass $T_N$ taucht in keinem der $p_j^{(i)}$ auf. Betrachte $T_n\cdot e_j\in\ker f\Rightarrow \exists Q^{(i,j)}\in A$ mit $T_N\cdot e_j = \sum_{i = 1}^nQ^{(i,j)}\underline{p^{(i)}}$.
	
	Folglich gilt für alle $k\in\{1,\dots, n\}:\sum_{i = 1}^n Q^{(i,j)}\underline{p_k^{(i)}} = \delta_{jk}T_N$
...
\begin{proof}
	\begin{description}
		\item[Beweis von (ii)] Die Abbildung $(M\oplus N)\times P \to (M\otimes_AP) \oplus(N\otimes_AP), ((x,y),z)\mapsto (x\otimes z, y\otimes z)$ ist $A$-bilinear, folglich faktorisiert sie über das Tensorprodukt $(M\oplus N)\otimes_aP$:
		
		\begin{tikzcd}
					(M\oplus N)\times P \arrow{r}\arrow{d}& Y(M\otimes_AP)\oplus(N\otimes_AP) \\
					(M\oplus N)\otimes_AP \arrow{ru}[swap]{\exists!\lambda}	 \\
		\end{tikzcd}
	
		Folglich gilt: $\lambda((x,y)\otimes z) = (x\otimes z, y\otimes z)$.
...
\begin{bem}
	Sei $M$ ein $A$-Modul, $B$ eine $A$-Algebra. Betrachte $B\otimes_AM$. Fixiere $b'\in B$. Betrachte die Abbildung
	$\begin{array}{ccccc}
	B\times M & \longto & B\times M & \longto & B\otimes_AM\\
	(b,x) & \longmapsto & (b'b,x) & \longmapsto & b'b\otimes x
	\end{array}$
	Diese Abbildung ist bilinear ($\otimes_A$ ist $A$-bilinear, Multiplikation auf $B$ ist $A$-bilinear).
	
	\begin{tikzcd}
		(b,x)\longmapsto & (b'bx)\\ %TODO: FIX THIS!!!
		B\times M \arrow{r}{}\arrow{d}  & B\times M\arrow{d} \\
		B\otimes_AM \arrow{r}[swap]{\exists ! m_b A-\text{linear}}	& B\otimes_AM \\
	\end{tikzcd} %TODO: unteren Pfeil gestrichelt machen
\end{bem}
\begin{defi}
	Definiere nun 
	\begin{eqnarray*}
		m\colon B\times B\otimes_AM&\longto& B\otimes_AM\\
		(b', y) & \longmapsto & m_{b'}(y)
	\end{eqnarray*}

	Behauptung: $(B\otimes_AM, +,\cdot)$ ist $B$-Modul.
	\begin{proof}
	Exemplarisch Assoziativität: Seien $b'',b'\in B$ und $y\in B\otimes_AM$. Zeige $m(b'', m(b',y)) = m(b''b', y)$. Sei $y = \sum b_i\otimes x_i$ für $b_i7in B, x_i\in M$.
	
	Dann ist $m(b'', m(b', \sum b_i\otimes x_i)) = m(b'', \sum b'b_i\otimes x_i) = \sum b''b'b_i\otimes x_i = m(b''b', \sum b_i\otimes x_i)$.	
	\end{proof}
	$B\otimes_AM$ heißt Skalarerweiterung von $M$ nach $B$. (extension of scalars) %TODO: füge literaturverweis zu ext. of scalars ein
	
\end{defi}
\begin{beme}
	Sei $f\colon M\to N$ $A$-linear, $B$ eine $A$-Algebra. Dann ist $\id_B\otimes f\colon B\otimes_AM\to B\otimes_AM$ $B$-linear.
\end{beme}
\begin{proof}
	Sei $b'\in B, y = \sum b_i\otimes x_i\in B\otimes_AM$. Dann gilt:
	$$(\id_B\otimes f)(b'\cdot(\sum b_i\otimes x_i)) = (\id_B\otimes f)(\sum b'b_i\otimes x_i) =\sum \id_B(b'b_i)\otimes f(x_i) = $$
	$$=\sum b'b_i\otimes f(x_i) = b'(\sum b_i\otimes f(x_i)) = b'(\id_b\otimes f)(\sum b_i\otimes x_i)$$ %TODO: fix this hässlichkeit
\end{proof}
...
\begin{satz}[Adjunktion von Skalarerweiterung und Restriktion der Skalare]
	...
	\begin{enumerate}
		\item Sei $f\colon M\to {}_AN$ $A$-linear. Dann existiert genau ein $f_B\colon B\otimes_AM\to N$ $B$-linear mit $f_B(b\otimes x) = bf(x)$
		\item Die Abbildung $\Phi\colon \Hom_A(M,{}_AN)\to \Hom_B(B\otimes_AM,N), f\mapsto f_B$ ist $A$-linear und bijektiv.
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		...
		\begin{center}
			\begin{tikzcd}
				...
				B\otimes_AM \arrow{ur}[swap]{\exists! f_B A-\text{linear}}	& \\
			\end{tikzcd}
		\end{center}
		Zeige: $f_B$ ist $B$-linear (so wie in Bemerkung 3.9) %TODO: Referenz einfügen
		\item Konstruiere Umkehrabbildung $\Psi\colon \Hom_B(B\otimes_AM,N)\to \Hom_A(M,{}_AN)$. Sei $g\colon B\otimes_AM\to N$. Definiere $\Psi(g) \coloneqq g(1\otimes \_)\colon M\to N, x\mapsto g(1\otimes x)$.
		Dabei ist $g(1\otimes\_)$ tatsächlich $A$-linear: $g(1\otimes ax ) = g(a(1\otimes x)) = ag(1\otimes x)$.
		
		Zeige nun noch $(\Psi\circ \Phi)(f) = f$ und $(\Phi\circ\Psi)(g) = g$:
		$$((\Psi\circ\Phi)(f))(x) = (\Phi(f))(1\otimes x) = f_b(1\otimes x) =1\cdot f(x)$$
		und
		$$((\Phi\circ\Psi)(g))(b\otimes x) = (\Psi(g))_B(b\otimes x) = b\cdot (\Psi(g))(x) = b\cdot g(1\otimes x) = g(b\otimes x)$$
...
\begin{lem}
	...
	\begin{enumerate}
		\item $N\otimes_AM$ ist ein $B$-Modul via $b\cdot(sum y_i\otimes x_i) = \sum by_i\otimes x_i$.
		\item $\exists!$ Isomorphismus $N\otimes_AM\to N\otimes_B(B\otimes_AM)$ von $B$-Moduln mit $y\otimes x\mapsto y\otimes (1\otimes x)$.
	\end{enumerate}
\end{lem}
\begin{proof}
	\leavevmode
...
\begin{satz}
	...
	$$C\otimes_A M  \longmapsto  C\otimes_B(B\otimes_AM)$$
	von $C$-Moduln mit $c|otimes x\mapsto c\otimes(1\otimes x)$.
\end{satz}
\begin{proof}
	Aus Lemma 3.11 folgt: Es existiert genau ein Isomorphismus von $B$-Moduln
	$$ f\colon C\otimes_AM\longto C\otimes_B(B\otimes_AM)$$
	mit $f(c\otimes x) = c\otimes(1\otimes x)$. Zeige nun noch: $f$ ist $C$-linear.
	
	Sei $z = \sum c_i\otimes x_i, c\in C$: 
	$$f(c\sum c_i\otimes x_i) = f(\sum cc_i\otimes x_i ) = \sum f(cc_i\otimes x_i) = \sum cc_i\otimes(1\otimes x_i) = c\sum c_i\otimes(1\otimes x_i)$$
\end{proof}
...
\begin{satz}
	...
	\begin{enumerate}
		...
		\item flach.
	\end{enumerate}	
	Dann gilt: Falls $M$ die Eigenschaft $(P)$ als $A$-Modul hat, hat $B\otimes_AM$ die Eigenschaft $(P)$ als $B$-Modul.
\end{satz}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item frei: Sei $M$ freier $A$-Modul $\Rightarrow M \cong \bigoplus\limits_{i\in I}A\Rightarrow B\otimes_AM\cong B\otimes (\bigoplus\limits_{i\in I}A) \cong \bigoplus\limits_{i\in I}(B\otimes_A A) \cong \bigoplus\limits_{i\in I} B$ als $B$-Moduln (Bemerkung 3.9).
		...
		$$\id_B\otimes f\colon B\otimes_aA^n\to B\otimes_AM$$ surjektiv nach Proposition 2.31 und $B$-linear nach Bemerkung 3.9. Aber wir haben eben gesehen, dass $B\otimes_AA^n \cong B^n\Rightarrow B\otimes_AM$ endlich erzeugt.
		...
		$$B\otimes A^m \longto B\otimes A^n \longto B\otimes_AM\longto 0$$
		Damit ist auch $B\otimes_AM$ endlich präsentiert als $B$-Modul.
		\item flach: Übung.
	\end{enumerate}
\end{proof}
