...
\begin{satz}
	...
	\begin{enumerate}
		...
		\item $(B\otimes_AC, +, m)$ ist ein Ring (und sogar eine $A$-Algebra). \label{thm:314:ii}
...
\begin{defi}
	Sei $A$ Ring. $S\subseteq A$ heißt multiplikative Teilmenge, wenn:
	\begin{itemize}
		\item $1\in S$
		\item Falls $a,b\in S\Rightarrow a\cdot b \in S$
	\end{itemize}
	Definiere auf $S\times A$ Äquivalenzrelation durch
	$$(s,a) \sim (t,b) \Leftrightarrow \exists u\in S: u(ta-sb) = 0$$
	Definiere $S^{-1}A = (S\times A)/\sim$. Das wird zu einem Ring durch (definiere $\frac as \coloneqq$ Äquivalenzklasse von $(s,a)$)
	$$\frac as+\frac bt = \frac {ta+sb}{st},\qquad \frac as\cdot \frac bt = \frac{ab}{st}$$
	(Wohldefiniertheit siehe Einführung in die Algebra.)
	
	Es gibt eine Abbildung $f\colon A\to S^{-1}A, a\mapsto \frac a1$; diese ist ein Ringhomomorphismus.
\end{defi}
\begin{bem}
	\leavevmode
	\begin{enumerate}
		...
		\item $\frac{ta}{ts} = \frac as$ (alle $a\in A, s,t\in S$).
	\end{enumerate}
	
\end{bem}
\begin{satz}
	Sei $S\subseteq A$ multiplikative Teilmenge, sei $g\colon A\to B$ Ringhomomorphismus mit $g(S) \in B^{\times}$. Dann kommutiert folgendes Diagramm:
	
	\begin{center}
	\begin{tikzcd}
		A \arrow{d}[swap]{f} \arrow{r}{g} & B \\
		S^{-1}A \arrow{ur}[swap]{\exists! g' \text{ Ringhomomorphismus}}	& 
	\end{tikzcd}
	\end{center}
\end{satz}
\begin{proof}
	Einführung in die Algebra oder Atiyah-Macdonald Proposition 3.1
\end{proof}

\begin{bsp}
	Sei $A$ Ring.
	\begin{enumerate}
		\item Sei $p\in \operatorname{Spec} A \Rightarrow S\coloneqq A\setminus p$ ist multiplikative Teilmenge. Definiere $A_p \coloneqq S^{-1}A$.
		
		Spezialfall: $A$ Integritätsbereich $\Rightarrow p\coloneqq (0)\in\Spec A\Rightarrow A_p = \operatorname{Quot}(A)$.
		\item Sei $t\in A\Rightarrow S\coloneqq \{t^n|n\in\IZ_{\geq 0}\}$ ist eine multiplikative Teilmenge. Definiere $A[t^{-1}] \coloneqq S^{-1}A$ (bei Atiyah-Macdonald heißt das $A_t$).
	\end{enumerate}
	
\end{bsp}
\begin{defi}
	Sei $S\subseteq A$ multiplikative Teilmenge, sei $M$ ein $A$-Modul. Definiere auf $S\times M$ Äquivalenzrelation durch
	$$(s,x)\sim (t,y) \Leftrightarrow \exists u\in S: u(tx-sy) =0$$
	Definiere damit $S^{-1}M\coloneqq (S\times M)/\sim$; $\frac xs\coloneqq$ Äquivalenklasse von $(s,x)$. Dann wird $S^{-1}M$ zu $S^{-1}A$-Modul via
	$$\frac cs +\frac yt \coloneqq \frac{tx+sy}{st},\qquad \frac{a}{s}\cdot \frac{y}{t}\coloneqq \frac{ay}{st}$$
...
\begin{bem}
	...
	$$S^{-1}f(\frac{x}{s}) \coloneqq \frac{f(x)}{s}$$
	Das ist wohldefiniert und $S^{-1}A$-linear.
	\begin{proof}
		Wir beweisen beispielhaft die Wohldefiniertheit: Sei $\frac xs = \frac yt$ in $S^{-1}M$, das heißt $\exists u\in S$ mit $u(tx-sy) = 0$. Also gilt $0 = f(u(tx-sy)) = u(tf(x)-sf(y))$, also $\frac{f(x)}{s} = \frac{f(y)}{s}$.
	\end{proof}
	Es gilt für $M\xrightarrow{f}N\xrightarrow{g}P$ mit $f,g$ beide $A$-linear: $S^{-1}(g\circ f) = (S^{-1}g)\circ(S^{-1}f)$.
\end{bem}
\begin{satz}
	Sei $M\xrightarrow{f}N\xrightarrow{g}P$ mit $f,g$ beide $A$-linear. Falls $\ker g = \im f$ gilt, dann ist $\ker(S^{-1}g) = \im (S^{-1}f)$.
\end{satz}
\begin{proof}
	Sei $\ker g = \im f$.
	\begin{description}
		\item[\glqq$\supseteq$\grqq:] $(S^{-1}g)\circ(S^{-1}f) = S^{-1}(g\circ f) = S^{-1}0 = 0$.
		\item[\glqq$\subseteq$\grqq:] Sei $\frac yt\in \ker(S^{-1}g)$. Das heißt, $0 = (S^{-1}g)(\frac yt) = \frac{g(y)}t$.
		
		Also gibt es ein $u\in S$ mit $ug(y) = 0$. Damit wissen wir, dass $uy\in\ker g = \im f$ gilt. Wir finden folglich ein $x\in M$ mit $f(x) = uy$. Betrachte $\frac x{ut}$. Dafür gilt $(S^{-1}f)(\frac x{ut}) = \frac{f(x)}{ut} = \frac{uy}{ut} = \frac yt$.
	\end{description}
...
\end{proof}
\begin{satz}
	Sei $S\subseteq A$ multiplikative Teilmenge und $M$ ein $A$-Modul. Dann gibt es genau einen Isomorphismus von $S^{-1}A$-Moduln
	$$ S^{-1}A\otimes_AM\xlongrightarrow{\phi} S^{-1}M$$
	mit $\phi(\frac as \otimes x) = \frac{ax}s$.
\end{satz}
\begin{proof}
	Betrachte $$S^{-1}A\times M\to S^{-1}M, (\frac as,x)\mapsto \frac{ax}s$$
	Das ist wohldefiniert und $A$-bilinear. Also erhalten wir folgendes kommutatives Diagramm:
	\begin{center}
	\begin{tikzcd}
		S^{-1}A\times M \arrow{r} \arrow{d}& S^{-1}M  \\
		S^{-1}A\otimes_AM\arrow{ru}[swap]{\exists!\phi A-\text{linear}} 
	\end{tikzcd}
	\end{center}
	Rechne nach: $\phi$ ist $S^{-1}A$-linear. Sei dazu $\frac bt \in S^{-1}A$, $z\in S^{-1}A\otimes_A M\Rightarrow z = \sum \frac {a_i}{s_i}\otimes x_i$ für $a_i\in A, s_i\in S, x_i\in M$. Wir erhalten also
	$$\phi(\frac{bt}\cdot z) =\phi(\sum\frac{ba_i}{ts_i}\otimes x_i ) = \sum\phi(\frac{ba_i}{ts_i}\otimes x_i) = \sum \frac{ba_ix_i}{ts_i} = \frac{b}{t} \cdot \sum \frac{a_ix_i}{s_i} = \frac bt \cdot \phi(\sum\frac{a_i}{s_i}\otimes x_i)$$
	Damit ist $\phi$ tatsächlich $S^{-1}A$-linear.
	
	Zeige noch: $\phi$ ist Isomorphismus.
	\begin{itemize}
		\item $\phi$ surjektiv: $\checkmark$
		\item $\phi$ injektiv:
		
		Behauptung: für alle $z\in S^{-1}A\otimes_AM$ existieren $x\in M, s\in S$, sodass $z = \frac 1s\otimes x$.
		
		Beweis der Behauptung: $\exists a_i\in A, s_i\in S, x_i\in M: z = \sum\frac{a_i}{s_i}\otimes x_i$. Definiere $s = \prod_i s_i, t_i = \prod {j \neq i} s_j$. Damit folgt
		$$ z = \sum \frac{a_it_i}{s}\otimes x_i = \sum \frac 1s \otimes a_it_ix_i = \frac{1}{s}\otimes\sum a_it_ix_i$$
		
		Zurück zur Injektivität: Sei $z\in S^{-1}A\otimes_AM$ mit $\phi(z) = 0$. Wähle $s,x$ mit $z = \frac 1s\otimes z$. Also haben wir $0 = \frac xs \Rightarrow \exists u\in S$ mit $ux = 0$. Damit gilt auch $0 = \frac 1{su}\otimes ux = \frac{u}{su}\otimes x = \frac{1}{s}\otimes x = z$.
	\end{itemize}
\end{proof}
\begin{kor}
...
\end{kor}
\begin{proof}
	Proposition 5.8 und Korollar 5.6
\end{proof}

\begin{satz}
	Seien $M,N$ zwei $A$-Moduln, $S\subseteq A$ multiplikative Teilmenge. Dann existiert genau ein Isomorphismus von $S^{-1}A$-Moduln
	$$\phi\colon S^{-1}M\otimes_{S^{-1}A}S^{-1}N\longto S^{-1}(M\otimes_AN)$$
	mit $\phi(\frac xs\otimes \frac yt) = \frac{x\otimes y}{st}$.
\end{satz}
\begin{proof}
	$\begin{array}{rcll}
		S^{-1}M\otimes_{S^{-1}A}S^{-1}N &\cong& S^{-1}M \otimes_{S^{-1}A}(S^{-1}A\otimes_AN) &\text{(Prop. 5.8)} \\
		&\cong& S^{-1}M\otimes_A N &\text{(Lemma 3.11)}\\
		&\cong& S^{-1}A\otimes_AM\otimes_AN& \\ 
		&\cong& S^{-1}(M\otimes_AN)&
	\end{array}$
	Verfolgen der Bilder von $\frac xs\otimes \frac yt$ unter Komposition liefert tatsächlich $\phi(\frac xs\otimes \frac yt) = \frac{x\otimes y}{st}$.
\end{proof}

\subsection{Lokal-global Prinzipien}
\begin{satz} Sei $M$ ein $A$-Modul. Dann sind äquivalent:
	\begin{enumerate}
		\item $M = 0$
		\item Für alle $ p\in \Spec A$ gilt $M_p = 0$
		\item Für alle $m \in\Max A$ gilt $M_m = 0$.
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{description}
		...
		\item[iii$\Rightarrow$ i:] Angenommen $M \neq 0$. Sei $x\in M, x \neq 0$. Dann ist $\Ann(x) \neq (1)$, also existiert ein $m\in \Max A$ mit $Ann(x)\subseteq m$. Aus (iii) folgt dann, dass $M_m = 0$ ist, also $\frac x1 = 0$ in $M_m$; folglich gibt es$n\in A\setminus m$ mit $ nx = 0\Rightarrow n\in Ann(x)\subseteq m$, was ein Widerspruch ist.
	\end{description}
\end{proof}

\begin{satz} Sei $f\colon M\to N$ eine $A$-lineare Abbildung. Dann sind äquivalent:
	\begin{enumerate}
		\item $f$ ist injektiv respektive surjektiv
		\item Für alle $p\in \Spec A$ gilt $f_p\colon M_p\to N_p$ ist injektiv respektive surjektiv
		\item Für alle $m\in \Max A$ gilt: $f_m\colon M_m\to N_m$ ist injektiv respektive surjektiv
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{description}
		\item[i$\Rightarrow$ ii:] Folgt aus Exaktheit (Korollar 5.6)
		\item[ii$\Rightarrow$ iii:] $\checkmark$
		\item[iii$\Rightarrow$ i:] Betrachte folgende exakte Sequenz
		$$0\longrightarrow \ker f \longrightarrow M \xlongrightarrow{f} N\longrightarrow \operatorname{coker} f \longrightarrow 0$$
		(von $A$-Moduln). Sei $m\in \Max A$. Dann ist 
		$$0\longrightarrow (\ker f)_m \longrightarrow M_m \xlongrightarrow{f_m} N_m\longrightarrow (\operatorname{coker} f)_m \longrightarrow 0$$
		exakt nach Korollar 5.6. Also ist $\ker(f_m) \cong (\ker f)_m$, genauso $\operatorname{coker}(f_m)\cong (\operatorname{coker} f)_m$.
		
		Da $f_m$ injektiv ist, ist $\ker (f_m) = 0$. Das impliziert, dass für alle $m\in \Max A$ gilt: $(\ker f)_m = 0$, und mit Proposition 5.11 folgt $\ker f = 0$, also $f$ injektiv.
		
		Genauso für surjektiv.\qedhere
	\end{description}
\end{proof}
...
\begin{satz}
	...
	\begin{enumerate}
		\item $M$ ist flacher $A$-Modul.
		\item Für alle $p\in\Spec A$ ist $M_p$ flacher $A_p$-Modul
		\item Für alle $p\in\Spec A$ ist $M_p$ flacher $A$-Modul
		\item Für alle $m\in\Max A$ ist $M_m$ flacher $A_m$-Modul
		\item Für alle $m\in\Max A$ ist $M_m$ flacher $A$-Modul.
	\end{enumerate}
...
\begin{itemize}
	...
	\item $I\subseteq A$ Ideal: $I\cdot B \coloneqq (f(I)) \subseteq B$ Ideal
	\item $I \subseteq (I\cdot B) \cap A$, $J \supseteq (J\cap A)\cdot B$
\end{itemize}
\begin{satz}
	Sei $S\subseteq A$ multiplikative Teilmenge.
	\begin{itemize}
		\item Sei $I\subseteq A$: $I\cdots S^{-1}A : \{\frac{a}{s}|a\in I, s\in S\}$ und $(I\cdot S^{-1}A)\cap A = \{a\in A|\exists n\in S: na \in I\}$.
		
		Insbesondere: $I\cdot S^{-1}A = (1)\Leftrightarrow I\cap S\neq \emptyset$.
		\item Sei $J\subseteq S^{-1}A$ Ideal: $(J\cap A)\cdot S^{-1}A = J$
		\item Falls $p\in\Spec A$ mit $p\cap S = \emptyset$: $p\cdot S^{-1}A\in \Spec (S^{-1}A)$ und $(p\cdot S^{-1}A)\cap A = p$
		\item Die Abbildungen 
		$$\Spec(S^{-1}A) \xlongleftrightarrow{1:1} \{p\in \Spec A|, p\cap S = \emptyset\}$$
		mit $q\mapsto q\cap A$, $p\cdot S^{-1}A\leftarrow p$ sind wohldefiniert, zueinander inverse Bijektionen und %TODO: ersetze leftarrow
		 inklusionserhaltend (das heißt: falls $p\subseteq p' \Rightarrow p\cdot S^{-1}A\subseteq p'\cdot p'S^{-1}A$ und falls $q\subseteq q'\Rightarrow q\cap A\subseteq q'\cap A$).
	\end{itemize}
\end{satz}
