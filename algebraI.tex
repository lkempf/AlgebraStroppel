...
\begin{lem} \label{lem:lokale ringe}
	...
	\begin{enumerate}
		\item Sei $I \subsetneq A$ ein Ideal. Dann sind äquivalent:
		      \begin{enumerate}
			      \item $\Max A = \{I\}$
			      \item $A \setminus I \subseteq A^{\times}$
...
\begin{lem}
    ...
    \begin{enumerate}
        ...
        \item $I\subseteq \sqrt{I} = \sqrt{\sqrt{I}}$
...
\end{kor}
\begin{proof}
	Da $M$ endlich erzeugt ist, existiert ein freier $A$-Modul $F$ mit $\rk F<\infty$ und eine surjektive, $A$-lineare Abbildung $f\colon F\to M$. Betrachte $f^{-1}(M')\subseteq F$. Nach \cref{thm:untermoduln freier moduln in hir} ist $F' := f^{-1}(M')$ nun frei und es gilt $\rk F'\leq \rk F <\infty$ und $f|_{F'}\colon F'\to M'$ ist somit surjektiv, also ist $M'$ endlich erzeugt.
\end{proof}
...
\begin{satz}[Noether-Normalisierung] \label{thm:noether-normalisierung}
	Sei $k$ ein Körper und $A$ eine endlich erzeugte $k$-Algebra. Seien $I_1\subseteq \dots \subseteq I_m\subsetneq A$ Ideale. Dann existiert ein $n\geq 0$ und $a_1,\dots, a_n\in A$ sowie $0\leq h_1\leq\dots \leq h_m\leq n$, sodass die folgenden Aussagen gelten. 
	\begin{enumerate}
...
\begin{defi}
	\leavevmode
	\begin{enumerate}
		\item Sei $n\geq 0$. $\IA^n= \IA^n (k) := k^n$ heißt \emph{$n$-dimensionaler affiner Raum}.
...
\paragraph{Beispiel.}
	...
	Sei $f\in \I(Y)\subseteq k[t_1,t_2]$, schreibe $f(t_1,t_2) = \sum_{i,j\geq 0}a_{ij}t_1^it_2^j$. Es gilt für alle $x\in k$ nun $0 = f(x,0) = \sum_{i\geq0}a_{i0}x^i$ und $0 = f(0,x) = \sum_{j\geq 0} a_{0j}x^j$. Da $\abs k = \infty $ ist, folgt $f(0,t_2) = f(t_1,0) = 0$. Damit erhalten wir
...
\begin{beme}
	...
	\begin{eqnarray*}
		\alpha: k[t_1,\dots, t_n] &\longrightarrow& \Abb(X,k), \\
		f &\longmapsto& \alpha(f)\colon X\to k, x\mapsto f(x).
	\end{eqnarray*}
	$\alpha$ ist ein Ringhomomorphismus. Es gilt
	\begin{align*}
		\im \alpha = \Hom(X,\IA^1) \quad\text{und}\quad \ker \alpha = \I(X).
	\end{align*}
	Somit induziert $\alpha$ einen Isomorphismus $\beta$.
	\begin{center}
		\begin{tikzcd}
...
\begin{bem} Sei $X\subseteq \IA^n$ algebraische Teilmenge, $A := \A(X)$. Haben Zariski-Topologie auf $X$:
...
\begin{beme}\leavevmode \begin{enumerate}
	...
	\item
		...
		(Denn: Betrachte Bild $\overline{f_i}$ von $s_i$ unter $k[s_1,\dots, s_m]\to \A(Y)\xto{\phi}\A(X)$. Wähle Urbild $f_i$ von $\overline{f_i}$ unter $k[t_1,\dots, t_n]\to \A(X)$. Definiere $\Phi := \ev_(f_1,\dots, f_m)$. (nicht eindeutig bestimmt!))
		...
		$\forall x \in X: f_i(x) = f_i'(x)$, also $F(x) = F'(x) := (f_1'(x),\dots, f_m'(x))$. Damit ist $f$ tatsächlich unabhängig von diesen Wahlen.
		
		Notation: $\phi^{\#} := f$. Es gilt: für $k$-Algebrenhomomorphismen: $\A(Z)\xto{\psi} \A(Y)$ ist $(\phi\circ \psi)^{\#} = \psi^{\#}\circ \phi ^{\#}$
...
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Sei $f\in\Hom(X,Y)$. Zeige $(f^*)^{\#} = f$. Definiere $\phi := f^*$. Betrachte Diagramm (*)
		\begin{center}
			\begin{tikzcd}
...
\begin{defi}
	Sei $X\subseteq \IA^n$ algebraische Teilmenge, $h\in A(X)$.
	$$D(h)\coloneqq \set{x\in X\given h(x) \neq 0} = X\setminus Z(h)$$
	heißt offene Hauptmenge von $X$. (\glqq principal open subset\grqq)
\end{defi}
\begin{beme}
	Offene Hauptmengen lassen sich als algebraische Teilmengen realisieren: $X\subseteq \IA^n\Rightarrow D(h)\subseteq \IA^{n+1}$ abgeschlossen. Es gilt $A(D(h))\cong A(X)[h^{-1}]$. 
\end{beme}
\begin{lem}
	Sei $X\subseteq \IA^n$ algebraische Teilmenge, $U\subseteq X$ offen. Dann gibt es $h_1,\dots, h_m\in A(X)$, sodass
	$$U = \bigcup\limits_{i = 1}^m D(h_1)$$
\end{lem}
\begin{proof}
	...
	$$Z  = \set{x\in X\given h_1(x) = \dots = h_m(x)= 0}$$
	und
	$$U = X\setminus U = \set{x\in X\given \exists i: h_i(x) \neq 0} = \bigcup\limits_{i = 1}^m D(h_m)$$
\end{proof}
\begin{prop}
	Seien $X\subseteq\IA^n, Y\subseteq \IA^m$ algebraische Teilmengen, $f\colon X\to Y$ Morphismus. Dann ist $f$ stetig.
\end{prop}
\begin{proof}
	Nach Lemma 8.31 genügt es zu zeigen: $\forall h\in A(Y)$ ist $f^{-1}(D(h))\subseteq X$ offen. Dabei ist $f^{-1}(D(h)) = \set{x\in X\given h(f(x))\neq 0} = \set{x\in X\given (f^*h)(x)\neq 0} = D(f^*h)$.
\end{proof}
\begin{beme}
	Sei $X = Z(t_1^3-t_2^2) = \set{(x_1,x_2)\given x_1^3 = x_2^2}\subseteq \IA^2$ und $Y = \IA^1$. Dann ist $f\colon \IA^1\to X, x\mapsto (x^2, x^3)$ ein Morphismus. $f$ ist bijektiv. $f$ ist offen (das heißt Bilder offener Teilmengen sind offen): Sei $h\in A(\IA^1) = k[t]$ und betrachte $D(h)$. Sei ohne Einschränkung $h(t) = (t-x_1)\dots (t-x_n)$. Also ist $D(h) = \IA^1\setminus\set{x_1,\dots, x_n}$. Damit haben wir $f(D(h)) = \set{f(x)\given x\in D(h)} = X\setminus\set{f(x_1),\dots, f(x_n)}$, wobei wir hier ausnutzen, dass $f$ bijektiv ist. Aber endliche Teilmengen sind abgeschlossen $\Rightarrow f(D(h))$ offen.
	
	Folglich ist $f$ ein Homöomorphismus. Aber: $f$ ist kein Isomorphismus: Betrachte $f^*\colon A(X)\to A(\IA^1)$, wobei $A(\IA^1) = k[t], A(X) = k[t_1,t_2]/(t_1^3-t_2^2)$. (Beachte $t_1^3-t_2^2$ irreduzibel).
	$$f^*\colon k[t_1,t_2]/(t_1^3-t_2^2) \longto k[t], t_1\longmapsto t^2, t_2\longmapsto t^3$$
	$f^*$ ist kein Isomorphismus, denn $t\notin \im f^*$. Nach Proposition 8.27 ist damit auch $f$ kein Isomorphismus.
	
	Außerdem: $\IA^1$ und $X$ sind nicht isomorph: Betrachte $(0,0)$ auf $X$. Dieser entspricht dem maximalen Ideal $(t_1,t_2)/(t_1^3-t_2^2) = m$ von $A(X)$.
	$$\fak{m}{m^2} \cong \fak{(t_1,t_2)}{(t_1^2, t_1t_2, t_2^2, t_1^2-t_2^2)} = \<t_1,t_2\>_k$$
	und $t_1, t_2$ in $\fak{m}{m^2}$ linear unabhängig.
	
	Aber für $x\in\IA^1$ gilt $m_x = (t-x)\Rightarrow \fak{m_x}{m_x^2}\cong \<t-x\>_k$.
	
	\noindent $\Rightarrow$ Es kann keinen Isomorpismus $\IA^1\to X$ geben.
\end{beme}

\section{Dimension}
\begin{defi}
	Sei $X$ topologischer Raum. Definiere $\dim X \in \IZ_{\geq 0}\cup \set{\pm \infty}$ als 
	$$\dim X\coloneqq \sup \set{l\given \exists X_0\supsetneq X_1\subsetneq\dots \subsetneq X_l \neq \emptyset,\enspace X_i\subseteq X\text{ abgeschlossen und irreduzibel}}$$
	(formal $\dim\emptyset = -\infty$)
\end{defi}

Falls $X$ Varietät ist (oder algebraische Teilmenge) $\subseteq \IA^n$
$$\set{\text{abg. irreduzible Teilmenge }Z\subseteq X}\xlongleftrightarrow{1:1}\set{\text{Primideale von }A(X)}$$
Das führt zu folgender Definition:
\begin{defi}
	Sei $A$ Ring. Definiere $\dim A\in \IZ_{\geq 0}\cup\set{\pm\infty}$ als
	$$\dim A \coloneqq \sup \set{l\in\IZ_{\geq 0}\given \exists \fp_0\subsetneq \dots \subsetneq \fp_l,\enspace \fp_i\in\Spec A}$$
	(formal $\dim 0 = -\infty$)
\end{defi}
\begin{bsp}
	\leavevmode
	\begin{enumerate}
		\item Sei $k$ Körper $\Rightarrow \dim k = 0$
		\item $A$ Hauptidealring, kein Körper $\Rightarrow \dim A = 1$ (das heißt bspw $\dim \IZ= 1$, $\dim k[t] = 1$ (geometrisch $\dim \IA^1 = 1$)
		\item In $k[t_1,\dots, t_n]$ betrachte Primidealkette
		$$(0)\subsetneq (t_1)\subsetneq \dots \subsetneq (t_1,\dots, t_n)$$
		$\Rightarrow \dim k[t_1,\dots, t_n]\geq n$, also $\dim\IA^n \geq n$. (Tatsächlich gilt Gleichheit, das sehen wir später.)
	\end{enumerate}
\end{bsp}

\begin{beme}
	Sei $A$ Ring, $\dim A = d \in\IZ_{\geq 0}\Rightarrow \exists $ Primidealkette $\fp_0\subsetneq \dots \subsetneq \fp_d$ in $A$. Dann ist diese Kette maximal, das heißt, es gibt kein $\fp $ in $\Spec A$ mit $\fp\subsetneq \fp_0$ oder $\fp_i\subsetneq \fp\subsetneq \fp_{i+1}, \fp_d\subsetneq \fp$. Insbesondere:
	\begin{enumerate}
		\item $\fp_0\in\operatorname{MinSpec}A\Rightarrow \dim A= max_{\fp_0\in \operatorname{MinSpec}A}\dim \fak a{\fp_0}$
		\item $\fp_d\in \Max A\Rightarrow \dim A = \max_{m\in Max A} A_m$
	\end{enumerate}
\end{beme}
\begin{beme}
	...
	\begin{enumerate}
		...
		\item $\dim X = \max_{x\in X}\dim A(X)_{m_x}$ (falls $X$ irreduzibel, dann folgt $\forall x\in X$: $\dim A(X)_{m_x} = \dim A(X)$; zeigen wir später)
	\end{enumerate}
\end{beme}
\begin{bsp}
...
\end{bsp}
\begin{prop}
	Sei $A\subseteq B$ ganze Ringerweiterung.
	\begin{enumerate}
		\item $\dim A = \dim B$
		\item $\forall \fq\in\Spec B$ ist $\dim A_{\fq\cap A}\geq \dim B_{\fq}$.
		\item Falls $A\subseteq B$ Going down erfüllt (d.h. $\forall \fp_1, \fp_2\in \Spec A, \fp_1\subseteq \fp_2\forall \fq_2\in\Spec B, \fq_2\cap A = \fp_2\exists \fq_1\in\Spec B: \fq_1\subseteq \fq_2, \fq_1\cap A = \fp_1$), dann gilt für alle $\fq\in\Spec B$ bereits Gleichheit: $\dim A_{\fq\cap A} =\dim B_{\fq}$
	\end{enumerate}
\end{prop}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Zeige $\dim A\geq \dim B$: Sei $\fq_0\subsetneq \dots \subsetneq \fq_l$ Primidealkette in $B$. Dann erhalten wir $\fq_0\cap A \subseteq \dots \subseteq \fq_l\cap A$ Primidealkette von $A$. Proposition 6.8 (iii) sagt uns, dass diese Inklusionen dabei alle echt sind. 
		
		Zeige noch $\dim A \leq \dim B$: Sei $\fp_0\subsetneq\dots\subsetneq \fp_d$ Primidealkette in $A$. Da $A\hookrightarrow B$ injektiv, folgt ebenfalls mit Proposition 6.8, dass es $\fq_0\in \Spec B$ gibt mit $\fq_0\cap A = \fp_0$. Wir haben also folgende Situation:
		\begin{center}
			\begin{tikzcd}
				B & \fq_0\arrow[mapsto]{d}&\\
				A\arrow[Subseteq]{u}& \fp_0&\subsetneq\dots \subsetneq \fp_d
			\end{tikzcd}
		\end{center}
		Mit Going up folgt, dass es $\fq_1\subsetneq\dots \subsetneq \fq_d$ gibt mit $\fq_i\cap A = \fp_i$.
		
		\item Primidealkette $\fq_0\subsetneq \dots \subsetneq \fq_l = \fq$ in $B$. Nach Proposition 6.8 (iii) gilt dann $\fq_0\cap A\subsetneq \dots \subsetneq \fq_l\cap A = \fq \cap A$.
		\item Sei $\fp_0\subsetneq \dots \subsetneq \fp_l = \fq \cap A$. Wir haben also folgende Situation:
		\begin{center}
			\begin{tikzcd}
				B & \fq\arrow[mapsto]{d}&\\
				A\arrow[Subseteq]{u}& \fq\cap A=\fp_l&\supsetneq\dots \supsetneq \fp_0
			\end{tikzcd}
		\end{center}
	\end{enumerate}
\end{proof}
...
\begin{defi}
	Sei $A$ Ring, $\fp\in\Spec A, n\geq 0$:
	$$\fp^{(n)}\coloneqq (\fp^n\cdot A_{\fp})\cap A$$
	heißt $n$-te sympolische Potenz von $\fp$.
\end{defi}
\begin{lem}
	$A, \fp, n$ wie oben.
	\begin{enumerate}
		\item $\fp^{(n)} = \set{ a\in A\given \exists s\in A\setminus \fp: sa \in \fp ^n}$
		\item $\fp^{(0)}\supseteq \fp^{(1)}\supseteq \fp^{(2)}\supseteq \dots$, wobei $\fp^{(0)} = A$ und $\fp^{(1)} = \fp$
		\item $\fp^n \subseteq \fp^{(n)}$
		\item $\fp^{(n)}\cdot A_{\fp} = \fp^n\cdot A_{\fp} = (\fp\cdot A_{\fp})^n$
	\end{enumerate}
\end{lem}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item \begin{description}
			\item[\glqq$\supseteq$\grqq:] Sei $a\in A$ mit $\exists s\in A\setminus \fp: sa\in \fp^n$. Dann ist $\frac a1 = \frac {sa}s\in \fp^n\cdot A_{\fp}\Rightarrow a\in (\fp^n\cdot A_{\fp})\cap A = \fp^{(n)}$.
			\item[\glqq$\subseteq$\grqq:] Sei $a \in (\fp^n\cdot A_{\fp})\cap A\Rightarrow \frac a1\in \fp^n\cdot A_{\fp}$. Es gibt also $b \in \fp^n, t\in A\setminus \fp$ mit $\frac a1 = \frac bt\Rightarrow \exists u\in A\setminus \fp$ mit $s\coloneqq uta = ub \in \fp^n\Rightarrow s\in A\setminus \fp$.
		\end{description}
		\item $\checkmark$
		\item $\checkmark$
		\item $\fp^(n)\cdot A_{\fp} = ((\fp^n\cdot A_{\fp})\cap A)\cdot A_{\fp} = \fp^n \cdot A_{\fp}$. (Allgemein gilt $((I\cdot B)\cap A)\cdot B = I\cdot B$.)
		
		Zeige $\fp^n\cdot A_{\fp} = (\fp\cdot A_{\fp})^n$:
		\begin{description}
			\item[\glqq$\subseteq$\grqq:] Sei $b\in \fp^n\Rightarrow \exists b_{ij}\in \fp$ mit $b = \sum b_{i1}\dots b_{in}$. Für $s\in A\setminus \fp$ folgt $\frac bs = \sum \frac{b_{i1}}{s}\cdot \frac{b_{i2}}{1}\dots \frac{b_{in}}{1}\in (\fp\cdot A_{\fp})^n$. 
			\item[\glqq$\supseteq$\grqq:] Sei $c \in (\fp\cdot A\fp)^n\Rightarrow \exists b_{ij}\in\fp, s_{ij}\in A\setminus \fp$ sodass $c = \sum \frac{b_{i1}}{s_{i1}}\cdots \frac{b_{in}}{s_{in}}$. Definiere
			$$s\coloneqq \prod_{ij}s_ij, b\coloneqq \sum_i(\prod{i\neq j, j = 1,\dots, n}s_{ij})\prod_{j = 1}^n b_{ij}$$
			$\Rightarrow s\in A\setminus \fp, b\in \fp^{(n)}$ und $c = \frac bs\in \fp^n\cdot A_{\fp}$
		\end{description}
	\end{enumerate}
