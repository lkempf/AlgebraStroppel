	...
	\begin{enumerate}
        ...
        \begin{center}
            \begin{tikzcd}
                    ...
                    \fak M{M'} \arrow[dashrightarrow]{ur}{\ol{f}}
            \end{tikzcd}
        \end{center}
...
\begin{proof}
	...
	\begin{enumerate}[label=\ref{lem:homsatz moduln:\roman*}]
        ...
        \begin{center}
            \begin{tikzcd}
                M \arrow{r}{f} \arrow{d}{\pi} & N \\
                \fak M{\ker f} \arrow[dashrightarrow]{ur}{\ol{f}}
            \end{tikzcd}
        \end{center}
...
\begin{proof}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	Betrachte $F\colon \IA^n\to \IA^m$. Es gilt $\phi^{\#}(x) = F(x)$ für alle $x\in X$. Für alle $g\in \Os_{\IA^m}(\IA^m)$ und $x\in X$ gilt $g(F(x)) = g(f(x))$, wie durch Verfolgen des äußeren Weges im folgenden Diagramm klar ist. %das tim hoffentlich komplett abgemalt hat
...
\begin{lem}
	...
	\begin{enumerate}
		...
		\begin{center}
			\begin{tikzcd}
				(Z,\Os_Z) \arrow{r}{f}\arrow{rd}[swap]{\exists!\tilde{f}} & (X,\Os_X)\\
				& (Y,\Os_X|_Y)\arrow{u}{i}
			\end{tikzcd}
		\end{center}
		kommutiert.
...
\end{lem}
\begin{proof}
	Zur Eindeutigkeit: Seien $\Os, \Os'$, sodass \ref{lem:10.11:i} und \ref{lem:10.11:ii} gelten. 
	\begin{center}
		\begin{tikzcd}
			(Y,\Os) \arrow{rd}{i}\arrow{r}{\exists! i_2} & (Y, \Os')\arrow{d}{i}\arrow{l}{\exists!i_1}\\
			& (X,\Os_X)
		\end{tikzcd}
	\end{center}
...
\lecture{12. Juli 2018}
	...
	Zur Existenz: Sei $U\subseteq Y$ offen $\Rightarrow \exists V\subseteq X$ offen: $Y\cap V = U$. Beobachtung: Sei $g\in\mathcal O_X(V)$. Dann ist $g|_{V\cap Y = U} \in \mathcal O_Y(U)$. Problem: Verklebung. Deswegen machen wir das ganze lokal: Definiere
	$$(\mathcal O_X|_Y)(U) \coloneqq \set{g\colon U\to k\given \forall y\in U\exists V'\subseteq X\text{ offen}, g\in V', \exists g'\in \mathcal O_X(V'): g'|_{U\cap V'} = g|_{K\cap V'}}$$
	Dann: Zeige $\mathcal O_X|_Y$ Garben von $k$-Funktionen.
	\begin{itemize}
		\item $(\mathcal O_X|_Y)(U) \subseteq \Abb(U,k)$ Unteralgebra $\checkmark$
		\item Unter-Prägarbe $\checkmark$
		\item Verklebung: Sei $U = \bigcup U_i$ offene Überdeckung, $g\in \Abb(U,k)$ mit $g|_{U_i} \in (\mathcal O_X|_Y)(U_i)$. Behauptung: $g\in (\mathcal O_X|_Y)(U)$. Sei $y\in U$. Dann $\exists i: y|in U_i$. Nach Definition $\exists V_i'\subseteq X$ offen, $y\in V_i', \exists g_i'\in \mathcal O_X(V_i'): g_i'|_{U_i\cap V_i'} = g|_{U_i\cap V_i'}$. Definiere $V' \coloneqq V_i'\cap V\cap V_i$ (wobei $V\subseteq X$ offen, sodass $Y\cap V = U$ und $V_i\subseteq X$ offen, sodass $Y\cap V_i = U_i$).
	...
	Zu \ref{lem:10.11:ii}: Sei $f\colon (Z, \mathcal O_Z)\to (X,\mathcal O_X)$ Morphismus mit $f(Z)\subseteq Y$. Definiere $\tilde f\colon Z\to Y, z\mapsto f(z)$; zeige $\tilde f$ ist Morphismus. Sei $U\subseteq Y$ offen, $g\in (\mathcal O_X|_Y)(U)$.
	
	Behauptung: $g\circ \tilde f|_{\tilde f^{-1}(U)}\in \mathcal O_Z(\tilde f^{-1}(U))$.
	
	Beweis: Da $g\in (\mathcal O_X|_Y)(U)$ ist, gilt: $\forall y\in U\exists V_y\subseteq X$ offen, $y\in V_y$, $\exists g_y\in \mathcal O_X(V_y): g_y|_{V_y\cap U} = g|_{V_y\cap U}$. Wieder: $V\subseteq X$ offen mit $V\cap Y = U$. Ohne Einschränkung: $V_y\subseteq V$. Dann gilt $U\cap V_y = Y\cap V_y$. Da $f\colon (Z,\mathcal O_Z)\to (X,\mathcal O_X)$ Morphismus ist, gilt $g_y\circ f|_{f^{-1}(V_y)} \in \mathcal O_Z(f^{-1}(V_y))$. Weil $f(Z)\subseteq Y\Rightarrow f^{-1}(V_y) = f^{-1}(V_y\cap Y) = \tilde f^{-1}(V_y\cap Y)$.
	
	Also $(g\circ \tilde f|_{\tilde f^{-1}(U)})|_{\tilde f^{-1}(V_y\cap Y)} = g|_{V_y\circ Y}\circ f|_{f^{-1}(V_y)} = g_y\circ f|_f^{-1}(V_y) \in \mathcal O_Z(f^{-1}(V_y)) = \mathcal O_Z(\tilde f^{-1}(V_y\cap Y))$
	
	Da $\tilde f^{-1}(U) = \bigcup\limits_{y\in U} \tilde f^{-1}(V_y\cap Y)$ folgt mit der Verklebung:
	$$g\circ \tilde f|_{\tilde f^{-1}(U)} \in \mathcal O_Z(\tilde f^{-1}(U))$$
\end{proof}

\begin{beme}
	Sei $(X,\mathcal O_X)$ Raum mit $k$-Funktionengarbe. $Y\subseteq X$.
	\begin{enumerate}
		\item Falls $Y\subseteq X$ offen $\Rightarrow  (\mathcal O_X|_Y)(U) = \mathcal O_X(U)$
		
		(denn: für $U\subseteq Y$ offen definiert $U\mapsto \mathcal O_X(U)$ Garbe von $k$-Funktionen auf $Y$ und diese erfüllt universelle Eigenschaft aus Lemma 10.11
		\item Ist $Z\subseteq Y$, so gilt: $(\mathcal O_X|_Y)|_Z = \mathcal O_X|_Z$. 
	\end{enumerate}
\end{beme}

\begin{lem}
	Sei $X\subseteq \IA^n$ irreduzible algebraisch Teilmenge. 
	\begin{enumerate}
		\item Sei $Y\subseteq X$ abgeschlossen irreduzibel. Dann ist $\mathcal O_X|_Y = \mathcal O_Y$.
		\item Sei $h\in A(X)$. Sei $Y = \set {(x,z)\in X\times \IA^1\given h(x)\cdot z = 1}\subseteq X\times \IA^1$ (abgeschlossen, irreduzibel). Dann ist $Y\subseteq \IA^{n+1}$ irreduzible algebraische Teilmenge. Es gilt $A(Y) \cong A(X)[h^{-1}]$. Die Abbildung $f\colon Y\to D(h), (x,z)\mapsto x$ ist ein Homöomorphismus und ein Isomorphismus
		$$f\colon (Y,\mathcal O_Y)\longto (D(h), \mathcal O_X|_{D(h)})$$
	\end{enumerate}
\end{lem}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Sei $h\subseteq X$. Zeige $(\mathcal O_X|_Y)(U) = \mathcal O_Y(U)$. 
		\begin{description}
			\item[\glqq$\subseteq$\grqq:] Sei $i\colon Y\to X$ die Inklusionsabbildung. Da $i$ Morphismus von algebraischen Teilmengen ist, ist nach Proposition 10.10 auch $i\colon (Y,\mathcal O_Y)\to (X,\mathcal O_X)$ ein Morphismus von Räumen mit $k$-Funktionengarben. Andererseits: $i\colon (Y,\mathcal O_X|_Y)\to (X,\mathcal O_X)$ ist auch Morphismus. Nach universeller Eigenschaft (Lemma 10.11 ii):
			\begin{center}
				\begin{tikzcd}
					(Y,\mathcal O_Y) \arrow{r}{i}\arrow{rd}[swap]{\exists!\tilde i} & (X,\mathcal O_X)\\
					& (Y, \mathcal O_Y|_Y)\arrow{u}{i}
				\end{tikzcd}
			\end{center}
			$\Rightarrow \tilde i = \id_Y$ (als Abbildung von Mengen). Also: Sei $U\subseteq Y$ offen, $g\in(\mathcal O_X|_Y)(U) \Rightarrow g = g\circ \tilde i|_U \in \mathcal O_Y(U)$.
			\item[\glqq$\supseteq$\grqq:] Sei $g\in\mathcal O_Y(U)$. Sei $y\in U$. Dann $\exists \overline{h}\in A(Y) = \fak {A(X)}{I(Y)}$ mit $y\in D_Y(\overline{h})\subseteq U$. Da $g|_{D_y(\overline{h})} \in \mathcal O_Y(\D(\overline{h})) = A(Y)[h^{-1}]$.
			
			$\Rightarrow \exists \overline{f}\in A(Y), r\geq 0$: 
			$$\overline{g} = \frac {\overline f}{\overline h^r}$$
			Wähle Urbilder $f,h$ von $\overline f, \overline h$ unter $A(X)\to A(Y)$ (d.h. $\overline{f} = f|_Y,\dots$). Betrachte $g' = \frac f{h^r} \in A(X)[h^{-1}] = \mathcal O_X(\D_X(h))$. Definiere $V'\coloneqq \D_X(h)$. Dann ist $V'\cap Y = D_Y(\overline{h})$ und $g'|_{V'\cap Y} = g|_{V'\cap Y}$. Also $g\in (\mathcal O_X|_Y)(U)$.
		\end{description}
		\item In der Übung bereits bewiesen: \begin{itemize}
			\item $Y$ irreduzibel abgeschlossen in $\IA^{n+1}$
			\item $A(Y)\cong A(X)[h^{-1}]$. 
			\item $f$ ist Homöomorphismus. 
		\end{itemize}
		Noch zu zeigen: \begin{enumerate}
			\item $f$ ist Morphismus von Räumen mit $k$-Funktionengarben
			\item $f^{-1}$ ist Morphismus von Räumen mit $k$-Funktionengarben
		\end{enumerate}
		Beweis:
		\begin{enumerate}
			\item Wir haben das kommutative Diagramm
				\begin{center}
					\begin{tikzcd}
						...
						Y \arrow[Subseteq]{u}{i}[swap]{\text{abgeschlossen}}\arrow{r}{f} & \D(h) \arrow[Subseteq]{u}{\text{offen}}
					\end{tikzcd}
				\end{center}
				Betrachte die Komposition $\overline{f}\colon Y \to X, \overline{f} =  {pr}_X\circ i$. Also ist $\overline{f}$ Morphismus von algebraischen Teilmengen. Nach Proposition 10.10 ist $\overline{f}\colon (Y,\mathcal O_Y)\to (X,\mathcal O_X)$ Morphismus. Es gilt: $\overline{f}(Y) = \D(h)$ und mit Lemma 10.11 kommutiert folgendes Diagramm:
				\begin{center}
					\begin{tikzcd}
						(Y,\mathcal O_Y) \arrow{r}{\overline{f}}\arrow{rd}{\exists!\tilde f} & (X,\mathcal O_X)\\
						& (\D(h), \mathcal O_X|_{\D(h)})
					\end{tikzcd}
				\end{center}
				Als Abbildung: $\tilde f = f$.
				
			\item Es gilt $f^{-1}\colon \D(h)\to Y, x\mapsto (x,\frac 1{h(x)})$. Zeige $f^{-1}$ ist Morphismus von Räumen mit $k$-Funktionengarben. Sei $U\subseteq Y$ offen, $g\in \mathcal O_Y(U)$.
			
			Ohne Einschränkung $U = D_Y(u)$ für ein $u\in A(Y) \cong A(X)[h^{-1}]$; das heißt, es gibt $a\in A(X), r\geq 0$ sodass $u\mapsto \frac a{h^r}$ unter obigem Isomorphismus.
			
			Also $u\colon Y\to k, (x,z)\mapsto \frac{a(x)}{h(x)^r} = a(x)z^r$. Damit gilt nun $D_Y(u) = \set{(x,z)\in Y\given a(x) \neq 0}$ und $(f^{-1})^{-1}(\D_Y(u)) = f(\D_Y(u)) = \set {x\in \D_X(h)\given a(x) \neq 0} = \D_X(ah)$.
			
			Es gilt $g\in \mathcal O_Y(\D_Y(a)) \cong A(X)[h^{-1}]$. Also existiert $g'\in A(X), n\geq 0: g\mapsto \frac{gÄ}{(ah)^n}$. Das heißt $g\colon \D_Y(h)\to k, (x,z)\mapsto \frac{g'(x)}{(a(x)h(x))^n} = \frac{g'(x)z^n}{a(x)^n}$.
			
			$\Rightarrow$ für  $g\circ f^{-1}|_{(f^{-1})^{-1}(\D_Y(a))} \colon \D_X(ah)\to k, x\mapsto \frac{g'(x)}{a(x)^nh(x)^n}$ (wobei $(f^{-1})^{-1}(\D_Y(a)) = \D_X(ah)$) gilt $g\circ f^{-1}|_{(f^{-1})^{-1}(\D_Y(a))} \in \mathcal O_X(\D_X(ah)) = \mathcal O_X((f^{-1})^{-1}(\D_Y(u)))$, also $f^{-1}\colon (\D(h), \mathcal O_X|_{\D(h)}) \to (Y,\mathcal O_Y)$ Morphismus.
		\end{enumerate}
	\end{enumerate}
\end{proof}
...
\begin{defi}
	\leavevmode
	\begin{enumerate}
		\item Sei $(X,\mathcal O_X)$ ein Raum mit $k$-Funktionengarbe. $(X,\mathcal O_X)$ heißt Prävarietät (\glqq pre-variety\grqq), wenn es eine endliche offene Überdeckung $X = U_1\cup\dots\cup U_N$ gibt, sodass $(U_i,\mathcal O_X|_{U_i})$ eine affine Varietät ist (für jedes $i\in\set{1,\dots, N}$).
		\item Seien $(X,\mathcal O_X), (Y, \mathcal O_Y)$ Prävarietäten. Ein Morphismus $f\colon (X,\mathcal O_X)\to (Y,\mathcal O_Y)$ ist ein Morphismus von Räumen mit $k$-Funktionengarben.
	\end{enumerate}
\end{defi}
...
% TODO: soll das ins Literaturverzeichnis? ja
