...
\begin{proof}
	...
	\begin{description}
		...
		Betrachte $M = (b\cdot \delta_{ij}-a_{ij})$. Dann ist $M\cdot (b_1,\dots, b_n)^{\TT} = 0$. Wir multiplizieren mit der adjunkten Matrix zu $M$ und erhalten
		\[\det M\cdot E_n = M^{\ad} M \cdot (b_1,\dots, b_n)^{\TT} = 0.\]
...
\begin{kor}
	...
	\begin{enumerate}
		\item $B$ ist eine endliche $A$-Algebra. \label{lem:endl algebra:i}
		\item Es existieren $b_1,\dots, b_n\in B$, welche ganz über $A$ sind, mit $B = A[b_1,\dots, b_n]$. \label{lem:endl algebra:ii}
		\item $B$ ist endlich erzeugt und ganz über $A$. \label{lem:endl algebra:iii}
	\end{enumerate}
\end{kor}
\begin{proof}
	\leavevmode
	\begin{description}
		\item[\ref{lem:endl algebra:iii} $\Rightarrow$ \ref{lem:endl algebra:ii}:] Klar.
		\item[\ref{lem:endl algebra:ii} $\Rightarrow$ \ref{lem:endl algebra:i}:] Siehe \cref{lem:ganzheit} und \cref{lem:ganzheit2}.
		\item[\ref{lem:endl algebra:i} $\Rightarrow$ \ref{lem:endl algebra:iii}:] Endliche Algebren sind insbesondere endlich erzeugt und außerdem ganz nach \cref{lem:ganzheit}.
		\qedhere
	\end{description}
\end{proof}
...
\subsection{Going up}
\begin{lem} Sei $A\hookrightarrow B$ eine ganze Ringerweiterung (also ein injektiver Ringhomomorphismus) sowie $A$ und $B$ Integritätsbereiche. Dann gilt: 
...
\end{description}	
\end{proof}
Für einen Ringhomomorphismus $f\colon A \to B$ erhalten wir eine Abbildung $f^* \Spec B\to Spec A, q\mapsto q\cap A$.
\begin{satz}
	Sei $A\xto{f} B$ ganz.
	\begin{enumerate}
		\item Sei $q\in \Spec B$. Dann $q\in \Max B \Leftrightarrow q\cap A\in \Max A$
		\item Sei $f$ injektiv. Dann: $\forall p \in\Spec A\exists q \in Spec B$ mit $q\cap A = p$.
		\item Seien $q_1,q_2\in\Spec B$ mit $q_1\subseteq q_2$ und $q_1\cap A = q_2\cap A$, dann folgt bereits $q_1 = q_2$.
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Betrachte $A\to B$. wir erhalten $\fak{A}{q\cap A}\hookrightarrow \fak{B}{q}$ ganz.
		
		\begin{center}
			\begin{tikzcd}
				A \arrow{r}{f\text{ ganz}} \arrow{d}& B \arrow{d}[swap]{\text{ganz}} \\
				\fak{A}{q\cap A} \arrow{r}	& \fak{B}{q}
			\end{tikzcd}
		\end{center}
		Mit Lemma 6.7 folgt die Behauptung.
		\item Sei $p\in\Spec A$. Betrachte $A\hookrightarrow B$. Dann ist $A_p\to B_p$ ganz. Also kommutiert
		...
		\begin{center}
			\begin{tikzcd}
				...
				\end{tikzcd}
			\end{center}
		Wir beweisen noch folgendes Lemma: Sei $A\to B$ ein Ringhomomorphismus, $S\subseteq A$ multiplikative Teilmenge. Falls $A\to B$ gnaz ist, dann ist $S^{-1}A\to S^{-1}B$ ganz.
		
		Beweis: Sei $\frac{b}{s}\in S^{-1}B$. Dann ist $b$ ganz über $A$, es gibt also $a_0,\dots, a_{n-1}$ mit $b^n + a_{n-1}b^{n-1}+\dots + a_0 = 0\Rightarrow 0 = \frac{b^n}{s^n}+\frac{a_{n-1}}{s}\cdot \frac{b^{n-1}}{s^{n-1}}+\dots+\frac {a_0}{s^n}$. Damit folgt das Lemma.
		
		Es gilt $B_p \neq 0$, denn $B_p\cong f(A\setminus p)^{-1}B$ und $f(A\setminus p)\not \ni 0$ (da $f$ injektiv). Da $B_p\neq 0$ ist, gibt es $n\in \Max B_p$. Nach (i) ist $n\cap A_p \in \Max(A_p)$, da wir eine ganze Ringerweiterung haben.
		
		Da $A_p$ lokaler Ring ist, ist $n\cap A_p = p\cdot A_p$. Definiere $q = n\cap B$. Also haben wir 
		$$q\cap A = (n\cap B)\cap A = (n\cap A_p)\cap A = (p\cdot A_p)\cap A = p$$
		wobei die letzte Gleichheit aus Lemma 5.14 folgt.
		\item 	Sei $p\coloneqq q_1\cap A = q_2\cap A$. Betrachte
		\begin{center}
		\begin{tikzcd}
			A \arrow{r}{f\text{ ganz}} \arrow{d}& B \arrow{d} \\
			A_p \arrow{r}{\text{ganz}}	& B_p
		\end{tikzcd}
		\end{center}
		Definiere $q_i'\coloneqq q_i\cdot B_p\in \Spec B_p$ für $i = 1,2$. (denn $q_i\cap f(A\setminus p) = \emptyset$)
	
		Es gilt \begin{eqnarray*}(q_i'\cap A_p)\cap A &=& (q_i'\cap B) \cap A \\
									   &=& ((q_i\cdot B_p)\cap B)\cap A\\
									   &=& q_i \cap A = p
		\end{eqnarray*}
		Also erhalten wir $p\cdot A_p = ((q_i'\cap A_p)\cap A)\cdot A_p = q_i'\cap A_p$ nach Primidealkorrespondenz (Lemma 5.14).
		Dabei ist $p\cdot A_p\in \Max(A_p)$. Mit (i) können wir folglich $q_i'\cdot q_2'\in \Max(B_p)$ folgern. Da $q_1'\subseteq q_2'$ ist, muss damit bereits $q_1' = q_2'$ gelten.
		
		Wieder mit Lemma 5.14 kännen wir schreiben
		$$q_1 = (q_1\cdot B_p)\cap B = q_1'\cap B = q_2'\cap B = q_2$$
	\end{enumerate}
\end{proof}

\begin{satz}[Going up] Sei $A\to B$ ganz. Seien $p_1,p_2\in \Spec A$ mit $p_1\subseteq p_2$ und $q_1\in \Spec B$ mit $q_1\cap A = p_1$. Dann existiert $q_2\in \Spec B$ mit $q_2\cap A = p_2$ und $q_1\subseteq q_2$.

Merkbild: %TODO: Merkbild hier einfügen
\end{satz}
\begin{proof}
	Betrachte \begin{center}
		\begin{tikzcd}
			A \arrow{r}{f\text{ ganz}} \arrow{d}{\tilde{\pi}}& B \arrow{d}{\pi} \\
			\fak{A}{p_1} \arrow{r}{\text{ganz}}	& \fak{B}{q_1}
		\end{tikzcd}
	\end{center}
	Es existiert $q_2'\in\Spec (\fak{B}{q_1})$ mit $q_2'\cap(\fak{A}{p_1}) = \fak{p_2}{p_1}$.
	
	Definiere $q_2\coloneqq \pi^{-1}(q_2') = q_2'\cap B$. Dann gilt $q_1\subseteq q_2$ und
	$$q_2\cap A = \tilde{\pi}^{-1}(q_2'\cap \fak{A}{p_1}) = \tilde{\pi}^{-1}(\fak{p_2}{p_1}) = p_2$$
\end{proof}

\subsection{Going down}
\begin{defi} Sei $A\to B$ Ringhomomorphismus, sei $I\subseteq A$ Ideal. Sei $b\in B$.$b$ heißt ganz über $I$, wenn es ein normiertes Polynom $p(t)$ gibt mit
	$$p(t) = t^n+a_{n-1}t^{n-1}+\dots + a_0\in A[t]$$
sodass $p(b) = 0$ und $a_0,\dots, a_{n-1}\in I$.
\end{defi}
\begin{lem} Sei $A\to B$ Ringhomomorphismus. Sei $C\coloneqq$ ganzer Abschluss von $A$ i $B$. Sei $I\subseteq A$ Ideal, $b\in B$. Äquivalent sind dann:
	\begin{enumerate}
		\item $b$ ganz über $I$
		\item $b\in \sqrt{I\cdot C}$
	\end{enumerate}
\end{lem}
...
\begin{proof}
	\leavevmode
	\begin{description}
		\item[i$\Rightarrow$ii:] Sei $b^n = a_{n-1}b^{n-1}+\dots+a_0$ mit $a_0,\dots, a_{n-1}\in I$. Da $b$ ganz über $I$ ist, ist $b$ insbesondere ganz über $A$. Folglich gilt $b\in C$ und damit auch $b^0,\dots, b^{n-1}\in C$. Also ist $b^n\in I\cdot C$ und $b\in\sqrt{I\cdot C}$.
		\item[ii$\Rightarrow$i:] Sei $b\in\sqrt{I\cdot C}$. Nach Definition gibt es $n>0$ sodass $b^n\in I\cdot C$. Das bedeutet, es existieren $a_1,\dots, a_r\in I$, $c_1,\dots, c_r\in C$ mit $b^n = a_1c_1+\dots+a_rc_r$. Da $c_1,\dots, c_r$ ganz über $A$ sind, ist $A[c_1,\dots, c_r] = C'$ endlich erzeugt als $A$-Modul (Korollar 6.5).
		
		Sei $\<b_1,\dots, b_m\>_A = C'$. Betrachte folgende $A$-lineare Abbildung:
		$$\phi\colon C'\longto C', c'\mapsto b^n\cdot c'$$
		Es gilt $\im\phi\subseteq I\cdot C'$. Also gibt es $a_{ij}\in I$ mit
		$$b^n\cdot b_j = \sum_{i = 1}^{n}a_{ij}b_i\enspace .$$
		Definiere $p(t) = \chi_{(a_{ij})}= \det(t\delta_{ij}-a_{ij})$ normiert und es gilt (vgl. Beweis von Lemma 6.3) $p(b^n) = 0$. Außerdem $p(t) -t^n \in I[t]$. Damit ist $b^n$ aber ganz über $I$, also ist auch schon $b$ ganz über $I$.
	\end{description}
\end{proof}

\begin{defi}
	Sei $A$ Integritätsbereich. $A$ heißt normal genau dann, wenn $A$ ganz abgeschlossen in $\Quot(A)$ ist.
\end{defi}
\begin{lem} 
	Sei $A\subseteq B$ Ringerweiterung, $A$, $B$ Integritätsbereiche, $A$ normal, $I\subseteq A$ Ideal. Sei $b\in B$ ganz über $I$. Sei
	$$m_b(t) = t^n+a_{n-1}t^{n-1}+\dots +a_0\in K[t]$$
	das Minimalpolynom von $b$ über $K\coloneqq \Quot(A)$. Dann gilt $a_0,\dots, a_{n-1}\in\sqrt{I}$.
\end{lem}

\begin{proof}
	Sei $L/K$ eine Erweiterung von $K$, sodass $\exists y_1,\dots, y_n\in L$ mit $m_b(t) = (t-y_1)\dots(t-y_n)$. Sei $f\in A[t]$ normiert mit $f(t) -t^{\deg(f)}\in I[t]$, sodass $f(b) = 0$. Dann ist $f$ Vielfaches von $m_b = m_{y_i}$. Also $f(y_i= 0)$ und damit sind $y_1,\dots, y_n$ ganz über $I$.
	
	Sei $C$ der ganze Abschluss von $A$ in $B$. Nach Lemma 6.11 ist $y_i\in\sqrt{I\cdot C}$.
	$$a_{n-k} = (-1)^k\sum_{1\leq i_1<\dots <i_k\leq n}y_{i_1}\dots y_{i_k}\in \sqrt{I\cdot C} \qquad (k = 1,\dots, n)$$
	Nach Lemma 6.11 sind damit auch $a_0,\dots, a_{n-1}$ ganz über $I$. Insbesondere sind $a_0,\dots, a_{n-1}$ ganz über $A$. Da $A$ normal ist, müssen $a_0,\dots, a_{n-1}\in A$. Wende nun Lemma 6.11 mit $B = K$ an und erhalte $a_0,\dots, a_{n-1}\in \sqrt{I}$.
\end{proof}

\begin{satz}[Going down]
	Sei $A\subseteq B$ ganze Ringerweiterung. $A$ und $B$ seien dabei Integritätsbereiche, $A$ normal.
	
	Seien $p_1,p_2\in \Spec A$ mit $p_1\subseteq p_2$ und sei $q_2\in\Spec B$ mit $q_2\cap A = p_2$. Dann gibt es $q_1\in\Spec B$ mit $q_1\cap A = p_1$ und $q_1\subseteq q_2$.
	
	Merkbild: %TODO: Insert merkbild here
	
\end{satz}
\begin{proof}
	Es genügt zu zeigen: \begin{equation}(p_1\cdot B_{q_2})\cap A = p_1\end{equation}
	
	(Denn: Sei $(p_1\cdot B_q)\cap A = p_1$. Dann folgt aus einer Übung: $\exists q_1' \in \Spec B_{q_2}$ mit $q_1'\cap A = p_1$. Definiere $q_1 \coloneqq q_1'\cap B\in\Spec B\Rightarrow q_1\cap A = (q_1'\cap B)\cap A = p_1$ und $q_1\subseteq q_2$.)
	
	\noindent
	Zeige (1): \begin{description}
		\item[\glqq$\supseteq$\grqq:] $\checkmark$
		\item[\glqq$\subseteq$\grqq:] Sei $x\in (p_1\cdot B_{q_2})\cap A$ und angenommen $x\notin p_1$. Da $x\in p_1\cdot B_{q_2} = (p_1\cdot B)\cdot B_{q_2}$, gibt es $y\in p_1\cdot B$, $s\in B-q_2$ mit $x = \frac ys$. Mit Lemma 6.11 (setze $C = B$) ist $y$ ganz über $p_1$. Sei $K\coloneqq \Quot(A)$. Betrachte das Minimalpolynom $m_y(t) \in K[t]$ von $y$ über $K$. Wir schreiben 
		$$m_y(t) = t^n+a_{n-1}t^{n-1}+\dots+a_0$$
		Dann folgt mit Lemma 6.13, dass $a_0,\dots, a_{n-1}\in\sqrt{p_1} = p_1$.
		
		Da $x = \frac ys\in A\setminus\{0\}$, ist $s = \frac yx$ (in $\Quot(B))$ und $\frac 1x \in K$. Damit gilt
		$$m_s(t) = t^n+\frac{a_{n-1}}xt^{n-1}+\dots + \frac {a_0}{x^n}$$
		ist das Minimalpolynom von $s$ über $K$. Definiere $\tilde{a_i}\coloneqq \frac{1}{x^{n-i}}a_i$. Es gilt $s\in B-q_2\subseteq B\Rightarrow$ $s$ ganz über $A$; also folgt mit Lemma 6.13 (mit $I = A$), dass $\tilde{a_0},\dots, \tilde{a_{n-1}}\in A$.
		
		Nach Annahme ist $x\notin p_1$. Aber $\tilde{a_i}\cdot x^{n-1} = a_i\in p_1$. Da $x^{n-1}$ nicht in $p_1$ ist, muss $\tilde{a_i}\in p_1$ gelten. Daraus folgt nun
		$$s^n = -(\tilde{a_{n.1}}s^{n-1}+\dots+\tilde{a_1})\in p_1B\subseteq p_2\cdot B\subseteq q_2$$
		im Widerspruch zu $s\notin q_2$.
	\end{description}
\end{proof}
