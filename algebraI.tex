...
\begin{proof}
	Eindeutigkeit: Seien $\Os,\Os'$ Garben von $k$-Funktionen mit $\Os(\D(h)) = \Os'(\D(h))$ (alle $h\in A(X)$). Sei $U\subseteq X$ offen $\Rightarrow \exists h_i\in A(X)$ mit $U = \D(h_1)\cup\dots\cup\D(h_N)$. Sei $f\in\Os(U)\Rightarrow f|_{\D(h_i)}\in\Os(\D(h_i)) = \Os'(\D(h_i))$. Mit Verklebung folgt $f\in \Os'(U)$.
	
	Existenz: Sei $U\subseteq X$ offen: Definiere $\Os_X(U) = \set{f\colon U\to k\given \forall x\in U\exists V\subseteq U\text{ offen}, x\in V: \exists g,s\in A(X)\forall y\in V: s(y) \neq 0\text{ und } f(y) = \frac{g(y)}{s(y)}}$. 
	\begin{enumerate}
	\item Zeige $\Os_X$ Garbe von $k$-Funktionen:
	\begin{itemize}
		\item $\Os_X(U)\subseteq \Abb(U, k)$ Unteralgebra $\checkmark$
		\item Unter-Prägarbe: gilt, da definierende Eigenschaft an $f$ lokal ist. 
		\item Verklebung: wieder wegen Lokalität
	\end{itemize}
	\item Zeige $\Os_X(\D(h)) = A(X)[h^{-1}]$
	\begin{description}
		\item[\glqq$\supseteq$\grqq:] $A(X)[h^{-1}] = \set{f\colon \D(h)\to k\given \exists g\in A(X), n\ge 0 \forall x\in\D(h): f(x) = \frac{g(x)}{h(x)^n}}$, also $A(X)[h^{-1}] \subseteq \Os_X(\D(h))$.
		\item[\glqq$\subseteq$\grqq:] Sei $f\in\Os_X(\D(h))$. Betrachte
		$I := \set{a\in A(X)\given \exists g\in A(X): \forall x\in \D(h): g(x) = (af)(x)}$, also $I\subseteq A(X)$ Ideal.
		...
		Also gibt es $n\ge 0$ sodass $h^n \in I\Rightarrow \exists g\in A(X)$ sodass
		$$g|_{\D(h)} = (fh^n)|_{\D(h)}$$
...
\begin{defi}
	Sei $X\subseteq \IA^n$ affine Varietät. Die Garbe $\Os_X$ von $k$-Funktionen heißt Strukturgarbe oder Garbe von regulären Funktionen auf $X$.
\end{defi}

\subsection{Morphismen}
\begin{defi}
	Seien $(X, \Os_X), (Y, \Os_Y)$ Räume mit $k$-Funktionengarben. Ein Morphismus $f\colon (X,\Os_X)\to (Y, \Os_Y)$ ist eine stetige Abbildung $f\colon X\to Y$, sodass gilt:
	$$\forall U\subseteq Y \text{ offen, für alle }g\in\Os_Y(U): g\circ f|_{f^{-1}(u)}\in \Os_X(f^{-1}(u))$$
\end{defi}
\begin{bem}
	Die Abbildung $\Os_Y(U)\to\Os_X(f^{-1}(U)), g\mapsto g\circ f|_{f^{-1}(U)}$ ist Homomorphismus von $k$-Algebren.
\end{bem}
\begin{beme}
	...
	\begin{enumerate}
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center}
		Sei $g\in\Os_Y(\D_Y(h)) = A(X)[h^{-1}]$. Dann gilt:
		$$f^*g \in A(X)[(f^*h)^{-1}] = \Os_X(\D_X(f^*h)) = \Os_X(f^{-1}(\D_Y(h)))$$
		Gleichzeitig gilt $f^*g = g\circ f|_{f^{-1}(\D(h))}$
		\item Sei $U\subseteq Y$ offen, sei $g\in\Os_Y(U)$. Also gibt es $h_i\in A(Y)$ mit $U = \D(h_1)\cup\dots\cup\D(h_N)$. Wir verwenden nun die Abkürzung $U_i = \D(h_i)$. Betrachte $g|_{U_i}\in\Os_Y(U_i)$. Wissen aus (i): $g|_{U_i}\circ f|_{f^{-1}(U_i)}\in \Os_X(f^{-1}(U_i))$.
		
		Aber $(g\circ f|_{f^{-1}(U)})|_{f^{-1}(U_i)} = g|_{U_i}\circ f|_{f^{-1}(U_i)}$. Da $f^{-1}(U) = \bigcup f^{-1}(U_i)$ folgt mit Verklebung $g\circ f|_{f^{-1}(U)}\in\Os_X(f^{-1}(U))$.
	\end{enumerate}
...
\begin{prop}
	...
	$$\Hom(X,Y)\to Hom((X,\Os_X), (Y, \Os_Y)), f\mapsto f$$
	aus Bemerkung 10.9 eine Bijektion.
\end{prop}
\begin{proof}
	Die Injektivität ist klar. Zur Surjektivität: Sei $f\colon (X,\Os_X)\to (Y,\Os_Y)$ Morphismus von Räumen mit $k$-Funktionengarben. Definiere $\phi\colon A(Y)\to A(X)$ durch
	\begin{center}
		\begin{tikzcd}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	Betrachte $F\colon \IA^n\to \IA^m$. Es gilt für alle $x\in X$: $\phi^{\#}(x) = F(x)$. Es gilt $\forall g\in \Os_{\IA^m}(\IA^m)$ und alle $x\in X$: $g(F(x)) = g(f(x))$. (Denn: verfolge den äußeren Weg im Diagramm.) %das tim hoffentlich komplett abgemalt hat
...
\paragraph{Re-Definition} Sei $(X,\Os_X)$ ein Raum mit $k$-Funktionengarbe. $(X,\Os_X)$ heißt affine Varietät, falls $\exists Y\subseteq \IA^n$ irreduzible algebraische Teilmenge, sodass $(X,\Os_X)\cong (Y,\Os_Y)$ (als Räume mit $k$-Funktionengarben).


\subsection{Teilräume}
\begin{lem}
	Sei $(X,\Os_X)$ Raum mit $k$-Funktionengarbe, $Y\subseteq X$ Teilmenge. Versieh $Y$ mit Relativtopologie von $X$. Betrachte die Teilmengeninklusion $i\colon Y\to X$ (stetig). Dann existiert genau eine $k$-Funktionengarbe $\Os_X|_Y$ auf $Y$, sodass:
	\begin{enumerate}
		\item $i\colon (Y,\Os_X|_Y)\to (X,\Os_X)$ ist Morphismus.
		\item $\forall f\colon (Z,\Os_Z)\to (X,\Os_X)$ Morphismus von Räumen mit $k$-Funktionengarben mit $f(Z)\subseteq Y$ kommutiert
		\begin{center}
			\begin{tikzcd}
...
\end{lem}
\begin{proof}
	Zur Eindeutigkeit: Seien $\Os, \Os'$, sodass (i) und (ii) gelten. 
	\begin{center}
		\begin{tikzcd}
		...
		\end{tikzcd}
	\end{center}
	Dann ist $i_1 = i_2 = \id_Y$ (als Abbildungen), also $\Os(U) \subseteq \Os'(U)$ und $\Os'(U)\subseteq \Os(U)$, da $i_1,i_2$ Morphismen.
\end{proof}
