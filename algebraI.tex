...
\begin{defi}
	Sei $(I, \leq)$ angeordnete Menge.
	\begin{enumerate}
		\item Man sagt $(I,\leq)$ erfüllt aufsteigende Kettenbedingung (\glqq ascending chain condition\grqq, acc), wenn für jede Kette
		$$i_1\leq i_2\leq i_3\leq \dots$$
		mit $i_j\in I$ für $j\in \IN$ ein $n\geq 1$ existiert, sodass für alle $m\geq n$ gilt: $i_m = i_n$.
		\item $(I,\leq)$ hat absteigende Kettenbedingung (dcc), wenn für alle Ketten
		$$i_1\geq i_2\geq \dots $$
		ein $n\geq 1$ existiert, sodass für alle $m\geq n$ gilt: $i_m = i_n$ (also acc für $(I,\geq)$).
	\end{enumerate}
\end{defi}
\begin{lem}
	...
	\begin{enumerate}
		\item $(I,\leq)$ hat acc
		\item $\forall \emptyset\neq J\subseteq I$ hat $J$ maximales Element.
	\end{enumerate}
\end{lem}
\begin{proof}
	\leavevmode
	\begin{description}
		\item[i$\Rightarrow$ii:] Angenommen $\exists \emptyset J\subseteq I$ ohne maximales Element. Da $J\neq \emptyset \exists i_1\in J$, $i_1$ nicht maximal $\Rightarrow \exists i_2\in J: i_2>i_1$, usw. (Widerspruch zu acc)
		\item[ii$\Rightarrow$i:] Sei $i_1\leq i_2\leq\dots$ Kette. Die Menge $\{i_n\}_{n\geq 1}$ hat ein maximales Element.
	\end{description}
\end{proof}
\begin{defi}
	Sei $A$ ein Ring, $M$ ein $A$-Modul. Definiere
	$$\mathcal{U}(M) = \{N|N\text{ ist }A\text{-Untermodul von }M\}$$
	$(\mathcal{U}, \subseteq)$ ist angeordnet.
	\begin{enumerate}
		\item $M$ heißt noethersch: $(\mathcal{U}(M),\subseteq)$ hat acc
		\item $M$ heißt artinsch: $(\mathcal{U}(M), \subseteq)$ hat dcc
		\item $A$ heißt noethersch/artinsch: ${}_AA$ ist noethersch/artinsch
	\end{enumerate}
\end{defi}
\begin{bsp}
	\leavevmode
	\begin{enumerate}
		\item $A$ Körper $\Rightarrow A$ noethersch und artinsch
		\item $k$ Körper, $A = k[t] \Rightarrow A$ noethersch, $A$ nicht artinsch.
		\item $A = \IZ\Rightarrow A$ noethersch, nicht artinsch.
		\item $A = k[t_1,t_2,\dots]\Rightarrow A$ weder noethersch noch artinsch
	\end{enumerate}
\end{bsp}
\begin{bem}
	\leavevmode
	\begin{enumerate}
		\item $A$ artinsch impliziert $A$ noethersch (aus Zeitgründen können wir hier keinen Beweis geben)
		\item In der Vorlesung werden wir uns auf noethersche Ringe beschränken. (Mehr zu artinschen Ringen : Atiyah-Macdonald Kapitel 8)
	\end{enumerate}
\end{bem}
...
\begin{satz}
	...
	\begin{enumerate}
		\item $M$ noethersch
		\item $\forall N\subseteq M$ $A$-Untermodul $\Rightarrow N$ endlich erzeugt. (Insbesondere $M$ noethersch $\Rightarrow M$ endlich erzeugt.)
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{description}
		\item[i$\Rightarrow$ii:] Sei $N\subseteq M$ ein $A$-Untermodul. Definiere $$\Sigma \coloneqq \{N'\subseteq N| N'\text{ ist }A\text{-Untermodul, }N'\text{ endlich erzeugt}\}\subseteq \mathcal{U}(M)$$
		Da $\Sigma \neq \emptyset \Rightarrow \exists N'\in \Sigma$ maximal. Sei $x\in N\Rightarrow N'\subseteq N'+\<x\>_a\subseteq N$ und $N'+\<x\>$ endlich erzeugt $\Rightarrow x\in N'$. Da $N'$ maximal gewählt war, ist $N = N'$ und $N$ ist endlich erzeugt.
		\item[ii$\Rightarrow$i:] Sei $N_1\subseteq N_2\subseteq \dots$ Kette in $\mathcal{U}(M)$. Definiere
		$$N = \bigcup\limits_{i = 1}^{\infty}N_i$$
		$N$ ist Untermodul. $\Rightarrow N = \<x_1,\dots, x_n\>$. $\Rightarrow \forall i\exists m_i$ mit $x_i \in N_{m_i}$. Wähle $m\geq \max\{m_1,\dots, m_n\}$. Dann folgt
...
\begin{satz}
	Sei $A$ Ring.
	\begin{enumerate}
		\item Sei $0\to M'\xto{i} M\xto{\pi} M''\to 0$ kurze exakte Sequenz von $A$-Moduln. Dann gilt:
		$$M\text{ noethersch}\Longleftrightarrow M'\text{ und }M''\text{ noethersch}$$
		\item Sei $A$ noethersch, $M$ endlich erzeugter $A$-Modul $\Rightarrow M$ noethersch.
	\end{enumerate}
...
\begin{proof}
	...
	\begin{enumerate}
		\item \begin{description}
			\item[\glqq$\Rightarrow$\grqq:] Sei $M_1''\subseteq M_2''\subseteq \dots$ eine Kette von Untermoduln von $M''$. Dann ist $\pi^{-1}(M_1'')\subseteq \pi^{-1}(M_2'')\subseteq \dots$ Kette in $M$. Also existiert $n$, sodass für alle $m\geq n$ gilt: $\pi^{-1}(M_m'') = \pi^{-1}(M_n'')$. Da $\ker\pi\subseteq M_n''\subseteq M_m''$ ist, folgt $M_m'' = M_n''$. Für $M'$ ähnlich.
			\item[\glqq$\Leftarrow$\grqq:] Sei $M_1\subseteq M_2\subseteq\dots$ Kette in $M$. Betrachte die Ketten
			$$i^{-1}(M_1)\subseteq i^{-1}M_2\subseteq\dots\qquad \text{in }M'$$
			und
			...
			Damit gilt bereits $M_m = M_n$, denn: Sei $x\in M_m$. Dann ist $\pi(x) \in \pi(M_m) = \pi(M_n)\Rightarrow \exists x'\in M_n$ mit $\pi(x) = \pi(x') \Rightarrow x-x'\in\ker \pi = \im  i \Rightarrow \exists y \in M'$ mit $i(y) = x-x'\in M_m\Rightarrow y\in i^{-1}(M_m) = i^{-1}(M_n)$, also $x = i(y) +x' \in M_n$.
		\end{description}
		\item $M$ endlich erzeugter $A$-Modul $\Rightarrow \exists \phi\colon A^n\to M$ $A$-linear und surjektiv. $A^n$ ist noethersch (verwende kurze exakte Sequenz $0\to A^{n-1}\to A^n\to A \to 0$, nutze Induktion). Nach (i) ist $M$ damit auch noethersch.
	\end{enumerate}
\end{proof}
...
\begin{satz}
	Sei $A$ Ring.
	\begin{enumerate}
		\item Falls $A$ noethersch, $I\subseteq A$ Ideal $\Rightarrow \fak{A}{I}$ ist noethersch.
		\item Falls $A$ noethersch, $S\subseteq A$ multiplikative Teilmenge $\Rightarrow S^{-1}A$ noethersch.
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Folgt sofort aus Satz 7.6 (i)
		\item $J\subseteq S^{-1}A \Rightarrow (J\cap A) \cdot S^{-1}A = J$. 
	\end{enumerate}
\end{proof}
\begin{satz}[Hilberts Basissatz]
	Sei $A$ noetherscher Ring. Dann ist $A[t_1,\dots, t_n]$ noethersch.
\end{satz}
\begin{proof}
	Ohne Einschränkung $n = 1$. Zeige, dass jedes Ideal $J\subseteq A[t]$ endlich erzeugt ist.
	
	Angenommen $J\subseteq A[t]$ Ideal ist nicht endlich erzeugt. Wähle $f_1\in J\setminus\set{0}$ mit $d_1 \coloneqq \deg f_1$ minimal. Dabei gilt $(f_1) \subsetneq J$. Wähle $f_2\in J\setminus (f_1)$ von minimalem Grad $d_2$\dots
	
	Erhalte Folge $f_1,f_2,f_3,\dots \in J$ mit $f_n\notin (f_1,\dots, f_{n-1})$ und $d_n = \deg f_n$ ist minimal. Insbesondere gilt
	$$d_1\leq d_2\leq d_3 \leq \dots$$
	Sei $a_n\in A$ mit $f_n(t) = a_nt^{d_n}+$ Terme kleineren Grades.
	Definiere $I \coloneqq (a_1,\dots, a_2)$. Da $A$ noethersch ist, ist $I$ endlich erzeugt. Folglich existiert ein $n$, sodass $I = (a_1,\dots, a_n)$.
	
	Betrachte nun $a_{n+1} \in (a_1,\dots, a_n)$. Also gibt es $u_i$, sodass $a_{n+1} = \sum_{i = 1}^n u_ia_i$. Definiere nun
	$$ g\coloneqq \sum_{i = 1}^n u_if_it^{d_{n+1}-d_i} \in J$$
...
\begin{defi} Sei $A$ Ring. $\fp\in \Spec A$ heißt minimales Primideal, wenn gilt: $\forall \fp'\in Spec A$ mit $\fp'\subseteq \fp \Rightarrow \fp' = \fp$. Wir bezeichnen die Menge aller minimalen Primideale mit $\operatorname{MinSpec}A\coloneqq \set{\fp\in\Spec A\given \fp\text{ minimal}}$.
\end{defi}
\begin{bsp}
	\leavevmode
	\begin{enumerate}
		\item Falls $A$ Integritätsbereich $\Rightarrow \operatorname{MinSpec} (A) = \set 0$.
		\item Sei $k$ Körper. Betrachte $A = \fak{k[x,y]}{(xy)}$. Dann $\operatorname{MinSpec}(a) = \set {(x), (y)}$.
		
		Denn: $$\Spec A \xlongrightarrow{1:1}\set {\fp\in\Spec k[x,y]\given p\supseteq (xy)}$$
		Für $\fp\in \Spec k[x,y]$ mit $\fp\supseteq (xy)$ folgt aus  $xy\in \fp$ aber $ x\in \fp$ oder $y \in\fp$, also $(x)subseteq \fp$ oder $(y)\subseteq \fp$. Falls $\fp$ minimal ist, dann $(x) = \fp$ oder $(y) = \fp$.
	\end{enumerate}
\end{bsp}
\begin{satz}
	Sei $A$ neoterscher Ring. Dann ist $\#\operatorname{MinSpec}(A)<\infty$.
\end{satz}
\begin{proof}
	Sei $\Sigma = \set{I\given I\subsetneq A\text{ Ideal}, \#\operatorname{MinSpec}(\fak{A}{I}) = \infty}$. Zeige $\Sigma = \emptyset$. Angenommen, $\Sigma \neq \emptyset$. Da $A$ noethersch ist, existiert $I\in \Sigma, I$ maximal in $\Sigma$. Also $I\notin \Spec A$ (sonst $\operatorname{MinSpec}(\fak{A}I) = \set {(0)}$). 
	
	Also existieren $a,b\in A\setminus I$ mit $ab\in I$. Folglich gilt $I+(a), I+(b)\supsetneq I$, also $I+(a), I+(b)\notin \Sigma$. Damit folgt, dass $\operatorname{MinSpec}(\fak A{I+(a)}), \operatorname{MinSpec}(\fak A{I+(b)})$ endlich sind.
	
	Sei $\fq\in\operatorname{MinSpec}(\fak AI)\Rightarrow \pi^{-1}(\fq)\in \Spec A$ mit $I\subseteq \pi^{-1}(\fq)$, also $ab\in \pi^{-1}(\fq) \eqqcolon \fp$. Also gilt $a\in \fp$ oder $b\in \fp$; sei ohne Einschränkung $a\in \fp\Rightarrow \fp\supseteq I+(a)\Rightarrow \pi_a(\fp) \in\Spec(\fak{A}{I+(a)})$, sogar $\pi_{a}(\fp) \in \operatorname{MinSpec}(\fak A{I+(a)})$ (wobei $\pi_a\colon A\to \fak A{I+(a)}$ ist).
	
	Also haben wir
	INSERT \glqq DIAGRAMM\grqq\ HIER
	
	Das ist ein Widerspruch.
\end{proof}

\subsection{Transzendenzbasen}
\begin{defi} Sei $K\subseteq L$ eine Körpererweiterung. Eine Familie $(x_i)_{i\in I}$. in $L$ heißt Transzendenzbasis von $L$ über $K$, falls
\begin{enumerate}
	\item $(x_i)_{i\in I}$ ist algebraisch unabhänig über $K$
	\item $K(x_i)_{i\in I}\subseteq L$ ist algebraisch.
\end{enumerate}
\end{defi}
\begin{satz}
	Sei $K\subseteq L$ Körpererweiterung, seien $x_i\in L$ ($i\in I$).
	\begin{enumerate}
		\item Ist $(x_i)_{i\in I}$ algebraisch unabhängig und maximal $\Rightarrow (x_i)_{i\in I}$ ist Transzendenzbasis
		\item Ist $K(x_i)_{i\in I}\subseteq L$ algebraisch und $(x_i)_{i\in I}$ minimal mit dieser Eigenschaft $\Rightarrow (x_i)_{i\in I}$ ist Transzendenbasis. 
		\item $K\subseteq L$ hat eine Transzendenbasis.
	\end{enumerate}
\end{satz}
\begin{proof} Ähnlich wie Basen von Vektorräumen über Körpern.
\end{proof}

\begin{satz}
	Seien $(x_i)_{i\in I}$, $(y_j)_{j\in J}$ Transzendenbasen von $K\subseteq L$. Dann existiert eine Bijektion $I\to J$.
\end{satz}
\begin{proof}
	Wir beweisen nur den Fall $\#I, \#J <\infty$. Es genügt zu zeigen: Seien $(x_1,\dots, x_m)$ algebraisch unabhängig über $K$ und $(y_1,\dots, y_n)$ mit $K(y_1,\dots, y_n)\subseteq L$ algebraisch, dann ist $m\leq n$ und $\exists 1\leq j_{m+1}<\dots < j_n\leq n$ mit $K(x_1,\dots, x_m, y_{j_{m+1}},\dots, y_{j_n})\subseteq L$ algebraisch. Beweis per Induktion nach $m$:
	\begin{description}
		\item[$m = 1$:] Wähle $s$ minimal, sodass $\exists 1\leq i_1<\dots <i_s\leq n$ und $f(t_0,t_{i_1}, \dots, t_{i_j})\in K[t_0, t_{i_1},\dots, t_{i_j}]\setminus \set 0$ mit $f(x_1, y_{i_1}, \dots, y_{i_j}) = 0$. Es gilt $s\geq 1$ (da $x_1$ transzendent). Schreibe
		$$f(t_0, t_{i_1},\dots, t_{i_j}) = \sum_{d\geq 0} f_d(t_0, t_{i_2}, \dots, t_{i_j})t_{i_1}^d$$
		Es existiert also ein $d\geq 1$ mit $f_d \neq 0$ (sonst $f = f_0\Rightarrow$ Widerspruch zur Minimalität von $s$) und $f_d(x_1, y_{i_1},\dots, y_{i_j}) \neq 0$ (wieder Minimalität von $s$). Damit ist $y_{i_1}$ algebraisch über $K(x_1, y_{i_2},\dot, y_{i_j})\subseteq K(x_1,y_1,\dots, \hat{y_{i_1}},\dots, y_n)$.
		\item[$m-1\to m$:] Insbesondere $(x_1,\dots, x_{m-1})$ algebraisch unabhängig. Nach Induktionsvoraussetzung ist $m-1\leq n$ und ohne Einschränkung $K(x_1,\dots, x_{m-1}, y_m,\dots, y_n)\subseteq L$ algebraisch. Außerdem $x_m$ transzendent über $K(x_1,\dots, x_{m-1})$. Nach dem Induktionsanfang ist $1\leq n-m+1$ und (nach eventuell umnummerieren) $K(x_1,\dots, x_{m-1})(y_m, y_{m+1}, \dots , y_n)\subseteq L$ algebraisch.
	\end{description}
\end{proof}
\begin{defi}
	Sei $K\subseteq L$ Körpererweiterung. Sei $(x_i)_{i\in I}$ Transzendenzbasis von $L$ über $K$. 
	$$\operatorname{trdeg}(L|K) \coloneqq \# I\in\IZ_{\geq 0}\cup \{\infty\}$$
	heißt Transzendenzgrad von $K\subseteq L$.
\end{defi}

\subsection{Noether-Normalisierung}
\begin{satz}[Noether-Normalisierung]
	Sei $k$ ein Körper, sei $A$ endlich erzeugte $k$-Algebra. Seien $I_1\subseteq \dots \subseteq I_m\subsetneq A$ Ideale. Dann existiert $n\geq 0$ und $a_1,\dots, a_n\in A$ sowie $0\leq h_1\leq\dots \leq d_m\leq n$ sodass 
	\begin{enumerate}
		\item $a_1,\dots, a_n$ algebraisch unabhängig über $k$
		\item $k[a_1,\dots, a_n]\subseteq A$ endlich ($\Leftrightarrow$ ganz)
		\item $I_l\cap k[a_1,\dots, a_n] = (a_1,\dots, a_{h_l})$
	\end{enumerate}
	Zusatz: 
	$$n = \max\limits_{\fp\in\Spec A} \operatorname{trdeg}(Q(A/\fp)|k) = \max\limits_{\fp \in \operatorname{MinSpec}(A)}\operatorname{trdeg}(Q(A/\fp)|k)$$
\end{satz}
...
\begin{proof}
	\begin{description}
		\item[Schritt 1:] Sei $A = k[t_1,\dots, t_n]$ Polynomring. Sei $a\in A\setminus(A^{\times}\cup \{0\})$. Behauptung: es existieren $a_2,\dots, a_n\in A$ mit $k[a, a_2,\dots, a_n]\subseteq A$ endlich.
		...
		$$\Lambda_a \coloneqq \set{\lambda\in\IZ_{\geq 0}^n \given c_{\lambda}\neq 0}$$
		Es gibt also $r_2,\dots, r_n\in\IZ_{\geq 0}$, sodass für alle $\lambda, \mu\in Lambda_a$, $\lambda\neq \mu$ gilt:
		$$\lambda_1+r_2\lambda_2+\dots +r_n\lambda_n\neq \mu_1+r_2\mu s_2+\dots + r_n\mu_n$$
		
		(Denn: Setze 
		$$R\coloneqq \max\limits_{\lambda\in\Lambda_a}\max_{i = 1,\dots, n}\lambda _i$$
		Definiere $r_i\coloneqq (R+1)^{i-1}$. Seien $\lambda, \mu\in \Lambda_a$ mit $\lambda\neq \mu$. Definiere $m\coloneqq \max\set{i\in\set{1,\dots, n}\given \lambda_m \neq \mu_m}$, also insbesondere $m\geq 1$. Ohne Einschränkung sei $\lambda_m<\mu_m$. Betrachte %TODO: overfull hbox fixen
		...
		Definiere $a_i = t_i-t_i^{r_i}$ für $i = 2, \dots, n$. Es gilt $k[a,a_2,\dots, a_n][t_1] = k[t_1,\dots, t_n]$. Genügt zu zeigen, dass $t_1$ ganz über $k[a,a_2,\dots, a_n]$ ist. Sei $\mu\in\Lambda_a$ das eindeutig bestimmte Element, sodass
		$$\mu_1+r_2\mu_2+\dots+r_k\mu_k = \max\limits_{\lambda\in\Lambda_a}(\lambda_1+r_2\lambda_2+\dots+r_n\lambda_n) \eqqcolon s$$
		Es gilt 
		$$0 = a-a = \sum_{\lambda\in\Lambda_a}c_{\lambda}t^{\lambda}-a = \sum c_{\lambda}t_1^{\lambda_1}(a_2+t_!^{r_2})^{\lambda_2}\dots (a_n+t_n^{r_n})^{\lambda_n} -a = b_st_i^s+b_{s-1}t_1^{s-1}+\dots +b_0$$
...
\lecture{18. Juni 2018}
		...
		\item Wähle $\fq\in\Max B$ und $b = 1$. Wende Konstruktion aus (i) an. Dann folgt, dass ein $m\in\Max A, m\supseteq q\cap A$ und $n\in \Max B$, $n\cap A = m, n\supseteq \fq$ existieren. Da $\fq$ maximal ist, ist $\fq = u$. Also gilt $m\supseteq \fq\cap A = n\cap A = m\in\Max A$. 
		
		Außerdem ist $Q(A'/m')\hookrightarrow Q(B'/n')$ eine endliche Körpererweiterung, aber $Q(A'/m') = A/m = A/\fq\cap A$ und $Q(B'/n') = B/n = B/\fq$.
	\end{enumerate}
\end{proof}
\begin{kor}
