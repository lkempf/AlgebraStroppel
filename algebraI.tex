...
\begin{bem}
	...
	\begin{center}
		\begin{tikzcd}[column sep=3cm]
			B\times M \arrow{r}{(b,x) \mapsto (b'bx)}\arrow{d}  & B\times M\arrow{d} \\
...
\begin{proof} NOCH UNSCHÖN FORMATIERT + FEHLENDE DIAGRAMME, WIRD DEMNÄCHST BEARBEITET % TODO: do stuff
	\leavevmode
	\begin{description}
...
\begin{proof}
	...
	\end{align*}\qedhere % TODO: qedhere fixen
\end{proof}
...
\begin{beme} \label{bem:8.26}
	...
	\begin{enumerate}
		...
		\begin{center}
			\begin{tikzcd}[column sep=2cm]
				\A(Y) \arrow[bend right=15]{rrr}{f^*} \arrow{r}{\beta_X} & \Hom(Y,\A^1) \arrow{r}{h \mapsto h \circ f} &\Hom(X,\A^1) \arrow{r}{\beta_X^{-1}} & \A(X).
			\end{tikzcd}
		\end{center}
		...
		Wir zeigen noch, dass $f$ unabhängig von den Wahlen der $f_i$ ist. Seien dazu $f_i'$ weitere Urbilder von $\overline{f_i}$. Dann gilt insbesondere $f_i-f_i'\in \I(X)$. Nun folgt $f_i(x) = f_i'(x)$ für alle $x \in X$, also $F(x) = F'(x) := (f_1'(x),\dots, f_m'(x))$.
...
\begin{lem}
	Seien $X\subseteq \IA^n$ und $Y\subseteq \IA^m$ algebraische Teilmengen sowie $f\colon X \to Y$ ein Morphismus. Sei $x\in X$ und $\fm_x\in\Max \A(X)$ das zugehörige maximale Ideal. Dann ist $\fm_x\cap \A(Y) = \fm_{f(x)}$ \textup(via $f^*\colon \A(Y)\to \A(X)$\textup). 
\end{lem}
\begin{proof}
...
INSERT REST OF LECTURE HERE

\lecture{5. Juli 2018}

\begin{kor}
	Sei $A$ noetherscher Ring (nicht notwendigerweise lokal), $a_1,\dots, a_l\in A$ und $\fp\in\Spec A$ minimales Primoberideal von $(a_1,\dots, a_l)\Rightarrow \dim A_{\fp}\leq l$. 
\end{kor}
\qedhere
...
\end{lem}
\begin{proof}
	$n\leq \dim A$ mit $A\coloneqq k[t_1,\dots, t_n]$ ist klar (siehe vorher). Zeige $\dim A\leq n$. Sei $\fp_0\subsetneq \dots \subsetneq \fp_l$ Primidealkette in $A$- Wende Noethernormalisierung an: es existieren $a_1,\dots, a_l\in A$ algebraisch unabhängig über $k$ und $0\leq h_0\leq \dots \leq h_l\leq n$:
	\begin{itemize}
		\item $C \coloneqq k[a_1,\dots, a_l]\subseteq A$ ganz
		\item $\fp_i\cap C= (a_1,\dots, a_{h_i})$.
	\end{itemize}
	Da $\fp_i\subsetneq\fp_{i+1}$ und $A\subseteq C$ ganz, folgt mit Proposition 6.8 (iii) schon $\fp_i\cap C\subsetneq \fp_{i+1}\cap C$, also $h_i<h_{i+1}\Rightarrow l\leq n$.
\end{proof}
\begin{satz}
	Sei $A$ endlich erzeugte $k$-Algebra, Integritätsbereich. Dann:
	\begin{enumerate}
		\item $\dim A = \operatorname{trdeg}(\Quot(A)/k)$
		\item Sei $\fp_0\subsetneq\dots \subsetneq \fp_l$ maximale Primidealkette $\Rightarrow l = \dim A$.
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Wende Noethernormalisierung an: $\exists a_1,\dots, a_n\in A$ algebraisch unabhängig, $k[a_1,\dots, a_n]\subseteq A$ ganz. Dann ist $n\coloneqq \operatorname{trdeg}(\Quot(A)/k)$ und nach Proposition 9.7 ist $\dim A = \dim k[a_1,\dots, a_n] = n$ (vergleiche Lemma 9.15).
		\item Gegeben Primidealkette $\fp_0\subsetneq \dots \subsetneq \fp_l$ maximal (das heißt: nicht verlängerbar). Wende wieder Noethernormalisierung an: es gibt $a_1,\dots, a_n\in A$ algebraisch unabhängig über $k$ und $0\leq h_0\leq \dots \leq h_l\leq n$:
		\begin{itemize}
			\item $C = k[a_1,\dots, a_n]\subseteq A$ ganz
			\item $\fp_i\cap C = (a_1,\dots, a_{h_i})$ mit $i = 0,\dots, l$.
		\end{itemize}
		Wegen (i) muss $\dim A = n$ sein. (Also $l\leq n$.)
		
		Behauptung: $h_i = i$ für alle $i = 0,\dots, l$.
		
		Beweis: Wissen wegen $C\subseteq A$ ganz und $\fp_i\subsetneq \fp_{i+1}$ bereits $h_i<h_{i+1}$. Nun Induktion nach $i$:
		\begin{description}
			\item[$i = 0$:] Angenommen $h_0\neq 0\Rightarrow (a_1,\dots, a_{h_0}) \neq (0)$; da die Erweiterung ganz ist, folg $\fp_0 \neq (0)\Rightarrow \fp_0\notin \MinSpec(A)$ im Widerspruch zu Kette maximal.
		...
		%			k[a_1,\dots, a_n] \arrow[=]{u}
		%		\end{tikzcd}
		%	\end{center}
			wobei $A$ Integritätsbereich und $k[a_1,\dots, a_n]$ normal ist.
			
			\item[$i\to i+1$:] Angenommen $h_i+1 < h_{i+1}$. Definiere $r\coloneqq h_i, s\coloneqq h_{i+1}$. Going down impliziert nun, dass es ein $\overline{\fp}\in \Spec \fak A{\fp_i}$ gibt mit $(0)\subsetneq \overline{\fp} \subsetneq \fak{\fp_{i+1}}{\fp_i}$. Betrachte Urbild $\fp$ von $\overline{\fp}$ unter $A\to \fak{A}{\fp_i}$. Dann ist $\fp_i\subsetneq \fp\subsetneq \fp_{i+1}$, was aber ein Widerspruch zur Maximalität ist.
			
			
			vgl folgendes Bild:
		%	\begin{center}
		%		\begin{tikzcd}
		%			A \arrow{r} & \fak{A}{\fp_i} && \exists \overline{\fp}\arrow[Mapsto]{d}\arrow[Subsetneq]{r} \fak{\fp_{i+1}}{\fp_i}\\
		%			C\arrow[Subseteq]{u}{\text{ganz}}\arrow{r} & \fak{C}{\fp_i\cap C}\arrow[Subseteq]{u}{\text{ganz}}&&\\
		%			k[a_1,\dots, a_n] \arrow[=]{u} & k[a_{r+1},\dots, a_n]\arrow[=]{u} & (0) \arrow[Subsetneq]{r} & (a_r+1)\arrow[Subsetneq]{r} & (a_{r+1},\dots, a_s)
		%		\end{tikzcd}
		%	\end{center}
			wobei $A$ Integritätsbereich und $k[a_1,\dots, a_n]$ normal ist.
		\end{description}
		
		Zeige nun noch $h_l = n$. Angenommen $h_l< n$ Dann sagt uns Going up, dass ein $\fp\in\Spec A$ exisitert, $\fp_l\subseteq \fp$ und $\fp\cap C = (a_1,\dot, a_n)$, also aufgrund der Ganzheit $\fp_l\subsetneq \fp$. Das ist wieder ein Widerspruch.
...
\begin{kor}
	Sei $A$ endlich erzeugte $k$-Algebra, Integritätsbereich. $m\in\Max A\Rightarrow \dim A_m = \dim A$. 
\end{kor}
\begin{proof}
	Die Ungleichung \glqq$\leq$\grqq\  ist klar. Für \glqq$\geq$\grqq\ wähle eine Primidealkette $\fq_0\subsetneq\dots\subsetneq \fq_d$ in $A_m$, wobei $d = \dim A_m$. Dann ist $\fq_0\cap A\subsetneq \dots \subsetneq \fq_d\cap A$ Primidelakette. Diese ist maximal wegen $\fq_0\subsetneq \dots \subsetneq \fq_d$ maximal. Aus Sazt 9.16 folgt dann $d = \dim A$.
\end{proof}
\begin{kor}
	Sei $A$ endlich erzeugte $k$-Algebra, Integritätsbereich. Sei $x\in A\setminus \set{0}$, $\fp\in \Spec A$ mit $x\in \fp$ minimal $\Rightarrow \dim \fak{A}{\fp} =\dim A-1$. 
\end{kor}
\begin{proof}
	Aus Krulls Hauptidealsatz wissen wir, dass $\dim A_{fp}\leq 1$ sein muss. Da $A$ Integritätsbereich ist, gilt sogar $\dim A_{\fp}  = 1$ (denn wäre $\dim A_{\fp} = 0\Rightarrow \fp\in\MinSpec A = \set{(0)}\Rightarrow x = 0$).
	
	Betrachte $\fak{A}{\fp}$ und definiere $d\coloneqq \dim \fak{A}{\fp}$. Sei 
%	\begin{center}
%		\begin{tikzcd}
%			\fak A{\fp} && (0) \arrow[Subsetneq]{r}\arrow[Mapsto]{d} & \overline{\fp_1}\arrow[Subsetneq]{r}\arrow[Mapsto]{d} & \dots \arrow[Subsetneq]{r} & \overline{\fp_d} \arrow[Mapsto]{d} & \text{Primidealkette}\\
%			A \arrow{u} & (0)\arrow[Subsetneq]{r}& \fp \arrow[Subsetneq]{r} & \fp_1\arrow[Subsetneq]{r} & |dots \arrow[Subsetneq]{r} & \fp_d &\\
%		\end{tikzcd}
%	\end{center}
	$\Rightarrow \dim A \geq \dim \fak A{\fp}+1$. Aber es gilt auch schon Gleichheit, da $\dim A_{\fp} = 1$.
\end{proof}

\begin{beme}
	Anwendung von Korollar 9.18 auf Geometrie: Sei $X\subseteq \IA^n$ affine Varietät und $f\in A(X)\setminus\set{0}$. Betrachte $\Z(f)\subseteq X$. Es gilt
	$$\set{\text{Irreduzible Komponenten von }\Z(f)} \xlongleftrightarrow{1:1} \set{\fp\in\Spec A(X)\given f\in\fp\text{ minimal}}$$
	Nach Korollar 9.18 folgt $\forall C\subseteq \Z(f)$ irreduzible Komponenten: $\dim C = \dim X-1$. Insbesondere $\dim \Z(f) = \dim X-1$. 
\end{beme}


\section{Varietäten}
Sei $k$ algebraisch abgeschlossener Körper.
\subsection{Räume mit $k$-Funktionengarben}
\begin{defi}
	Sei $X$ ein topologischer Rau. Eine Garbe von $k$-Funktionen $\mathcal{O}$ ist Tupel $(\mathcal{O}(U))_{U\subseteq X \text{ offen}}$ von Unteralgebren
	$$\mathcal{O}(U)\subseteq \operatorname{Abb}(U,k)$$
	sodass gilt:
	\begin{enumerate}
		\item (Unter-Prägarbe): Seien $V\subseteq U\subseteq X$ beide offen. Falls $f\in\mathcal{O}(U)\Rightarrow f|_V \in \mathcal{O}(V)$.
		\item (Verklebung, \glqq gluing\grqq): Sei $U\subseteq X$ offen, $\set{U_i}_{i\in I}$ offene Überdeckung von $U$ (das heißt $U_i\subseteq U$ offen und $\bigcup U_i = U$). Falls $f\in\operatorname{Abb}(U,k)$ mit $f|_{U_i}\in\mathcal{O}(U_i) \Rightarrow f \in\mathcal{O}(U)$.
	\end{enumerate}
\end{defi}
\begin{beme}
	Sei $\operatorname{O}$ Garbe von $k$-Funktionen auf $X$.
	\begin{enumerate}
		\item Seien $V\subseteq U\subseteq X$ offen. Dann ist $\mathcal{O}(U)\to \mathcal{O}(V), f\mapsto f|_V$ ein Homomorphismus von $k$-Algebren. (Denn: $\operatorname{Abb}(U,k)\to \operatorname{Abb}(V,k), f\mapsto f|_V$ ist Homomorphismen von $k$-Algebren.)
		\item Sei $U\subseteq X$ offen, $U = \bigcup U_i$ offene Überdeckung. Dann:
		\begin{enumerate}
			\item Falls $f,g\in\mathcal{O}(U)$, sodass $f|_{U_i} = g|_{U_i}\Rightarrow f = g$.
			\item Seien $f_i\in\mathcal{O}(U_i)$, sodass $f_i|_{U_i\cap U_j} = f_j|_{U_i\cap U_j}$ für alle $i,j$. Dann existiert genau ein $f\in\mathcal O(U)$ mit $f|_{U_i} = f$. 
		\end{enumerate}
	\end{enumerate}
\end{beme}

\begin{defi}
	Ein Paar $(X, \mathcal O_X)$ bestehend aus einem topolgischen Raum $X$ und Garbe $\mathcal O_X$ von $k$-Funktionen heißt Raum mit $k$-Funktionengarbe. 
\end{defi}

\begin{bsp}
	Sei $X$ topologischer Raum. Bespiele für $k$-Funktionengarben auf $X$:
	\begin{enumerate}
		\item $\mathcal O(U) = \operatorname{Abb}(U,k)$ liefert Garbe von $k$-Funktionen
		\item falls $k = $($\IR$ oder) $\IC$: $\mathcal O(U) = \set{\text{ stetige Abbildungen }U\to k}$
		\item Falls $k = \IC$, $X$ komplexe Mannigfaltigkeit: $\mathcal O(U) = \set{\text{holomorphe Funktionen }U\to \IC}$
	\end{enumerate}
\end{bsp}
\begin{beme}
	Sei $X\subseteq \IA^n$ affine Varietät. Sei $h\in A(X)$. Betrachte den Isomorphismus $\beta$ von $k$-Algebren:
	\begin{center}
		\begin{tikzcd}
		 A(X)\arrow{d}\arrow{r}{\beta} &\Hom(X,\IA^1) \arrow[Subseteq]{r}& \operatorname{Abb}(X,k)\arrow{d} & f \arrow[Mapsto]{d}\\
		 A(X)[h^{-1}] &&\operatorname{Abb}(D(h), k)& f|_{D(h)}
		\end{tikzcd}
	\end{center}
	Es gilt $h(x) \neq 0$ für alle $x \in\D(h)$, also $h|_{\D(h)} \in \operatorname{Abb}(\D(h), k)^{\times}$. Die Komposition $A(X)\to \operatorname{Abb}(D(h), k)$ faktorisiert folglich über $A(X)[h^{-1}]$ zu $A(X)[h^{-1}]\to \operatorname{Abb}(\D(h), k), \frac{f}{h^n}\mapsto \left[x\mapsto \frac{f(x)}{h(x)^n}\right]$.
	
	Letztere Abbildung ist dabe injektiv: Falls $\frac {f(x)}{h(x)^n}  = 0$ (für alle $x\in\D(h)$), dann bedeutet das, dass $f(x) = 0$ für alle $x\in\D(h)\Rightarrow f(x) h(x) = 0$ für alle $x\in X$. Daraus folgt $fh = 0$, da $\#k  = \infty$. Also $\frac{f}{h^n} = 0$ in $A(X)$. 
\end{beme}

\begin{lem}
	Sei $X\subseteq \IA^n$ afine Varietät. Dann gibt es genau ein $\mathcal O_X$ Garbe von $k$-Funktionen auf $X$, sodass für alle $h\in A(X)$ gilt:
	$$\mathcal O_X(\D(h)) = A(X)[h^{-1}]$$
	(nach Identifikation von $A(X)[h^{-1}]$ mit seinem Bild unter $A(X)[h^{-1}]\hookrightarrow \operatorname{Abb}(\D(h), k)$).
\end{lem}
...
\begin{proof}
	Eindeutigkeit: Seien $\mathcal O,\mathcal O'$ Garben von $k$-Funktionen mit $\mathcal O(\D(h)) = \mathcal O'(\D(h))$ (alle $h\in A(X)$). Sei $U\subseteq X$ offen $\Rightarrow \exists h_i\in A(X)$ mit $U = \D(h_1)\cup\dots\cup\D(h_N)$. Sei $f\in\mathcal O(U)\Rightarrow f|_{\D(h_i)}\in\mathcal O(\D(h_i)) = \mathcal O'(\D(h_i))$. Mit Verklebung folgt $f\in \mathcal O'(U)$.
	
	Existenz: Sei $U\subseteq X$ offen: Definiere $\mathcal O_X(U) = \set{f\colon U\to k\given \forall x\in U\exists V\subseteq U\text{ offen}, x\in V: \exists g,s\in A(X)\forall y\in V: s(y) \neq 0\text{ und } f(y) = \frac{g(y)}{s(y)}}$. 
	\begin{enumerate}
	\item Zeige $\mathcal O_X$ Garbe von $k$-Funktionen:
	\begin{itemize}
		\item $\mathcal O_X(U)\subseteq \Abb(U, k)$ Unteralgebra $\checkmark$
		\item Unter-Prägarbe: gilt, da definierende Eigenschaft an $f$ lokal ist. 
		\item Verklebung: wieder wegen Lokalität
	\end{itemize}
	\item Zeige $\mathcal O_X(\D(h)) = A(X)[h^{-1}]$
	\begin{description}
		\item[\glqq$\supseteq$\grqq:] $A(X)[h^{-1}] = \set{f\colon \D(h)\to k\given \exists g\in A(X), n\geq 0 \forall x\in\D(h): f(x) = \frac{g(x)}{h(x)^n}}$, also $A(X)[h^{-1}] \subseteq \mathcal O_X(\D(h))$.
		\item[\glqq$\subseteq$\grqq:] Sei $f\in\mathcal O_X(\D(h))$. Betrachte
		$I\coloneqq \set{a\in A(X)\given \exists g\in A(X): \forall x\in \D(h): g(x) = (af)(x)}$, also $I\subseteq A(X)$ Ideal.
		
		Behauptung: $h\in\sqrt I$
		
		Beweis: Aus dem Nullstellensatz folgt $\sqrt{I} = I(Z(I))$. Angenommen $h\notin I(Z(I))\Rightarrow \exists x\in Z(I): h(x) \neq 0$, also $x\in\D(h)$. Folglich existiert eine offene Umgebung $x\in V\subseteq \D(h)$ $\exists g, s\in A(X)\forall y\in V: s(y) \neq 0$ und $\forall y\in V$ gilt $f(y) = \frac{g(y)}{s(y)}$, was äquivalent ist zu:
		$$\forall y\in V\qquad s(y)f(y) = g(y)$$
		Ohne Einschränkung ist $V = \D(u)$ für ein $u\in A(X)$. Insbesondere gilt für alle $y\in \D(u)$: $(usf)(y) = (ug)(y)$. Daraus folgt jetzt, dass $(usf)(y) = (ug)(y)$ auch für alle $y\in D(h)$ gilt: Denn für alle $y\in \D(h)\setminus \D(u)$ gilt $u(y) = 0$.
		
		Damit ist nun $us\in I$, aber $us(x) \neq 0 \Rightarrow x\notin Z(I)$. Das ist ein Widerspruch zu unser Konstruktion. Folglich ist der Beweis von obiger Behauptung ($h\in\sqrt I$) vollbracht.
		
		Also gibt es $n\geq 0$ sodass $h^n \in I\Rightarrow \exists g\in A(X)$ sodass
		$$g|_{\D(h)} = (fh^n)|_{\D(h)}$$
...
\begin{defi}
	Sei $X\subseteq \IA^n$ affine Varietät. Die Garbe $\mathcal O_X$ von $k$-Funktionen heißt Strukturgarbe oder Garbe von regulären Funktionen auf $X$.
\end{defi}

\subsection{Morphismen}
\begin{defi}
	Seien $(X, \mathcal O_X), (Y, \mathcal O_Y)$ Räume mit $k$-Funktionengarben. Ein Morphismus $f\colon (X,\mathcal O_X)\to (Y, \mathcal O_Y)$ ist eine stetige Abbildung $f\colon X\to Y$, sodass gilt:
	$$\forall U\subseteq Y \text{ offen, für alle }g\in\mathcal O_Y(U): g\circ f|_{f^{-1}(u)}\in \mathcal O_X(f^{-1}(u))$$
\end{defi}
\begin{bem}
	Die Abbildung $\mathcal O_Y(U)\to\mathcal O_X(f^{-1}(U)), g\mapsto g\circ f|_{f^{-1}(U)}$ ist Homomorphismus von $k$-Algebren.
\end{bem}
\begin{beme}
	Seien $X\subseteq \IA^n, Y\subseteq \IA^m$ affine Varietäten, sei $f\colon X\to Y$ Morphismus von affinen Varietäten. Sei $f^*\colon A(Y)\to A(X)$.
	Frage: Ist $f$ ein Morphismus von Räumen mit $k$-Funktionengarben?
	\begin{enumerate}
		\item Sei $k\in A(Y)$. Betrachte $\D_Y(h)$ (offene Hauptmenge von $h$ in $Y$). Wir erhalten folgendes Diagramm:
		\begin{center}
			\begin{tikzcd}
				A(Y)\arrow{r}{f^*}\arrow{d} & A(X)\arrow{d}\\
				A(Y)[h^{-1}] \arrow{r}{\exists!f^*}& A(X)[(f^*h)^{-1}]
			\end{tikzcd}
		\end{center}
		Sei $g\in\mathcal O_Y(\D_Y(h)) = A(X)[h^{-1}]$. Dann gilt:
		$$f^*g \in A(X)[(f^*h)^{-1}] = \mathcal O_X(\D_X(f^*h)) = \mathcal O_X(f^{-1}(\D_Y(h)))$$
		Gleichzeitig gilt $f^*g = g\circ f|_{f^{-1}(\D(h))}$
		\item Sei $U\subseteq Y$ offen, sei $g\in\mathcal O_Y(U)$. Also gibt es $h_i\in A(Y)$ mit $U = \D(h_1)\cup\dots\cup\D(h_N)$. Wir verwenden nun die Abkürzung $U_i = \D(h_i)$. Betrachte $g|_{U_i}\in\mathcal O_Y(U_i)$. Wissen aus (i): $g|_{U_i}\circ f|_{f^{-1}(U_i)}\in \mathcal O_X(f^{-1}(U_i))$.
		
		Aber $(g\circ f|_{f^{-1}(U)})|_{f^{-1}(U_i)} = g|_{U_i}\circ f|_{f^{-1}(U_i)}$. Da $f^{-1}(U) = \bigcup f^{-1}(U_i)$ folgt mit Verklebung $g\circ f|_{f^{-1}(U)}\in\mathcal O_X(f^{-1}(U))$.
	\end{enumerate}
	Also: Ja, $f$ ist Morphismus von Räumen mit $k$-Funktionengarben.
\end{beme}
\begin{prop}
	Seien $X\subseteq\IA^n, Y\subseteq \IA^m$ affine Varietäten. Dann ist die Zuordnung
	$$\Hom(X,Y)\to Hom((X,\mathcal O_X), (Y, \mathcal O_Y)), f\mapsto f$$
	aus Bemerkung 10.9 eine Bijektion.
\end{prop}
\begin{proof}
	Die Injektivität ist klar. Zur Surjektivität: Sei $f\colon (X,\mathcal O_X)\to (Y,\mathcal O_Y)$ Morphismus von Räumen mit $k$-Funktionengarben. Definiere $\phi\colon A(Y)\to A(X)$ durch
	\begin{center}
		\begin{tikzcd}
			A(Y)\arrow{r}{\phi} \arrow{d}{=} & A(X)\arrow{d}{=}\\
			\mathcal O_Y(Y)\arrow{r}{g\mapsto g\circ f} & \mathcal O_X(X)
		\end{tikzcd}
	\end{center}
	(Nach Definition von Morphismen von Räumen mit $k$-Funktionengarben ist $\phi$ wohldefiniert und $k$-Algebrenhomomorphismus.)
	
	Betrachte $\phi^{\#}\in \Hom(X,Y)$. Zeige $f = \phi^{\#}$. Zur Erinnerung: $\phi^{\#}$ entsteht wie folgt: wähle Lift $\Phi$ wie im Diagramm:
	\begin{center}
		\begin{tikzcd} %TODO: ergänze Diagramm noch (siehe Tims Notizen)
			k[s_1,\dots, s_m]\arrow{r}{\exists \Phi} \arrow{d} & k[t_1,\dots, t_n]\arrow{d}\\
			A(X) \arrow{r}{\phi}& A(X)
		\end{tikzcd}
	\end{center}
	Betrachte $F\colon \IA^n\to \IA^m$. Es gilt für alle $x\in X$: $\phi^{\#}(x) = F(x)$. Es gilt $\forall g\in \mathcal O_{\IA^m}(\IA^m)$ und alle $x\in X$: $g(F(x)) = g(f(x))$. (Denn: verfolge den äußeren Weg im Diagramm.) %das tim hoffentlich komplett abgemalt hat
	
	Daraus folgt nun $F(x) = f(x)$ für alle $x\in X$. Damit folgt die Behauptung, da $\phi^{\#}(x) = F(x)$.
	
\end{proof}

Das rechtfertigt nun folgende
\paragraph{Re-Definition} Sei $(X,\mathcal O_X)$ ein Raum mit $k$-Funktionengarbe. $(X,\mathcal O_X)$ heißt affine Varietät, falls $\exists Y\subseteq \IA^n$ irreduzible algebraische Teilmenge, sodass $(X,\mathcal O_X)\cong (Y,\mathcal O_Y)$ (als Räume mit $k$-Funktionengarben).


\subsection{Teilräume}
\begin{lem}
	Sei $(X,\mathcal O_X)$ Raum mit $k$-Funktionengarbe, $Y\subseteq X$ Teilmenge. Versieh $Y$ mit Relativtopologie von $X$. Betrachte die Teilmengeninklusion $i\colon Y\to X$ (stetig). Dann existiert genau eine $k$-Funktionengarbe $\mathcal O_X|_Y$ auf $Y$, sodass:
	\begin{enumerate}
		\item $i\colon (Y,\mathcal O_X|_Y)\to (X,\mathcal O_X)$ ist Morphismus.
		\item $\forall f\colon (Z,\mathcal O_Z)\to (X,\mathcal O_X)$ Morphismus von Räumen mit $k$-Funktionengarben mit $f(Z)\subseteq Y$ kommutiert
		\begin{center}
			\begin{tikzcd}
				(Z,\mathcal O_Z) \arrow{r}{f}\arrow{rd}[swap]{\exists!\tilde{f}} & (X,\mathcal O_X)\\
				& (Y,\mathcal O_X|_Y)\arrow{u}{i}
			\end{tikzcd}
		\end{center}
	\end{enumerate}
\end{lem}
\begin{proof}
	Zur Eindeutigkeit: Seien $\mathcal O, \mathcal O'$, sodass (i) und (ii) gelten. 
	\begin{center}
		\begin{tikzcd}
			(Y,\mathcal O) \arrow{rd}{i}\arrow{r}{\exists! i_2} & (Y, \mathcal O')\arrow{d}{i}\arrow{l}{\exists!i_1}\\
			& (X,\mathcal O_X)
		\end{tikzcd}
	\end{center}
	Dann ist $i_1 = i_2 = \id_Y$ (als Abbildungen), also $\mathcal O(U) \subseteq \mathcal O'(U)$ und $\mathcal O'(U)\subseteq \mathcal O(U)$, da $i_1,i_2$ Morphismen.
\end{proof}

\end{document}


%Literatur für Alggeo: Görtz-Wedhorn: Algebraic Geometry I
