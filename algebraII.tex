...
\begin{proof}
  ...
  \begin{enumerate}[label=\ref{prop:VIII.1:\arabic*}]
    ...
    For $h\in k[X_1]$ define $\tilde h\in \Maps(X_1 \dotcup X_2 ,k)$ by $\tilde h |_{X_2} = h$ % TODO ?!?!
    \item 
    \item Left to the reader.
    \qedhere
  \end{enumerate}
...
\lecture{December 20, 2018}

TO BE INSERTED

\lecture{January 11, 2019}
...
Assume $G \operates^\alpha X$ for $g \in G$ consider \blockeq{\beta_g}XXx{g^{-1}.x} (a morphism since $\beta_g = x \mapsto (g,x) \mapsto (g^{-1},x) \mapsto g^{-1}.x$). Hence we get a comorphism \blockeq {\beta_g^*}{k[X]}{k[X]}f{f\circ \beta_g.} Note $\beta_g^*(f)(x) = f(g^{-1}x)$ for all $x \in X$. Moreover $\beta_{gh}^* = \beta_g^* \circ \beta_h^*(f)$ for all $g,h \in G$.

If $\aav X = \aav G$ is an affine algebraic group, then we can also consider \blockeq {\gamma_g} GGx{xg} and get a comorphism \blockeq{\gamma_g^*}{k[G]}{k[G]} f {f \circ \gamma_g.} Note $\gamma_g^*(f)(x) = f(xg)$ for all $g,x\in G$. Moreover $\gamma_{gh}^* = \gamma_g^* \circ \gamma_h^*$.
\begin{deff}
...
\begin{proof}
  ...
  \item[\enquote{$\subseteq$}] Let $\rho_g(I) \subseteq I$. Then for all $f\in I$ we have $0 = \rho_g(f)(e) = f(eg) = f(g)$, so $f(g) = 0$ for all $f\in I$. This implies $g \in H$. % TODO insert spam?
  \end{description}
\end{proof}
...
\lecture{January 18, 2019}

TO BE INSERTED

\lecture{January 21, 2019}
