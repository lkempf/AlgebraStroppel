...
\paragraph{Remark.} Graded $A$-modules with graded $A$-module homomorphisms form a category (where $A$ is graded algebra).
...
\begin{deff}
  ...
  \begin{align*}
    p_r^{(n)} &:= X_1^r+X_2^r+\ldots+X_n^r \\ \intertext{the \emph{$r-th$ power symmetric polynomial} (with $p_0^{(n)} = n$),}
    h_r^{(n)} &:= \sum_{\abs a = r}X_1^{a_1}X_2^{a_2}\cdots X_n^{a_n} \\ \intertext{where $a = (a_i)_{1\le i\le n}\in \IZ_{\ge 0}^n$ with $\abs a = \sum_{i=1}^na_i$, the \emph{$r$-th complete symmetric polynomial} ($h_0^{(n)} = 1$),}
    e_r^{(n)} &:= \sum_{\mathclap{1\le i_1<\ldots<i_r\le n}} X_{i_1} X_{i_2}\cdots X_{i_r} = \sum_{\substack{I \subseteq \set{1,\ldots,n}\\\abs I = r}} \prod_{i \in I}X_i, \\ \intertext{the \emph{$r$-th elementary symmetric polynomial} (with $e_0^{(n)} = 1$ and $e_r^{(n)} = 0$ if $r>n$).}
  \end{align*}
\end{deff}
...
\end{lem}
\begin{proof}
  The coefficients of $t^{n-j}$ on the left hand side equals \begin{align*} \sum_{\mathclap{i \le i_1 < \ldots < i_j \le n}} (-X_{i_1})(-X_{i_2})\cdots(-X_{i_j}) = (-1)^j e_j^{(n)}. \qedhereb \end{align*}
\end{proof}
...
\begin{lem} \label{lem:III.5/6}
  Let $G$ be a group and $V_i$ ($i\in I$) representations of $G$ (over some fixed field $k$). Then \[ \left(\bigoplus_{i\in I}V_i\right)^G = \bigoplus_{i \in I}V_i^G \] as vector subspaces of $\bigoplus_{i\in I} V_i$. 
\end{lem}
\begin{proof}
  \leavevmode
  \begin{description}
    \item[\enquote{$\supseteq$}] Obvious.
    \item[\enquote{$\subseteq$}] Let $v = \sum_{i\in I}v_i \in \left(\bigoplus_{i\in I}V_i\right)^G$. Then we have \[ v = g.v = g. \left(\sum_{i\in I}v_i\right) = \sum_{i \in I}g.v_i \] for all $g \in G$ since the sum is direct. We get $v_i = g.v_i$ for all $i\in I$ and $g \in G$, and therefore $v_i \in V_i^G$ for all $i\in I$.
    \qedhere
  \end{description}
\end{proof}
...
The following formula holds for all $1 \le r \le n$ ($n \in \IZ_{>0}$). \[ e_r^{(n)} = e_r^{(n-1)} + X_ne_r{-1}^{(n-1)} \]
\begin{proof}
  \begin{align*}
    \qedherea
...
\begin{proof}
  ...
  \begin{description}
    ...
    For $n=1$ we have $k[X_1]^{S_1} = k[X_1]^{\set e} = k[X_1] = k[e_1^{(1)}]$.
    ...
    \begin{itemize}
      ...
      \end{align*}
    \end{itemize}
    By induction $q(f)$ is a polynomial $P(e^{(n-1)}_1,\ldots,e_{n-1}^{(n-1)})$ in $e^{(n-1)}_1,\ldots,e_{n-1}^{(n-1)}$. Set $g= P(e_1^{(n)},\ldots,e_{n-1}^{(n)})\in k[X_1,\ldots,X_n]$. Because $q$ is an algebra homomorphism we have \[q(g) = P(q(e_1^{(n)}),\ldots,q(e_n^{(n)})) = P(e_n^{(n-1)},\ldots,e_{n-1}^{(n-1)},0) = q(f). \] Therefore, $q(f-g) = 0$ in $\fak{k[X_1,\ldots,X_n]}{(X_n)}$, and we get $X_n \mid f-g$.
...
\begin{proof}[Proof ot the \namereff{thm:symmetric polys}]
  We still have to show that the $e_1^{(n)},\ldots,e_n^{(n)}$ are algebraicaly independent (over $k$). We use induction on $n$. For $n=1$ we have $k[X_1]^{S_1} = k[X_1] = k[e_1^{(1)}]$.
  
  Assume the claim holds for $n-1 \ge 1$, but it does not hold for $n$. Then there exists a polynomial $0 \neq P \in k[t_1,\ldots,t_n]$ such that $P(e_1^{(n)},\ldots,e_n^{(n)})=0$. Let $p$ be of minimal possible degree. Then \[ 0 = q(P(e_1^{(n)},\ldots,e_n^{(n)})) = P(q(e_1^{(n)}),\ldots,q(e_n^{(n)}) = p(e_1^{(n-1)},\ldots,e_{n-1}^{(n-1)},0) \] and by induction hypothesis $X_n \mid P$.
  
  Therefore, there exists a $\hat p \in k[t_1,\ldots,t_n]$ such that $P = t_n \hat P$. In particular $\hat P \neq 0$ and $\deg \hat P < \deg P$. We have $0 = P(e_1^{(n)},\ldots,e_n^{(n)}) = e_n^{(n)}\hat P (e_1^{(n)},\ldots,e_n^{(n)})$. Thus, $\hat P(e_1^{(n)},\ldots,e_n^{(n)}) = 0$ since $e_n^{(n)} \neq 0$ and $P \mid 0$. This contradicts the minimality of $\deg P$.
\end{proof}

\paragraph{Remark.} The proof givs also an algorithm how to express a symmetric polynomial $f$ in the $e_1^{(n)},\ldots,e_n^{(n)}$.
...
\end{cor}
\begin{proof}
  Let $H^r(t)$ be the formal derivation of $H(t)$ with respect to $t$, so $H_r'(t) = \sum_{r\ge 0}rh_r^{(n)}t^{r-1}$. On the other hand (by \cref{lem:III.9}) \[ H'(t) = \sum_{i=1}^n\left(\frac{X_i}{(1-X_it)^2}\prod_{j\neq i}\frac1{1-X_jt}\right) = \sum_{i=1}^n\frac{X_i}{1-X_it}\left(\prod_{j=1}^n\frac1{1-X_jt}\right). \] By comparing coefficients of $t^{r-1}$ we get \[ rh_r^{n} = \sum_{s=1}^rp_s^{(n)}h_{r-s}^{(n)} \] using \cref{lem:III.9} \ref{lem:III.9:2} and \ref{lem:III.9:3}.
\end{proof}
