...
\begin{proof}
	...
	\end{description}
\end{proof}
\begin{satz}[Satz vom primitiven Element]\label{thm:19.14}
...
\begin{satz}[Galoiserweiterungen]\label{thm:21.1}
	...
	\begin{enumerate}
		...
		\item $K = L^{\Gal(L\sslash K)}\eqqcolon\{x\in L\mid\phi(x) = x~\forall \phi\in \Gal(L\sslash K)\}$
...
\begin{bsp}[Berechnung der Galoisgruppe]
	Sei $L$ der Zerfällungskörper von $f = t^3-2\in \IQ[t]$. Sei $K\coloneqq \IQ$. $f$ hat 3 Nullstellen in $L$, nämlich $\sqrt[3]2, \sqrt[3]2\zeta, \sqrt[3]2\zeta^2$, wobei $\zeta = e^{\frac{2\pi i}3}$. Beachte: $\IQ[\sqrt[3]2]\subseteq \IR$. Nach \cref{thm:grupOpTreuTransGaloisGrup} ist $\Gal(L\sslash K)\subseteq S_3$. Nun betrachte den surjektiven Ringhomomorphismus 
	\begin{eqnarray*}
		\ev = \ev_{\sqrt[3]2} \colon \IQ[t] &\longto & \IQ[\sqrt[3]2]\\
		t &\longmapsto &\sqrt[3]2\\
		g(t) &\longmapsto &g(\sqrt[3]2)
	\end{eqnarray*}
	Er induziert einen Ringhomomorphismus $\overline{\ev}\colon \IQ[t]/(t^3-2)\to \IQ(\sqrt[3]2)$ nach Homomorphiesazt immernoch surjektiv. Da $t^3-2$ irreduzibel ist, ist $\IQ[t]/(t^3-2)$ Körper. Damit ist $\overline{\ev}$ injektiv, also bijektiv, also Körperisomorphismus. Folglich ist $[\IQ(\sqrt[3]2):\IQ] = \deg m_{\sqrt[3]2}(t) = 3$. Aber $L$ enthält echte komplexe Zahlen, also $[L:\IQ(\sqrt[3]2)]>1$. Das heißt, dass $[L:\IQ] = [L:\IQ(\sqrt[3]2)][\IQ(\sqrt[3]2):\IQ]\geq 2\cdot 3 = 6$. Dabei ist $L$ Zerfällungskörper von $f$ über $K$, also normal und separabel, da $\chr\IQ= 0$ (und $L\sslash K$ algebraisch). Insgesamt ist $L\sslash K$ Galoissch, und mit \cref{thm:21.1} folgt $6\leq [L:K] = |\Gal(L\sslash K)| \leq 6\Rightarrow \Gal(L\sslash K) \cong S_3$.
...
\begin{satz}[Galoiskorrespondenz]\label{thm:22.1}
	...
	\begin{eqnarray*}
		...
		H &\longmapsto & L^H \coloneqq \{x\in L\mid\phi(x) = x~\forall \phi\in H\}\\
		\{g\in G\mid g(m) = m~\forall m\in M\}\eqqcolon H_M &\longmapsfrom& M
	\end{eqnarray*}
...
\begin{satz}\label{thm:22.2}
	Sei $L$ ein Körper. $G\subseteq \{\phi\colon L\to L\mid\phi\text{ bijektiver Körperhomomorphismus}\}$. $G$ sei eine endliche Gruppe. Dann: $L\sslash L^G$ ist galoissch und $\Gal(L\sslash L^G) = G$. 
\end{satz}
\begin{proof}
...
\begin{description}
	...
	\begin{proof}
		$z_i, z_j\in Z\Rightarrow (z_iz_j)^n-1 = z_i^nz_j^n-1 = 1\cdot 1-1 = 0\Rightarrow z_iz_j\in Z$ und $(z_i^{-1})^n -1 = (z_i^n)^{-1}-1 = 1-1 = 0$; folglich ist $z_i^{-1}\in Z$ (klar: $e  = 1\in Z$). Damit ist $Z$ eine Untergruppe von $\overline{K}^{\times}$. Da $Z<\overline{K}^{\times}$ endlich ist, ist nach \ref{thm:endlKoeZyklMult} $Z$ zyklisch. 
	\end{proof}
\end{description}
Folglich $Z \cong \IZ/n\IZ$ als Gruppe, weil $|Z| = n$ nach der 1. Behauptung. Damit hat $Z$ genau $\phi(n)$ verschiedene Erzeuger, da $\phi(n) = |\{\overline{m}\in \IZ/n\IZ|ggT (m,n) = 1\}|$. %TODO: ggT als mathoperator hinzufügen
...
\begin{proof}
	...
	\begin{eqnarray*}
		\phi\colon G & \longto & \Aut(Z) =\{\phi\colon Z\to Z \mid \phi\text{ bijektiver Gruppenhomomorphismus}\}\\
		g & \longmapsto & g|_Z
	\end{eqnarray*}
