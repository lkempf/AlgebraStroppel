...
Dies ist eine Mitschrift der Vorlesung \glqq Einführung in die Algebra\grqq von Prof. Dr. Catharina Stroppel an der Universität Bonn, gehalten im Wintersemester 2017/18.

%TODO: iwelche credits/special thanks?

\lecture{9. Oktober 2017}

\paragraph {Organisatorisches}
\begin{itemize}
	\item Website der Vorlesung: \url{http://www.math.uni-bonn.de/people/palmer/A1.html}
	\item Assistent: Dr. Martin Palmer
	\item Abgabe der Übungsblätter: Donnerstag vor der Vorlesung
	\item Beginn der Übungsgruppen: Zweite Vorlesungswoche
...
\begin{itemize}
	...
	\item Es reicht sogar, die Existenz von Linksneutralem und Linksinversem oder Existenz von Rechtsneutralem und Rechtsinversem zu fordern.
...
\begin{defi}
	$(G,\circ)$ heißt abelsch, falls $g\circ h = h\circ g$ für alle $g,h\in G$ gilt.
\end{defi}
...
\begin{itemize}
	\item $(\mathbb Z, +)$
	\item Für einen Körper $(K,+,\cdot)$ sind $(K,+)$ und $(K^*=K\setminus \{0\}, \cdot)$ Gruppen.
	\item Ist $(V,+,\cdot)$ ein $K$-Vektorraum, so ist $(V,+)$ eine Gruppe.
	\item Für einen Körper $K$ und $n\in\mathbb N$ ist $\GL_n(K)$ eine Gruppe mit der Matrixmultiplikation als Verknüpfung.
	\item Sei $M$ eine nichtleere Menge sowie $S_M \coloneqq \{f\colon M\to M \mid \text{$f$ invertierbar}\}$. Dann ist $(S_M, \circ)$ eine Gruppe, wobei $\circ$ die Komposition von Abbildungen ist. Der Spezialfall $M = \{1,\dots n\},\ n\in\mathbb N$ ergibt die symmetrische Gruppe $S_n$ der Ordnung $n!$.
	\item Sei $(G,\circ)$ eine Gruppe und $a\in G$ fest gewählt. Dann ist $(G,\circ_a)$ eine Gruppe, wobei $g\circ_a h = g\circ a\circ h$.
\end{itemize}
\end{bsp}

\begin{defi} 
	Sei $(G,\circ)$ eine Gruppe. Dann ist die Anzahl $\abs G$ der Elemente von $G$ die Ordnung von $G$.
\end{defi}

\begin{defi}
	 Sei $(G,\circ)$ eine Gruppe. Eine Teilmenge $H\subseteq G$ heißt Untergruppe, falls $H\neq\emptyset$ und aus $h_1,h_2\in H$ bereits $h_1\circ h_2^{-1}\in H$ folgt. Wir schreiben dann $H<(G,\circ)$ oder $H<G$.
\end{defi}
...
\begin{bsp}
	...
	\begin{itemize}
		\item $2\IZ < (\IZ, +)$
		\item $n\in\mathbb N$; $\OO(n) = \{A\in \GL_n(\IR)|AA^{T} = \mathds 1_n\}< \GL_n(\IR)$ die orthogonale Gruppe
		\item  $n\in\mathbb N$; $\UU(n) = \{A\in GL_n(\IC)|A\overline A^{T} = \mathds 1_n\}< \GL_n(\IC)$ die unitäre Gruppe
		\item $\SL_n(K) = \{A\in \GL_n(K)|\det(A)=1\}<\GL_n(K)$
		\item $\SO(n) = \OO(n)\cap \SL_n(\IR)<\OO(n)$
		\item Spezielle unitäre Gruppe
		\item $\HH(3,\IR) = \left\{\left(\begin{array}{ccc}
			1 & a & b \\ 
			0 & 1 & c \\ 
			0 & 0 & 1
		\end{array}\right) \right\}$: Obere Dreiecksmatrizen, nur Einsen auf der Diagonalen (Heisenberggruppe)
	
	\end{itemize}
\end{bsp}

\begin{defi}
	Sei $(G,\circ)$ eine Gruppe. Sei $\emptyset\neq N\subseteq G$. Dann ist $\<N\>$ die (bezüglich Inklusion) kleinste Untergruppe von $G$, die $N$ enthält (aus $H<G$ mit $N\subseteq H$ folgt also $\<N\>\subseteq H$). Wir nennen $\<N\>$ die von $N$ erzeugte Untergruppe von $(G,\circ)$.
\end{defi}

\begin{bem}
	$\<N\>$ ist wohldefiniert, denn seien $H_1, H_2<G$ mit $N\subseteq H_1, N\subseteq H_2$, dann liegt $N$ in $H_1\cap H_2$ und es gilt $H_1\cap H_2<G$. Also eine existiert kleinste Untergruppe, die $N$ enthält; $\<N\>$ ist wohldefiniert.
\end{bem}

\begin{defi}
	Sei $G$ eine Gruppe und $N\subseteq G$.
	\begin{enumerate}
		\item $N$ erzeugt die Gruppe $G$, falls $\<N\> = G$. In diesem Fall heißt $N$ Erzeugendensystem der Gruppe $G$.
		\item $(G,\circ)$ heißt endlich erzeugt als Gruppe, falls ein $N\subseteq G$ mit $|N|$ endlich und $G = \<N\>$ existiert.
	\end{enumerate}
\end{defi}

\begin{bem}
	Sei $(G,\circ)$ eine Gruppe sowie $N\subseteq G$. Dann wird $G$ genau dann von $N$ erzeugt (also $G = \<N\>$), wenn für alle $g \in G$ Elemente $n_1,\dots,n_r\in G$ mit $r\in \IN_0$ existieren, sodass $g = n_1\circ \dots \circ n_r$ (mit $g=e$, falls $r=0$) und $n_i\in N$ oder $n_i^{-1}\in N$ für alle $1\leq i\leq r$ gelten.
\end{bem}

\begin{proof}
	\leavevmode
	\begin{description}
		\item[\glqq$\Leftarrow$\grqq:] Sei $g\in G$ und $g = n_1\circ\dots \circ n_r$ wie oben. Daraus folgt $g\in \<N\>$, da $n_1,\dots,n_r \in \<N\>$ und damit auch $g\in \<N\>$, weil $\<N\>$ eine Gruppe ist. Deshalb folgt $G\subseteq \<N\>$, also $G = \<N\>$.
		\item[\glqq$\Rightarrow$\grqq:] Sei $G = \<N\>$. Wir behaupten $H\coloneqq\{g\in G \mid \text{$g$ von obiger Form}\}<G$, was der aufmerksame Leser sich in einer ruhigen Minute selber überlegen möge.
		
		Da $\<N\>\subseteq H$ nach Definition von $\<N\>$ gilt und $\<N\>$ eine Gruppe ist, muss also $\<N\> = H$ wegen Minimalität gelten, da $N\subseteq H$ gilt. Nach Voraussetzung folgt $G = H$. Also hat jedes $g\in G$ die obige Form.
		\qedhere
	\end{description}
\end{proof}
...
\begin{defi}
	Eine Gruppe $G$ heißt zyklisch, falls ein $g\in G$ existiert, sodass $\gen{\{g\}} = G$ gilt, also $G$ von einem Element erzeugt wird.
\end{defi}

\medskip

Man beachte $\gen{\{g\}} = \{e, g, g^{-1}, g^2, g^{-2},\dots\} = \{g^i\mid i\in\IZ\}$.
...
\begin{defi}
	Seien $(G,\circ)$ und $(G',\circ')$ Gruppen. Ein Gruppenhomomorphismus von $G$ nach $G'$ ist eine Abbildung $f\colon G\to G'$ mit $f(g\circ h) = f(g)\circ'f(h)$ für alle $g,h \in G$.
	
	$f$ ist ein Gruppenisomorphismus, falls $f$ zusätzlich invertierbar ist. Wir schreiben $(G,\circ)\cong (G',\circ')$, falls ein Gruppenisomorphismus von $G$ nach $G'$ existiert und nennen die Gruppen isomorph.
\end{defi}

\paragraph{Eigenschaften von Gruppenhomomorphismen.} Sei $f\colon G\to G'$ ein Gruppenhomomorphismus von $G$ nach $G'$. Dann gilt:
\begin{enumerate}
	\item[(E1)] $f$ ist genau dann ein Gruppenisomorphismus, wenn $f^{-1}$ ein Gruppenisomorphismus ist. Nach Definition existiert $f^{-1}$; es ist somit $f^{-1}(g'\circ' h')= f^{-1}(g')\circ f^{-1}(h')$ für alle $g',h'\in G$ zu zeigen. Seien also $g', h' \in G' $. Es existieren $g, h\in G$ mit $f(g) = g', f(h) = h'$. Es folgt
	\begin{equation*}
		f^{-1}(g'\circ'h')= f^{-1}(f(g)\circ'f(h)) = f^{-1}(f(g\circ h))= g\circ h  = f^{-1}(g')\circ f^{-1}(h').
	\end{equation*}
	\item[(E2)] $f$ bildet das neutrale Element auf das neutrale Element ab.

	\lecture{12. Oktober 2017}

	\item[(E3)] $f$ bildet Inverse auf Inverse ab.\label{ghomo:e3}
	\item[(E4)] Sei $(G'',\circ'')$ eine weitere Gruppe sowie $f'\colon G'\to G''$ ein Gruppenhomomorphismus von $(G',\circ')$ nach $(G'',\circ'')$. Dann ist $f'\circ f$ ein Gruppenhomomorphismus, da
	\begin{equation*}
...
\begin{bsp}
	...
	\begin{itemize}
		\item $\id_G\colon G\to G,\ g\mapsto g$ ist ein Gruppenhomomorphismus von $(G,\circ)$ nach $(G,\circ)$.
		
		\textbf{Achtung:} $\id_G$ ist kein Gruppenhomomorphismus von $(G,\circ)$ nach $(G,\circ_a)$, falls $a\neq e$.
		
		\item $\det\colon \GL_n(K)\to K^*$ für einen Körper $K$ ist ein Gruppenhomomorphismus.
		\item $f\colon \IR^*\to \IR_{\geq 0},\ x\mapsto |x|$ ist ein Gruppenhomomorphismus von $(\IR^*,\cdot)$ nach $(\IR_{\geq 0}, \cdot)$.
		\item $x\mapsto \exp(x)$ ist ein Gruppenhomomorphismus von $(\IZ,+)$ nach $(\IR^*,\cdot)$.
		...
		1 & a \\ 
		0 & 1
		\end{array} \right)$. Das ist ein Gruppenhomomorphismus von $(\IZ,+)$ nach $(G,\mbox{Matrixmultiplikation})$. Es ist sogar ein Gruppenisomorphismus mit dem Inversen $\left(\begin{array}{cc}
		1 & a \\ 
		0 & 1
		\end{array} \right)\mapsto a$.
		\item \textit{Trivialer Gruppenhomomorphismus}: Schicke alles auf das neutrale Element.
		\item Sei $(G,\circ)$ eine Gruppe und $a\in G$. Dann ist $f\colon G\to G,\ g\mapsto g\circ a^{-1}$ ein Gruppenhomomorphismus von $(G,\circ)$ nach $(G,\circ_a)$.
	\end{itemize}
\end{bsp}
...
\begin{lem}
	Sei $n\in \IZ$.
	\begin{enumerate}
		\item Dann existiert genau ein Gruppenhomomorphismus $\can\colon \IZ\to \IZ/n\IZ$ von $(\IZ,+)$ nach $(\IZ/n\IZ,+)$ mit $\can(1)=\overline{1}$.
		\item Falls $n\neq 0$, so existiert kein nichttrivialer Gruppenhomomorphismus $f\colon \IZ/n\IZ\to\IZ$.
	\end{enumerate}
\end{lem}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item \begin{description}
			\item[Eindeutigkeit:] Sei $f\colon \IZ\to \IZ/n\IZ$ ein Gruppenhomomorphismus. Dann folgt $f(0)=\overline{0}$ und falls $f(1) = \overline{1}$, so gilt $f(n)= f(1+\dots 1) = n\cdot f(1)$ für alle $n\in\IN$ und damit auch $f(-n) = -nf(1)$. Dadurch ist $f$ eindeutig definiert.
			\item[Gruppenhomomorphismus:] Es gilt dann $\text{can}(x) = \overline{x}$ für alle $x\in\IZ$ und da $\text{can}(x+y) = \overline{x+y} = \overline{x}+\overline{y} = \text{can}(x)+\text{can}(y)$ ist das auch ein Gruppenhomomorphismus.
		\end{description}
		\item Sei $n\neq 0$, $f\colon \IZ/n\IZ\to \IZ$ ein Gruppenhomomorphismus und $f(\overline{1})= x$. Dann gilt (o.B.d.A. $n\in\IN$) $0=f(\ol 0)=f(\overline{n})= f(\overline{1}+\dots \overline{1})=nf(\overline{1})= nx$, also $x=0$. Somit ist $f$ ein trivialer Gruppenhomomorphismus.
		\qedhere
	\end{enumerate}
\end{proof}
...
\begin{lem}
	...
	\begin{enumerate}
		\item Sei $\Aut(G) := \{f\colon G\to G \mid \text{$f$ Gruppenisomorphimus von $(G,\circ)$ nach $(G,\circ)$}\}$. Dann ist $\Aut(G)$ eine Gruppe, die Automorphismengruppen von $G$.
		\item Betrachte die Abbildung $\Konj\colon G\to \Aut(G) ,\ g\mapsto \Konj(g)$, wobei $\Konj(g)(h)= g\circ\ h\circ g^{-1}$ für alle $h\in G$. Dann ist $\Konj$ ein Gruppenhomomorphismus von $G$ nach $\Aut(G)$, welcher im Allgemeinen nicht injektiv ist.
	\end{enumerate}
\end{lem}
...
\begin{bem}
	\leavevmode
	\begin{enumerate}
		\item 	Falls $(G,\circ)$ abelsch ist, dann ist jede Konjugation die Identität.
		\item $\Konj(g) = \text{id}_G \Leftrightarrow g\in Z(G) := \{x\in G \mid x\circ y = y\circ x\enspace \forall y\in G\}$
	\end{enumerate}
\end{bem}
...
\begin{satz} \label{thm:kerim_g}
	Sei $f\colon G\to G'$ ein Gruppenhomomorphismus. Dann gilt:
	\begin{equation*}
	\begin{array}{llll}
		\ker(f) & \coloneqq \{g\in G\mid f(g) = e\}                  & <G & \mbox{ Kern von }f\\
		\im(f)  & \coloneqq \{g'\in G'\mid \exists g\in G: f(g)=g'\} & <G'& \text{ Bild von }f
	\end{array}
	\end{equation*}
\end{satz}
\begin{proof}
...
\paragraph{Übung:} Ein Gruppenhomomorphismus$f$ ist genau dann injektiv, wenn $\ker f = \{e\}$.
...
\begin{proof}
	\leavevmode
	\begin{description}
	\item[Wohldefiniertheit:] $\Phi(g)$ ist invertierbar mit Inversem $h\mapsto g^{-1}h$.
	...
	\item[Injektivität:] Es reicht zu zeigen, dass der Kern trivial ist. Sei $g\in \ker\Phi\Leftrightarrow \Phi(g) = e = \text{id}_G \Leftrightarrow \Phi(g)(h)= h ~ \forall h\in G\Leftrightarrow gh = h ~ \forall h\in G\Leftrightarrow g= e$.
	\qedhere
	\end{description}
\end{proof}
...
\begin{defi}
	...
	\begin{itemize}
		\item $aH = \{ah\mid h\in H\}\subseteq G$ die Linksnebenklasse von $H$ zu $a$
		\item $Ha = \{ha\mid h\in H\}\subseteq G$ die Rechtsnebenklasse von $H$ zu $a$
	\end{itemize}
...
\noindent Aus der Linearen Algebra wissen wir folgendes: \begin{enumerate}
	\item Zwei Nebenklassen sind gleich oder disjunkt d.h. $aH\cap bH \neq \emptyset \Leftrightarrow aH = bH\Leftrightarrow b^{-1}a \in H$.
	\item Die Abbildung $aH\to H,\ ah\mapsto h$ ist bijektiv. Alle Nebenklassen haben somit dieselbe Kardinalität. \label{enumi:nebenkl:2}
	\item Es gilt \[ G = \bigcup_{\mathclap{g\in G}}gH = \dot{\bigcup_{\mathclap{b\in R}} }bH, \] wobei $R\subseteq G$, sodass die $bH$ mit $b\in R$ genau ein Repräsentantensystem für die verschiedenen Nebenklassen bilden. \label{enumi:nebenkl:3}
	\item $g\in aH\Leftrightarrow g^{-1}\in Ha^{-1}$ (dadurch ergibt sich eine Bijektion zwischen Links- und Rechtsnebenklassen) \label{enumi:nebenkl:4}
\end{enumerate}

\begin{defi}
	Bezeichne mit $G/H$ die Menge der Linksnebenklassen von $G$ bezüglich $H$ und mit $ H\backslash G$ die Menge der Rechtsnebenklassen. Dann gilt $|G/H| = |H\backslash G|$ (nach dem \ref{enumi:nebenkl:4}. Beispiel oben). Wir nennen diese Zahl den Index, auch $(G:H)$, von $H$ in $G$.
\end{defi}

\begin{satz}[Satz von Lagrange] \label{thm:lagrange}
	Sei $G$ eine eine endliche Gruppe sowie $H<G$ eine Untergruppe. Dann gilt
	\[
		|G| = |H|\cdot (G:H).
	\]
...
\begin{proof}
	Die Formel folgt direkt aus Beispiel \ref{enumi:nebenkl:3}, \ref{enumi:nebenkl:2} und der Definition des Index.
...
\end{align*}
Im Allgemeinen (falls $G$ nicht abelsch ist) ist $\circ$ nicht wohldefiniert (siehe \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-02.pdf}{Übungsblatt 2}).

\begin{defi}
	Sei $G$ eine Gruppe. Eine Untergruppe $H<G$ heißt Normalteiler, falls
	\begin{equation*}
	\forall g\in G, h\in H: g\circ h\circ g^{-1}\in H
	\end{equation*}
	gilt. Wir schreiben dann $H\vartriangleleft G$.
\end{defi}
\begin{bem}
...
\begin{lem} \label{lem:ker_nt}
	Sei $f\colon G\to G'$ ein Gruppenhomomorphismus.  Dann gilt $\ker(f)\nt G$.
\end{lem}
\begin{proof}
...
\begin{lem}
	...
	\begin{enumerate}
		...
		\item $f\colon G/G_x\to G.x, gG_x\mapsto g.x$ ist wohldefiniert, bijektiv und ein $G$-Homomorphismus \textup(wobei $G$ links wie in Beispiel \ref{bsp:linkstrans} oben und rechts durch $G\operates X$ operiert\textup).
...
\begin{satz} \label{thm:bahnenformel fuer konj}
...
\end{satz}
\begin{proof}
	\href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-03.pdf}{Übungsblatt 3}.
\end{proof}
...
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Sei $1\leq k \leq r$. Der Fall $k = 0$ ist klar mit $H = \{e\}$. Sei $X = \{A\subseteq G \mid |A| = p^k\}$, wobei $|X| = \binom{p^rm}{p^k}$. Nach \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-03.pdf}{Übungsblatt 3} gilt: $p^{r-k+1}\nmid |X|$.
		
		Nun $G \operates X$ durch $g.A \coloneqq gA=\{ga\mid a\in A\}$ für alle $g\in G, A\in X$. (klar: $|gA| = p^k $, also $gA\in X$). Nachrechnen: (O1), (O2) gelten (offensichtlich).
		
		Nach der \namereff{thm:bahnformel} folgt $|X| = \sum_{i\in I}(G:G_{x_i})$, wobei $\exists i\in I$, sodass $p^{r-k+1} \nmid (G:G_{x_i})$, weil $p^{r-k+1}\nmid |X|$. Wähle solch ein $x_i = : A'\in X$.
		...
		\item Sei $U<G$ mit $|U| = p^s$ für ein $s\in\IN$. Sei $S\in \Syl_p(G)$. Sei $U \operates G/S$ wie in Beispiel \cref{bsp:linkstrans} auf Seite \pageref{bsp:linkstrans} durch Linksmultiplikation gegeben. Es gilt nun
		\begin{align*}
		...
		\item \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-03.pdf}{Übungsblatt 3}.
  \qedhere
	\end{enumerate}
\end{proof}
...
\begin{defi}
	Sei $S$ ein kommutativer Ring und $R\subseteq S$ ein Unterring. Seien $a_1,\dots, a_n\in S$. Dann heißen $a_1,\dots,a_n$ algebraisch unabhängig (über $R$), fallls $\ev = \ev_{a_1,\dots, a_n}$ (von \cref{thm:evalpolyring}) injektiv ist.
...
\begin{satz}\label{thm:max_ideal_ex}
	...
	und $R \neq \{0\}$.
	\begin{enumerate}
		\item $R$ besitzt maximale Ideale \textup(und damit auch Primideale nach \cref{cor:maxprim}\textup).
...
\begin{defi} Sei $R$ ein kommutativer Ring und $R\neq \{0\}$. Eine Teilmenge $S\subseteq R$ heißt multiplikativ abgeschlossen, falls $1\in S$ und $s_1,s_2\in S\Rightarrow s_1s_2\in S$ gelten. Dann nennen wir $S^{-1}R$ die Lokalisierung von $R$ an $S$. Dabei ist $S^{-1}R$ ein kommutativer Ring, wobei die Elemente die Äquivalenzklassen in $R\times S$ bezüglich $\sim$ definiert durch
	...
	\end{equation*}
	(siehe \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-05.pdf}{Übungsblatt 5}) sind.
	...
	\begin{enumerate}
		...
		\begin{center}
			\begin{tikzcd}
				...
				& S^{-1}R\arrow{u}[swap]{\exists! \hat{\phi}\text{ Ringhomomorphismus}} \\
			\end{tikzcd}
		\end{center}
	\end{enumerate}
\end{satz}
...
\begin{bsp}
	...
	\begin{enumerate}
		...
		$\begin{array}{rcccccl}
		\phi\colon R & \xlongrightarrow{\can} & \Quot(R) &\longto& (\Quot(R))[t] &\longto& \Quot((\Quot(R))[t])\\
...
\begin{kor}
	Sei $K$ ein Körper. Dann ist $K[t_1,\dots,t_n]$ faktoriell. Ebenso ist $\IZ[t_1,\dots, t_n]$ faktoriell \textup(aber kein Hauptidealring, vgl. \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-05.pdf}{Übungungsblatt 5}\textup).
\end{kor}

\begin{proof} $K$ und $\IZ$ sind beide Hauptidealringe, weshalb $K$ und $\IZ$ nach \cref{thm:hirfakt} also faktoriell sind. Laut \namereff{thm:gauss} sind damit $K[t_1]$ und $\IZ[t_1]$ faktoriell. Durch iterative Anwendung folgt die Aussage.
\end{proof}
...
\begin{proof}
	Wir nehmen an, dass $PQ\in R[t]$ nicht primitiv ist. Dann existiert ein $r\in R\setminus R^{\times}$ mit $r\mid PQ$. Sei ohne Beschränkung der Allgemeinheit $r$ prim. Betrachte $R/(r)$. Da $r$ prim ist, ist $(r)$ ein Primideal. Folglich ist $R/(r)$ ein Integritätsbereich, also insbesondere nullteilerfrei.
...
\begin{proof}[Beweis des \hyperhyper{thm:gauss}{Satzes von Gauß}] %TODO: herausfinden, warum hier optionale Argumente nicht funktionieren
	...
	\begin{description}
		...
		%TODO: Ka wie man diese overfull hbox fixt, ich geb mal auf
...
\begin{satz}[Eisensteinsches Irreduzibilitätskriterium] \label{thm:eisenstein}
	...
	\end{align*}
	Existiert ein solches $p$, dann ist $f$ irreduzibel in $\IZ[t]$ und somit auch in $\IQ[t]$.
\end{satz}
...
\begin{anw}
Sei $p$ prim. Dann ist $\Phi_p(t) \coloneqq t^{p-1}+\dots +t+1$ irreduzibel in $\IZ[t]$. Wir nennen $\Phi_p(t)$ das $p$-te Kreisteilungspolynom.
\end{anw}
...
\begin{proof}
	...
	\end{align*}
	Nun gilt $p \nmid a_{p-1}$ und $p \mid a_i$ für $0 \leq i \leq p-2$ und $p^2 \nmid a_0$. Somit sind die Kriterien für das \hyperreff[thm:eisenstein]{Eisensteinsche Irreduzibilitätskriterium} erfüllt. Folglich ist $f\left( \Phi_p(t) \right) \in \IZ[t]$ irreduzibel. Damit ist auch $\Phi_p(t)$ irreduzibel.
\end{proof}
...
Deshalb können wir $\IQ$ als Teilmenge von $K_p$ vermöge der (injektiven) Einbettung \[\IQ \longhookrightarrow \IQ[t] \xlongrightarrow{\can} \IQ[t]/(\Phi_p(t))\] auffassen. $K_p$ ist dadurch ein $\IQ$-Vektorraum (nachrechnen!).
...
\begin{proof}
	...
	\begin{description}
		...
		\]
		Dann liegt $Q(t)$ in $I = \Phi_p(t)$ und folglich gilt $\Phi_p(t) \mid Q(t)$. Es folgt also, dass $\deg(Q(t)) \geq p-1$ oder $Q(t) = 0$, wobei ersteres ein Widerspruch zur Definition ist. Folglich gilt $Q(t) = 0$, womit alle $a_i = 0$ und $B$ ist damit linear unabhängig.
  \qedhere
	\end{description}
\end{proof}
...
\begin{lem} \label{lem:kreisteilkoerpernst}
	Sei $\phi \in G$ sowie $a_i \in \IQ$ für alle (endlich viele) $i$. Sei weiterhin
	\begin{equation*}
...
Nun ist $\ol t$ eine Nullstelle von $X^p-1\in K_p[X]$, weil $X^p-1=\Phi_p(X)\cdot (X-1)$ und $\Phi_p(\ol{t})=0$ in $K_p$ gilt. Damit ist $\varphi(\ol t)$ eine Nullstelle von $X^p-1$ falls $\varphi \in G$.

Andererseits ist $\ol t^k$ für $0\le k \le p-1$ eine Nullstelle von $X^p -1$, weil $$\left(\ol t^k\right)^p = \left(\ol t ^p\right)^k = \ol 1 = 1 \in K_p$$ gilt. Die $\ol t^k$ für $0\le k \le p-2$ sind paarweise verschieden, da sie linear unabhängig sind.
...
\begin{defi}
	Sei $K$ ein Körper sowie $N\subseteq K$ eine Teilmenge von $K$. Dann ist
	\[
...
\begin{proof}
	...
	\begin{description}
		\item[\emph{Fall 1: $\chr K = p > 0$.}] Dann liegt $p\cdot 1  = 0$ in $K$, es gilt also $\phi(pm) = 0$ für alle $m\in\IZ$, also folgt $p\IZ\subseteq \ker \phi$. Nun existiert nach dem \namereff{thm:homsatz_r} ein Ringhomomorphismus $\overline{\phi}\colon \IZ/p\IZ\to K$ mit $\overline{\phi}\circ \can  = \phi$. Da $p$ prim ist, ist $\IZ/p\IZ = \IF_p$ ein Körper. Nach \cref{lem:koerhomoinj} ist $\overline{\phi}$ folglich injektiv und $\im\ol\phi$ ist im Primkörper von $K$ enthalten, wobei $\im\overline{\phi} \cong \IF_p$ nach dem \namereff{thm:homsatz_r}. Weil der Primkörper der kleinste Unterkörper von $K$ ist und $\im\overline{\phi}$ ein Körper (isomorph zu $\IF_p$) ist, ist $\im \ol \phi$ der Primkörper von $K$.
		\item[\emph{Fall 2: $\chr K = 0$.}] Dann ist $\phi$ injektiv, denn aus $\phi(n) = \phi(m)$ folgt $(n-m)1 = 0$ in $K$, also gilt $n = m$ oder  $\chr K > 0$, wobei letzteres aber nicht der Fall ist. Also ist $\phi(n) \neq 0$ für alle $n\neq 0$, da $\phi$ ein Gruppenhomomorphismus ist. Damit gilt $\phi(n)\in K^{\times}$ für alle $n \in \IZ\setminus\{0\}$. Nach der \hyperreff[thm:unieig_lokali]{universellen Eigenschaft der Lokalisierung} existiert ein Ringhomomorphismus $\hat{\phi}\colon \IQ= \Quot (\IZ)\to K, \frac ab \mapsto \phi(a)(\phi(b))^{-1}$. Mit \cref{lem:koerhomoinj} folgt dann, dass $\hat{\phi}$ injektiv ist, und weiter gilt nach dem \namereff{thm:homsatz_r} $\IQ/\ker\hat{\phi} \cong \im\hat{\phi}$, also $\IQ \cong \im \hat{\phi}$ (Isomorphie von Ringen bzw. Körpern). Wir wissen, dass $\im \phi$ im Primkörper von $K$ enthalten ist. Nach Definition von $\hat{\phi}$ liegt auch $\im\hat{\phi}$ im Primkörper von $K$, weil der Primkörper ein Körper ist. Damit haben wir nun einen Unterkörper $\im \hat{\phi}$ als Teilmenge des Primkörpers von $K$, also ist $\im \hat{\phi}$ der Primkörper, weil dieser minimal ist. Also ist $\IQ$ isomorph zum Primkörper von $K$.
  \qedhere
	\end{description}
\end{proof}
...
\begin{defi} \label{def:endlerz und einfach}
...
Sei $L\sslash K$ eine Körpererweiterung sowie $a\in L$. Die Inklusion $K\to L, \lambda\mapsto\lambda$ ist ein Ringhomomorphismus. Nach der \hyperreff[thm:unieig_polyring]{universellen Eigenschaft des Polynomrings} existiert genau ein Ringhomomorphismus
\begin{eqnarray*}
...
\begin{description}
	...
	Also existiert ein Isomorphismus von Ringen $K[t]\to K[a]$ nach dem \namereff{thm:homsatz_r}. Nach Definition ist $K[a]\subseteq K(a)$ und somit gilt
	...
	\item[\emph{Fall 2: $a$ ist algebraisch über $K$.}] Nach Definition ist $\ev_a$ nicht injektiv. Also gilt, dass $\ker(\ev_a)\neq \{0\}$ ein Ideal in $K[t]$ ist. Da $K[t]$ ein Hauptidealring ist, existiert ein $p(t)\in K[t]$ mit $\ker(\ev_a) = (p(t))$. Sei ohne Beschränkung der Allgemeinheit $p(t)$ normiert (d.h. der Leitkoeffizient ist $1$).
	...
	Vermöge des \namereff{thm:homsatz_r} erhalten wir schließlich einen Ringhomomorphismus $\overline{\ev_a}\colon K[t]/(m_a(t))\to \im(\ev_a)\subset L$.
...
\lecture{30. November 2017}
	...
	Da $\overline{\ev_a}\colon K[t]/(m_a(t))\to \im\ev_a = K[a]\subseteq L$ ein Ringhomomorphismus und auf einem Körper definiert ist, ist $\overline{\ev_a}$ injektiv und $K[a]$ somit ein Körper. Also gilt $K[a]\subseteq K(a) \subseteq L$. Da $K(a)$ der kleinste Körper ist, der $K$ und $a$ enthält, folgt $K[a] = K(a)$.
	
	\emph{Behauptung:} $[K(a):K] = d \coloneqq \deg(m_a(t))$. Genauer: $1, a, a^2,\dots, a^{d-1}$ ist eine Basis von $K(a)$ als $K$-Vektorraum.
	\begin{proof}
		\leavevmode
		\begin{description}
			\item[\emph{Erzeugendensystem.}] Sei $k\geq 0$. Dann gilt $t^{d+k} = t^km_a(t)+Q$ in $K[t]$, wobei $Q$ eine Linearkombination von Polynomen mit Grad kleiner als $d+k$ ist. Also gilt in $K[t]/(m_a(t))$.
			\[\overline{t^{d+k}} = \overline{t^k}\overline{m_a(t)} + \overline{Q} = \overline{Q}.\]
			Damit folgt $\overline{t^{d+k}} \in \< \{1, \overline{t}, \dots, \overline{t^{d-1}}\}\>$. Folglich erzeugt $B \coloneqq  \{1, \overline{t}, \dots, \overline{t^{d-1}}\}$ dann $K[t]/(m_a(t))$ als $K$-Vektorraum. Wende nun die Evaluationsabbildung an:
			$\overline{\ev_a}(B)$ erzeugt $\im\overline{\ev_a} = K(a)$ als $K$-Vektorraum, da $\overline{\ev_a}$ insbesondere ein $K$-Vektorraum-Isomorphismus ist. Da $\overline{\ev_a}(B) = \{1, a, a^2,\dots, a^{d-1}\}$ gilt, folgt die Aussage.
...
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Sei $a\in L$. Dann existiert ein $m\in \IN$, sodass $1, a, a^2, \dots, a^m$ linear abhängig über $K$ ist, da nach Voraussetzung $\dim_KL<\infty$ gilt. Folglich existieren $c_i\in K$ ($0\leq i \leq m$), die nicht alle $0$ sind, mit
		$\sum_{i=0}^mc_ia^i = 0$. Also ist $\ev_a\colon K[t]\longto L$ nicht injektiv und $a$ somit algebraisch über $K$.
		\item Sei $L$ algebraisch und endlich erzeugt über $K$. Dann existieren Elemente $a_1,\dots, a_n\in L$ mit $L = K(a_1,\dots, a_n)$. Betrachte \[K \subseteq K(a_1) \subseteq K(a_1)(a_2) = K(a_1,a_2)\subseteq \dots \subseteq K(a_1,\dots, a_n) = L.\] Nach der \namereff{kor:gradformel} gilt $$[L:K] = [K(a_1,\dots, a_n):K] = \prod_{i = 1}^{n}[K(a_1, \dots, a_i):K(a_1,\dots, a_{i-1})]$$
		Nach Voraussetzung ist $a_i$ algebraisch über $K$, also insbesondere algebraisch über $K(a_1,\dots, a_{i-1})$. Somit gilt $[K(a_1,\dots, a_i): K(a_1,\dots , a_{i-1})] < \infty$ nach \cref{thm:algebraisch_aequivalenzen}. Schließlich folgt $[L:K]< \infty$.
		...
		Nach Konstruktion ist $K\sslash L_3$ endlich erzeugt; mit \ref{enumi:alg+endlicherz=>endlich} folgt, dass $K\sslash L_3$ endlich ist. Mit der \namereff{kor:gradformel} erhalten wir $[K(a):L_3] = [K(a):K][K:L_3]$. Somit ist $[K(a):L_3] <\infty$, also ist $a$ algebraisch über $L_3$ nach \ref{enumi:endlich=>algebraisch}. Somit ist $L_1 \sslash L_3$ algebraisch.
  \qedhere
	\end{enumerate}
\end{proof}
...
\begin{bsp}
	\leavevmode
	\begin{enumerate}
	\item Sei $p$ eine Primzahl und $n \in\IN\setminus\{0\}$. Dann ist $f(t) = t^n-p\in \IQ[t]$ irreduzibel (nach dem \hyperreff[thm:eisenstein]{Eisensteinschen Irreduzibilitätskriterium} irreduzibel in $\IZ[t]$ und primitiv, also irreduzibel in $\IQ[t]$ nach dem \namereff{thm:gauss}).
	Sei nun $a = \sqrt[n]{p}\in\IC$ eine Nullstelle von $f(t)$.
	
	Behauptung: $[\IQ(\sqrt[n]{p}): \IQ] = n$. Denn nach Definition des Minimalpolynoms $m_a(t)$ gilt $m_a(t)\mid f(t)$, also $f(t) = m_a(t)q(t)$ für ein $q(t)\in\IQ[t]$. Da $f(t)$ irreduzibel ist, folgt daraus $m_a(t)$ oder $q(t)\in \IQ[t]^{\times} = \IQ^{\times}$. Aber $m_a(t)$ kann keine Einheit sein; also $f(t) = m_a(t)\cdot \epsilon$ mit $\epsilon\in \IQ^{\times}$. Da $f(t)$ aber normiert ist, folgt $\epsilon = 1$. Mit \cref{thm:algebraisch_aequivalenzen} folgt die Behauptung.
...
\begin{satz} \label{thm:algkeirredpoly}
...
\begin{bem}
	Sei $f\in K[t]$ nicht notwendig irreduzibel. Dann können wir \cref{thm:algkeirredpoly} auf irreduzible Faktoren von $f$, etwa $g\in K[t]$, anwenden. Wir schreiben $f = g\cdot h$ mit $h \in K[t]$. Dann existiert eine Körpererweiterung $L\sslash K$, sodass $g$ in $L$ eine Nullstelle hat, und damit auch $f$. Weiterhin gilt $[L:K] = \deg(g)\leq \deg (f)$.
\end{bem}

\begin{bsp} 
	Sei $K = \IR$, sei $f= t^2-1\in \IR[t]$. Dann ist $f$ nicht irreduzibel, da $f = (t-1)(t+1) = gh$ mit $g = t-1$ und $h = t+1\in\IR[t]$ gilt. Sei also $L = K = \IR$, und wir erhalten 
	$[L:K] = 1 = \deg(g)<2 = \deg(f)$.
...
\begin{proof}[Beweis von \cref{thm:algab}]
	...
	\begin{description}
		...
		\item[\glqq$4\Rightarrow1$\grqq:] Sei $f\in K[t]\setminus K$. Nach Satz \ref{thm:algkeirredpoly} existiert eine algebraische Körpererweiterung $L\sslash K$, sodass $f$ in $L$ eine Nullstelle hat. Aber nach Voraussetzung ist $L = K$. 
  \qedhere
	\end{description}
\end{proof}
...
Zur Erinnerung: Für einen kommutativen Ring $R$ und eine Teilmenge $N\subseteq R$ bezeichnet $(N)$ das von $N$ erzeugte Ideal, also das kleinste Ideal in $R$, das $N$ enthält. Es gilt \[(N) = \left\{\sum_{i = 1}^mr_in_i \mathrel{\Big|} m\in\IN, n_i\in N, r_i\in R \;\forall i\right\}.\]
...
\begin{proof}[Beweis von \cref{thm:alg_ab}]
	...
	\begin{description}
		...
		Betrachte $R = K\left[X_f\mid f\in J\right]$ (siehe \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-09.pdf}{Übungsblatt 9} für eine saubere Definition).
		Das ist ein Polynomring über $K$ in unendlich vielen Variablen.
		
		Wir definieren $I \coloneqq (f(X_f)\mid f\in J)\subseteq R$ vermöge der \hyperref[thm:unieig_polyring]{universellen Eigenschaft von $K[t]$}. Es ist klar, dass $I$ ein Ideal ist; wir behaupten, dass $I$ sogar ein echtes Ideal ist. Der Beweis hierfür folgt weiter unten.
		...
		Wir wissen $f(\can(X_f)) = \sum_{i = 0}^{\infty}b_i\ol{X_f}^i$, wobei $b_i\in K$ ist. Also gilt \[f(\can(X_f)) = \sum_{i = 0}^{\infty}\ol{b_i}{X_f}^i = \ol{\sum_{i = 0}^{\infty}b_iX_f^i} = \can(f(X_f)),\] wobei $f(X_f)\in I\subseteq m$, also $f(\can(X_f)) = 0$. Also ist $\can(X_f)\in L_1$ eine Nullstelle von $f(t)$.
		...
		\item[\emph{Beweis von $I\neq R$:}] Sei $I = R$, also liegt $1\in I$. Somit existieren gewisse $g_i\in R$ und $f_i\in J$ mit $1 = \sum_{i =1}^{n}g_if_i(X_{f_i})$. Nach wiederholter Anwendung von \cref{thm:algkeirredpoly} existiert eine Körpererweiterung $L'\sslash K$, wobei $f_1,\dots, f_n$ Nullstellen in $L'$ haben, sagen wir $a_1,\dots, a_n$. Nach der \hyperref[thm:unieig_polyring]{universellen Eigenschaft für Polynomringe} existiert ein Ringhomomorphismus
		\begin{eqnarray*}
			\ev\colon R & \longto & L'\left[X_f\mid f\in J\right]
		\end{eqnarray*}
...
\begin{proof}
	...
	was nach \cref{lem:alguk} ein Körper ist.
...
\end{defi}
\bigskip
\noindent Wie sieht es nun mit der Eindeutigkeit von $\overline{K}$ aus (bis auf Isomorphie)?
\begin{defi}
...
\begin{defi}
	Die Menge 
	$$\Aut(L\sslash K) := \{\phi\colon L\to L\mid \phi\ K\text{-Automorphismus}\}$$
	ist eine Gruppe bezüglich der Komposition von Abbildungen (für jede Körpererweiterung $L\sslash K$). Wir nennen sie die Automorphismengruppe von $L\sslash K$.
\end{defi}
...
\noindent
Unser Ziel ist nun:
\begin{satz}[Eindeutigkeit des algebraischen Abschlusses]\label{thm:algAbEindeutig}
...
\begin{bem}
	Sei $\phi\colon L_1\to L_2$ ein $K$-Homomorphismus und $[L_1:K] = [L_2:K]<\infty$. Dann ist $\phi$ ein $K$-Isomorphismus, da $\phi$ ein Ringhomomorphismus und $L_i$ ein Körper ist. Somit ist $\phi$ injektiv. Außerdem ist $\phi(x+y) = \phi(x)+\phi(y)$ und $\phi(\lambda x) = \phi(\lambda)\phi(x) = \lambda\phi(x)$ für $\lambda\in K$ sowie $x,y\in L_i$. Also ist $\phi$ eine $K$-lineare Abbildung. Folglich ist $\phi$ eine $K$-lineare injektive Abbildung zwischen endlichdimensionalen $K$-Vektorräumen der gleichen Dimension, also ist $\phi$ bijektiv und damit ein $K$-Isomorphismus.
\end{bem}
...
\begin{bem}
	Beachte: $\phi$ bildet algebraische $a\in L_1$ auf Nullstellen von $m_a(t)$ ab!
\end{bem}
...
\begin{proof}
	...
	\begin{eqnarray*}
		...
		p & \longmapsto & p(a).
	\end{eqnarray*}
	Dann gilt $m_a(t)\mapsto m_a(a) = 0$, also $m_a(t) \in \ker(\ev_a)$, woraus $(m_a(t))\subseteq \ker(\ev_a)$ folgt. Nach dem \namereff{thm:homsatz_r} existiert genau ein Ringhomomorphismus \[\overline{\ev_a}\colon K[t]/(m_a(t))\to L,\] sodass $\overline{\ev_a}\circ \can = \ev_a$.
		...
		\begin{itemize}
			...
			\begin{proof}[Beweis der Behauptung]
				...
				\begin{itemize}
					...
					sodass $\overline{\ev_{a'}}\circ \can = \ev_{a'}$. Da $a$ algebraisch ist, ist andererseits 
					...
					Wir erhalten also einen Körper $M_{\max}(a)\subseteq L$ mit $M_{\max}\subsetneq M_{\max}(a)$.
...
\noindent Damit folgt der Eindeutigkeitssatz \ref{thm:algAbEindeutig}.

\medskip

\noindent Wie sehen nun zum Beispiel $\overline{\IQ}$ oder $\overline{\IF_2}$ aus?
\begin{bem}
...
Ziel: Gegeben eine Primzahl $p$ und $r\in\IN= \IZ_{>0}$. Dann existiert ein Körper $\IF$ mit $|\IF| = p^r$. Genauer gilt der folgende Satz.
\begin{satz}[Klassifikationssatz endlicher Körper]\label{thm:KlassEndlKoe}
...
\begin{proof}
	...
	\begin{description}
		\item[Beweis 1. Behauptung:] Sei $|G| = p_1^{c_1}\dots p_n^{c_n}$ mit paarweise verschiedenen Primzahlen $p_i$. Nach den \hyperreff[thm:sylow]{Sylowsätzen} existiert eine $p_i$-Sylowuntergruppe $G_{p_i}$ von $G$. Diese ist eindeutig, da jede andere $p_i$-Sylowuntergruppe zu dieser konjugiert ist, woraus sogar Gleichheit folgt, da $G$ abelsch ist. Es ist nach dem \namereff{thm:lagrange} klar, dass $G_{p_i}\cap G_{p_j} = \{e\}$ für $i\neq j$ gilt. Betrachte
		\begin{eqnarray*}
		...
		\begin{itemize}
			\item $f$ ist ein Gruppenhomomorphismus, da 
			\begin{multline*}
				f((g_1,\dots, g_n)(g_1',\dots, g_n')) = f((g_1g_1',\dots, g_ng_n')) = g_1g_1'g_2g_2'\dots g_ng_n'\\ = g_1g_2\dots g_ng_1'\dots g_n' = f((g_1,\dots, g_n))f((g_1',\dots, g_n'))
			\end{multline*} gilt.
			...
			Sei $\tilde{G} = \{e\}\times G_{p_2}\times\dots\times G_{p_n}$. Nach dem \namereff{thm:homsatz_g} gilt $f(\tilde{G})\cong \tilde {G}/\ker f|_{\tilde{G}}$. Damit folgt 
			$$|f(\tilde{G})| = \frac{|\{e\}\times G_{p_2}\times\dots\times G_{p_n}|}{\abs{\ker f|_{\tilde{G}}}}$$ nach dem \namereff{thm:lagrange}. $|f(\tilde{G})|$ teilt somit $p_2^{c_2}\dots p_n^{c_n}$. Analog zeigt man, dass $|f(G_{p_1})|$ dann $p_1^{c_1}$ teilt. 
			
			$\ord (g_2\dots g_n(g_2'\dots g_n')^{-1})$ teilt also $p_2^{c_2}\dots p_n^{c_n}$ und $\ord(g_1^{-1}g_1')$ teilt $p_1^{c_1}$. Da die $p_i$ paarweise verschieden sind, gilt $\ord(g_1^{-1}g_1') \mid \operatorname{ggT}(p_1^{c_1}, p_2^{c_2}\dots p_n^{c_n}) = 1$, also $g_1^{-1}g_1' = e$ und schließlich $g_1' = g_1$. Völlig analog folgt $g_i = g_i'$ für alle $1\leq i\leq n$.
	...
	Also ist $g^{p^m} = 1$ und $g$ ist eine Nullstelle des Polynoms $t^{p^m}-1$, und somit auch von $t^{p^{c-1}}-1$. Damit ist $g$ eine Nullstelle von $t^{p^{c-1}}-1$ für alle $g\in G_p$. Aber $t^{p^{c-1}}-1$ hat höchstens $p^{c-1}$ verschiedene Nullstellen, was im Widerspruch dazu steht, dass $|G_p| = p^c> p^{c-1}$ ist. Also ist $G_{p_i}$ zyklisch für alle $1\leq i\leq n$.
	...
	mit $m = p_1^{c_1}\dots p_n^{c_n}$. Dies ist insbesondere ein Isomorphismus von Gruppen. Da $\IZ/m\IZ$ zyklisch ist, folgt schließlich, dass $H$ zyklisch ist.
	\qedhere
\end{description}
\end{proof}
...
\end{bem}
\begin{lem}\label{lem:endlKoeIsoF_p}
...
\begin{proof}
	...
	\end{eqnarray*}
	Dieser induziert, weil $p(x) = 0$ ist, nach dem \namereff{thm:homsatz_r} einen Ringhomomorphismus
	\[\overline{\ev_x}\colon \IF_p[t]/(m_x(t)) \to \IF.\]
	Wir wissen, dass $K \coloneqq\IF_p[t]/(p(t))$ ein Körper ist, weil $p(t)$ irreduzibel ist. Folglich ist $\overline{\ev_x}\colon K\to \IF$ injektiv.
...
\begin{bem}
	Wir betrachten das $\Phi$ aus \cref{thm:KlassEndlKoe}. \cref{lem:endlKoeIsoF_p} besagt insbesondere, dass zwei endliche Körper derselben Kardinalität isomorph sind; $\Phi$ ist also injektiv. Es bleibt zu zeigen, dass $\Phi$ surjektiv ist.
\end{bem}

\medskip

Beachte: Sei $\IF$ wie im \cref{lem:endlKoeIsoF_p} definiert. Dann gilt:
\begin{enumerate}
...
\begin{satz}\label{thm:endlKoeEx}
	Sei $p$ eine Primzahl und $r\in\IN$. Dann existiert ein Körper $\IF$ mit $|\IF| = p^r$.
\end{satz}

\medskip

Zunächst noch ein {Nachtrag\label{nachtrag zu 17.4}} zu \cref{lem:endlKoeIsoF_p}. Beim dortigen Beweis bleibt noch zu zeigen, dass ein solches $p(t)$ existiert.
\begin{proof}
	Wir wissen:
	\begin{itemize}
...
\begin{satz}
	Sei $L\sslash K$ eine Körpererweiterung sowie $f,g\in K[t]$ und $g\neq 0$. Dann:
	\begin{enumerate}
...
\begin{proof}
	...
	\end{description}
\end{proof}
\begin{kor} \label{cor:irredPolyÄquiv}
	 %TODO: This makes no sense. Ein nicht konstantes Polynom ist wohl kaum die Charakteristik eines Körpers
...
\begin{proof}
	...
	\begin{description}
		...
		\item[\glqq$1\Rightarrow2$\grqq:] Sei $a$ eine mehrfache Nullstelle von $p$. Nach \cref{thm:18.3} ist $a$ eine gemeinsame  Nullstelle von $p$ und $p'$ bzw. $p$ und $p'$ haben einen gemeinsamen Teiler $h(t)\in L[t]\setminus L$. Es gilt $h(t)\neq 0$, da sonst schon $p(t) = 0$ im Widerspruch zur Irreduzibilität von $p$ ist. Da $p$ irreduzibel ist und $\deg(h(t)) \leq \deg(p')<\deg(p)$ im Fall von $p' \neq 0$, ist damit $h(t)$ eine Einheit im Widerspruch zu $h(t) \notin L$. Also ist $p' = 0$.
  \qedhere
	\end{description}
\end{proof}
...
\begin{proof}[Beweis von \cref{thm:endlKoeEx}]
...
\end{proof}
\noindent
Damit ist der \namereff{thm:KlassEndlKoe} gezeigt.

\begin{kor}
	Der Zerfällungskörper von $p(t) = t^n-t\in\IF_p[t]$ \textup(mit $p,n$ wie in \cref{thm:endlKoeEx}\textup) ist eindeutig bis auf Isomorphie.
\end{kor}
\begin{proof}
...
\end{satz}
\begin{proof}
	Wähle den bis auf Isomorphie eindeutigen algebraischen Abschluss $\overline{K}$ bzw. $\overline{K'}$ von $K$ bzw. $K'$ so, dass $K\subseteq L\subseteq \overline{K}$ und $K'\subseteq L'\subseteq \overline{K'}$. Der \namereff{thm:fortsetz} sagt uns nun, dass ein Homomorphismus $\hat\phi\colon \overline{K}\to\overline{K'}$ mit $\hat\phi|_K = \phi$ existiert. Falls $\phi$ ein Isomorphismus ist, dann ist auch $\hat\phi$ Isomorphismus von Körpern.
...
\begin{satz}
	...
	\begin{itemize}
		...
		\item $\Gal(\IF\sslash\IF_p) \cong (\IZ/r\IZ, +)$ als Gruppe, erzeugt von $\Fr$. Wir werden zeigen, dass daraus folgt, dass eine Bijektion \textup(Galoiskorrespondenz\textup) existiert:
		\begin{eqnarray*}
...
\begin{satz}[Spezialfall der Galoiskorrespondenz]\label{thm:spezielle galoiskorrespondenz} Sei $\IF$ ein endlicher Körper mit $|\IF| = n = p^r$ \textup($p$ prim\textup). Dann definiere
\[
...
\begin{lem} \label{lem:19.2}
	Sei $\IF$ ein endlicher Körper mit $|\IF| = n = p^r$ \textup($p$ prim\textup). Dann ist $G = \Gal(\IF/\IF_p)$ zyklisch der Ordnung $r$.
\end{lem}
\begin{proof}
...
\begin{proof}
  ...
  \qedhere
	\end{enumerate}
\end{proof}
Wir rufen uns an dieser Stelle die Definition einer einfachen Körpererweiterungen ins Gedächtnis (siehe \cref{def:endlerz und einfach}).
\begin{defi} $L\sslash K$ heißt einfach, falls ein $a\in L$ existiert, sodass $L\cong K(a)$ gilt.
\end{defi}
\begin{satz} \label{thm:khomo <-> nullstellen}
...
\begin{bsp}
	...
	\end{enumerate}
\end{bsp}
\begin{satz}[Charakterisierung von Normalität] \label{thm:normalitaet}
Sei $L\sslash K$ eine endliche Körpererweiterung. Dann sind äquivalent:
\begin{enumerate}
	\item $L\sslash K$ ist normal.
	\item $L$ ist Zerfällungskörper eines Polynoms $f\in K[t]$.\label{thm:normalitaet:2}
	\item Sei $M\sslash K$ eine Körpererweiterungen . Dann haben alle Körperhomomorphismen $\phi\colon L\to M$, die die Inklusion $K \hookrightarrow M$ fortsetzen, dasselbe Bild; das folgende Diagramm kommutiert also. \label{thm:normalitaet:3}
	\begin{figure}[h!]
		\centering
		\begin{tikzcd}
			L \arrow{r}{\phi} & M  \\
			K \arrow[hookrightarrow]{u}\arrow[hookrightarrow]{ru}\\
		\end{tikzcd}
	\end{figure}
\end{enumerate}
\end{satz}
%TODO: herausfinden, warum hier so viel freier Platz ist
\lecture{11. Januar 2018}
\begin{proof}
	\leavevmode
	\begin{description}
		\item[$1\Rightarrow2$:] Sei $L = K(a_1,\dots,a_n)$ für gewisse $a_1,\dots,a_n\in L$. Wir betrachten das Minimalpolynom $m_{a_i}(t)\in K[t]$ von $a_i$. Sei $f(t):=m_{a_1}(t)\dots m_{a_n}(t)\in K[t]$. Jedes $m_{a_i}$ hat eine Nullstelle in $L$, nämlich z.B. $a_i$. Da $L\sslash K$ normal ist, sind alle Nullstellen von $m_{a_i}(t)$ in $L$ für $1\le i \le n$, weil $m_{a_i}(t)$ irreduzibel. Damit sind alle Nullstellen von $f(t)$ in $L$ und somit ist $L$ Zerfällungskorper von $f(t)$.
		\item[$2\Rightarrow3$:] Ist $L$ Zerfällungskörper von $f(t)\in K[t]$, so gilt $ L = K(a_1,\dots, a_n)$ mit den $a_i$ als Nullstellen von $f(t)$. Seien $\phi_j\colon L\to M$ zwei $K$-Homomorphismen mit $j = 1,2$ wie in \ref{thm:normalitaet:3}. Es ist $\im \phi_1 = \im\phi_2$ zu zeigen.
		
		Es ist $\phi_j(a_i)$ eine Nullstelle von $f(t)$. Da die Nullstelle eindeutig ist, gilt nach \cref{thm:khomo <-> nullstellen} $\{\phi_1(a_i)\mid1\leq i\leq n\} = \{\phi_2(a_i)\mid1\leq i\leq n\}$. Da dadurch $\phi_1$ und $\phi_2$ durch $\phi|_K = \id$ und $\phi(a_i)$ für $1\leq i\leq n$ bereits eindeutig bestimmt sind, folgt $\im\phi_1 = \im\phi_2$.
		\item[$3\Rightarrow 1$:] Sei $f(t)\in K[t]$ irreduzibel. Sei $z\in L$ eine Nullstelle von $f(t)$. Für die Normalität von $L \sslash K$ ist zu zeigen, dass alle Nullstellen von $f(t)$ in $L$ liegen. Nach Voraussetzung gilt $L = K(b_1,\dots, b_n)$ für gewisse $b_i\in L$. Sei $M\sslash L$ eine Körpererweiterung, sodass $m_{b_1}(t),\dots , m_{b_n}(t)\in K[t]$ (die Minimalpolynome der $b_i$) über $M$ vollständig zerfallen. Sei $z'$ eine weitere Nullstelle von $f(t)$. Betrachte
		\begin{center}
			\begin{tikzcd}
				K(z) \arrow{r}{\phi} & M  \\
				K \arrow[hookrightarrow]{u}\arrow[hookrightarrow]{ru}
			\end{tikzcd}
		\end{center}
		wobei $K\hookrightarrow K(z)$ und $K\hookrightarrow M$ Inklusionen sind und $\phi$ ein $K$-Homomorphismus mit $\phi(z) = z'$ ist, welcher nach \cref{thm:khomo <-> nullstellen} existiert.
		
		Deshalb existiert ein $\hat{\phi}\colon L\to M$ mit $\hat{\phi}|_{K(z)} = \phi$, weil $L\sslash K$ endlich, also algebraisch ist und damit auch $L\sslash K(z)$ algebraisch ist. Offensichtlich gilt $\hat{\phi}(z) = z'$. Nun liegt jede Nullstelle von $f(t)$ im Bild von $\hat{\phi}$. Nach Voraussetzung liegt jede Nullstelle von $f(t)$ dann schon im Bild jedes $K$-Homomorphismus $L\to M$; insbesondere also in $L$; man nehme etwa $\id_L$ mit $\im \id_L = L$.
		\qedhere
	\end{description}
\end{proof}

\begin{bem}
	Man kann statt $L\sslash K$ endlich auch nur $L\sslash K$ algebraisch fordern und \ref{thm:normalitaet:2} ersetzen durch: \glqq$L$ ist Zerfällungskörper über $K$ (d.h. alle $f(t)\in K[t]$ zerfallen über $L$).\grqq. Letzterer existiert und ist gegeben durch den Zwischenkörper $K\subseteq M\subseteq \overline{K}$, welcher von allen Nullstellen der Polynome in $K[t]$ erzeugt ist. Wie vorher für Zerfällungskörper von Polynomen kann man zeigen, dass obiger Zerfällungskörper eindeutig bis auf $K$-Isomorphie ist.
	
	Zum Beweis dieser alternativen Formulierung des Satzes siehe \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-12.pdf}{Übungsblatt 12}.
\end{bem}

\begin{satz}\label{thm:existenz normaler oberkoerper}
...
\end{satz}
\begin{kor} \label{kor:19.8}
	Sei $L\sslash K$ eine Körpererweiterung und $K\subseteq M\subseteq L$ ein Zwischenkörper mit $M\sslash K$ normal (und endlich, wobei man das nach obiger Bemerkung auch weglassen kann). Sei $\phi\colon M\to L$ ein $K$-Homomorphismus. Dann ist $\phi(M) = M$.	
\end{kor}
\begin{proof}
	Betrachte die $K$-Homomorphismen $M\xrightarrow{\phi}L$ und $M\xrightarrow{\id}M\subseteq L$. Da $M\sslash K$ normal ist, gilt nach \cref{thm:normalitaet} $\im\phi = \im(\id_M) = M$.
\end{proof}

\begin{proof}[Beweis von \cref{thm:existenz normaler oberkoerper}]
	Da $L\sslash K$ endlich ist, gilt $L = K(a_1,\dots, a_n)$ für gewisse \linebreak $a_1,\dots,a_n\in L$. Sei $f(t) = m_{a_1}(t)\dots m_{a_n}(t)\in K[t]$ mit $m_{a_i}(t)$ als Minimalpolynom von $a_i$ für $1\le i \le n$. Sei $M$ Zerfällungskörper von $f(t)$ über $L$, also auch Zerfällungskörper von $f(t)$ über $K$. Nach \cref{thm:normalitaet} ist $M\sslash K$ normal.Es bleibt zu zeigen, dass $M\sslash K$ endlich ist. Wir behaupten, dass $[M:K]\leq \deg(f(t))$. Damit folgt der Satz; die Behauptung folgt aus \cref{lem:grad zerfkoerper}.
\end{proof}
\begin{lem} \label{lem:grad zerfkoerper}
	Sei $K$ ein Körper, $f(t)\in K[t]$, und $L$ Zerfällungskörper von $f(t)$. Dann gilt $[L:K]\leq(\deg(f(t)))!$.
\end{lem}
\begin{proof}
	Sei $L = K(b_1,\dots, b_n)$, wobei $b_i$ die (verschiedenen) Nullstellen von $f(t)$ sind. Sei weiterhin $m_{b_i}$ das Minimalpolynom von $b_i$ für $1 \le i \le n$. Insbesondere teilt $m_{b_i}(t)$ das $f(t)$, also gilt $[K(b_i):K] = \deg m_{b_i}(t)\leq \deg f(t)$. Es gilt $[L:K] = [L:K(b_1)][K(b_1):K]$. Da $L = K(b_1)(b_2,\dots, b_n)$ Zerfällungskörper von $f(t)/(t-b_1)$ ist, folgt induktiv $[L:K(b_1)][K(b_1):K]\leq (\deg(f(t)/(t-b_1))!\cdot\deg(f(t)) = (\deg(f(t)))!$
\end{proof}

\begin{bem} Sei $L\sslash K$ eine normale Körpererweiterung und $K\subseteq M\subseteq L$ ein Zwischenkörper. Dann ist $L$ eine normale Hülle von $M\sslash K$, falls es keinen Zwischenkörper $M\subseteq N\subseteq L$ mit $N\sslash K$ normal gibt. $L$ ist also der kleinste Körper, der $M$ enthält und normal über $K$ ist. Man kann zeigen, dass die normale Hülle eindeutig bis auf $K$-Isomorphie ist.
\end{bem}

\begin{defi} Sei $L\sslash K$ eine algebraische Körpererweiterung. \begin{enumerate}
		\item $f(t)\in K[t]$ heißt separabel, falls $f(t)$ keine mehrfachen Nullstellen in $\overline{K}$ hat.
		\item $a\in L$ heißt separabel, falls das Minimalpolynom von $a$ separabel ist.
...
\end{defi}
\begin{bsp} Jeder endliche Körper $\IF$ ist perfekt, da $\Fr\colon \IF\to \IF$ injektiv, da $\IF$ ein Körper ist, und damit bijektiv ist, weil $|\IF|<\infty$.
\end{bsp}
\begin{bsp} \label{bsp:separabel}
	\leavevmode
	\begin{enumerate}
	\item Jedes irreduzible Polynom $f(t)\in K[t]$ mit $\chr K = 0$ ist separabel.
	\item Betrachte $\IF_p(x)\sslash \IF_p(x^p)$ mit $x$ transzendent über $\IF_p$. Sei $f(t) = t^p-x^p\in \IF_p(x^p)[t]$. Dann ist $f(t)$ irreduzibel (nachprüfen!), aber zerfällt über $\IF_p(x)$ und hat mehrfache Nullstellen, da $t^p-x^p = (t-x)^p$.
	\end{enumerate}
\end{bsp}
...
\begin{proof}
	\leavevmode
	\begin{description}
		\item[$\chr K = 0$:] Sei $a\in L$ und $m_a(t) \in K[t]$ das zugehörige (irreduzible) Minimalpolynom. Nach \cref{bsp:separabel} hat $m_a(t)$ keine mehrfachen Nullstellen, also ist $a$ separabel über $K$. Da $a$ beliebig gewählt war, ist $L$ separabel über $K$.
		\item[$\chr K = p>0$:] Sei $a\in L$ und $m_a(t)\in K[t]$ das zugehörige Minimalpolynom. Wir nehmen an, $m_a(t)$ habe eine mehrfache Nullstelle. Mit \cref{cor:irredPolyÄquiv} folgt $m_a'(t) = 0$, $m_a(t) = Q(t^p)$ für ein $Q(t)\in K[t]$. Sei $Q(t)  =\sum_{i = 0}^n b_it^i\in K[t]$. Weil $K$ perfekt ist, existieren $a_1, \dots, a_n$ mit $a_i^p = b_i$. Dann folgt $\left(\sum_{i = 0}^na_it^i\right)^p = \sum_{i = 0}^nb_i(t^p)^i = Q(t^p) = m_a(t)$. Das ist ein Widerspruch zur Irreduzibilität von $m_a(t)$; folglich hat $m_a(t)$ keine mehrfachen Nullstellen. Damit ist $a$ separabel über $K$ für alle $a\in L$; $L\sslash K$ ist folglich separabel.
		\qedhere
	\end{description}
\end{proof}

\begin{defi}
	Sei $L\sslash K$ eine algebraische Körpererweiterung. Wir definieren
	\[[L:K]_{\sep} \coloneqq |\{\phi\colon L\to\overline{K}\mid \text{$\phi$ $K$-Homomorphismus}\}|\]
	als den Separabilitätsgrad von $L\sslash K$.
\end{defi}

\begin{bem}
	\leavevmode
	\begin{enumerate}
		\item Der Separabilitätsgrad ist unabhängig von der Wahl von $\overline{K}$.
		\item Seien $L_1\sslash K$ und $L_2\sslash K$ Körpererweiterungen und sei $\phi\colon L_1\to L_2$ ein $K$-Isomorphismus. Dann gilt $[L_1:K]_{\sep} = [L_2:K]_{\sep}$.
		\item Wir nennen Elemente $a\in L$ für eine Körpererweiterung $L\sslash K$ inseparabel, falls $a$ nicht separabel ist.
	\end{enumerate}
\end{bem}
\begin{lem}\label{lem:19.11}
	Sei $L\sslash K$ eine algebraische endliche Körpererweiterung. Dann gilt \[[L:K]_{\sep}\leq [L:K].\]
\end{lem}
\begin{proof}
	Sei $L = K(a_1,\dots, a_n)$ für gewisse $a_1,\dots, a_n\in L$. Sei $L_0 = K$ und sei $L_i = K(a_1,\dots, a_i)$, also $L_i = L_{i-1}(a_i)$. Wir wissen, dass für eine einfache algebraische Erweiterung $M(a)\sslash M$ für einen Körper $M$ nach \cref{thm:khomo <-> nullstellen} \[[M(a):M]_{\sep} = \left|\left\{\phi\colon M(a)\to \overline{M} \left| \substack{\text{$\phi$ $M$-Homo,}\\\deg \phi \le \deg m_a(t)\in M[t]}\right.\right\}\right|\] gilt, wobei $\deg m_a(t) = [M(a):M]$. Wir erhalten $[L:K]_{\sep} = \prod_{i = 1}^n[L_i:L_{i-1}]_{\sep} \leq \prod_{i = 1}^n[L_i:L_{i-1}] = [L:K]$.	
\end{proof}
\begin{lem}[Transitivität] \label{lem:sepgrad transitiv}
	Seien $L_1\sslash L_2$ und $L_2\sslash L_3$ Körpererweiterungen. Dann gilt \[[L_1:L_3]_{\sep} = [L_1:L_2]_{\sep}\cdot [L_2:L_3]_{\sep}.\]
\end{lem}
\begin{proof}
	Siehe \href{http://www.math.uni-bonn.de/people/palmer/EIDA-Blatt-12.pdf}{Übungsblatt 12}.
\end{proof}
\begin{bem}
	Sei $L\sslash K$ eine endliche, normale Körpererweiterung. Dann ist $\phi(L) = L$ für jeden $K$-Homomorphismus nach \cref{kor:19.8}. Also gilt \[[L:K]_{\sep} = |\{\text{$\phi\colon L\to L$ $K$-Homomorphismus}\} = |\Aut_K(L)|,\] weil jeder Homomorphismus injektiv und wegen der Endlichkeit von $\dim_LK$ sogar bijektiv ist.
\end{bem}



\lecture{15. Januar 2018}
\begin{lem}[Charakterisierung von Separabilität] \label{lem:separabel}
	...
	\begin{enumerate}
		...
		\item Für jeden Zwischenkörper $K\subseteq M \subseteq L$ gilt $[M:K]_{\sep} = [M:K]$.
	\end{enumerate}
	Falls $L \sslash K$ endlich ist, ist 3. äquivalent zu
	\begin{enumerate}
		\item[3'.] $[L:K]_{\sep} = [L:K]$.
	\end{enumerate}
\end{lem}
\begin{proof}
	Sei zunächst $L\sslash K$ endlich.
	\begin{description}
		\item[$3\Leftrightarrow 3'$:] Es gilt in beiden Fällen $[L:K]_{\sep} = [L:K]$. Sei $M$ ein Zwischenkörper von $L$ und $K$. Dann gilt $[L:M]_{\sep} \leq [L:M]$ und $[M:K]_{\sep}\leq [M:K]$; nach Transitivät (\cref{lem:sepgrad transitiv}) gilt jeweils Gleichheit, also folgt $3$.
	\end{description}
	Ab jetzt sei $L\sslash K$ nicht mehr notwendigerweise endlich.
	\begin{description}
		\item[$1\Rightarrow 2$:]  $\checkmark$
		\item[$2\Rightarrow 3$:] Betrachte $a\in L$. Dann folgt daraus, dass $a$ separabel über $K$ ist, auch, dass $a$ separabel über $M$ ist. Denn $m_{a,M}(t)\in M[t]$ teilt $m_{a, K}(t)\in K[t]\subseteq M[t] $ (das sind jeweils die Minimalpolynome von $a$ über den entsprechenden Körpern).
		
		Betrachte nun zunächst doch wieder $L\sslash K$ endlich. Sei $L = K(a_1,\dots, a_n)$ mit separablen $a_1, \dots, a_n\in L$. Setze $L_0:= K, L_i = K(a_1,\dots, a_i)$; insbesondere $L_i = L_{i-1}(a_i)$, wobei $a_i$ separabel über $L_{i-1}$ ist. Daraus folgt $[L_i = L_{i-1}(a_i):L_{i-1}]_{\sep} = [L_i:L_{i-1}]$ und nach Transitivität (\cref{lem:sepgrad transitiv}) erhalten wir $[L:K]_{\sep} = [L:K]$. Daraus folgt $3'$ und damit für endliche Körper $3$.
		
		Jetzt $L\sslash K$ beliebig (aber algebraisch). Betrachte zwei Fälle: \begin{itemize}
			\item Falls $M\sslash K$ endlich ist, existieren algebraische und separable $a_1,\dots, a_n\in L$ mit $M\subseteq K(a_1,\dots, a_n)$. Damit können wir $K(a_1,\dots, a_n)$ betrachten und mit unserer Vorüberlegung folgt die Aussage.
			\item Falls $M\sslash K$ nicht endlich ist, so gilt: $[M:K]_{\sep} = [M:M']_{\sep}[M':K]_{\sep}$ für jeden Zwischenkörper $K\subseteq M'\subseteq M$, also insbesondere für solche mit $M'\sslash K$ endlich. Also erhalten wir
		...
		\end{itemize}
		\item[$3\Rightarrow 1$:] Sei $a\in L$ und $M: = K(a)$. Nach Voraussetzung ist $[K(a):K]_{\sep} = [K(a):K]$, was äquivalent zur Separabilität von $a$ über $K$ ist. Da $a$ beliebig ist, ist $L\sslash K$ separabel, also folgt $1$.
		\qedhere
	\end{description}
\end{proof}
\begin{satz}[Satz vom primitiven Element]\label{thm:primitives element}
	Sei $L\sslash K$ eine endliche, separable Körpererweiterung. Dann existiert ein $a\in L$ mit $L = K(a)$. Man nennt $a$ dann primitives Element.
\end{satz}

\begin{proof}
	Da $L\sslash K$ endlich ist, reicht es zu zeigen, dass für alle $x,y\in L$ ein $a \in L$ existiert, sodass $K(x,y) =K (a)$ gilt.
	\begin{description}
		\item[Fall 1: $|K|<\infty$:] Da $K$ ein endlicher Körper ist, ist $L$ endlicher Körper, da $L\sslash K$ endlich ist. Auch $K(x,y)$ ist ein endlicher Körper. Folglich ist $K(x,y)^{\times}$ eine zyklische Gruppe (Eigenschaft endlicher Körper), also erzeugt als Gruppe von einem Element, sagen wir $c$. Es folgt $K(x,y) =K(c)$ (als Körper). Wir setzen $a\eqqcolon c$.
		\item[Fall 2: $|K|  =\infty$:] Sei $N = K(x,y)$ und $\overline{K}$ der algebraische Abschluss von $K$. Sei ${[N:K]_{\sep}} = m$ und $\{\phi_1,\dots,\phi_m\} = \{\phi\colon N\to \overline{K}\mid\text{$\phi$ $K$-Homomorphismus}\}$. Es gilt $[N:K]_{\sep} = [N:K]$ nach \cref{lem:separabel}. Sei
		\[ f = \prod_{i<j}( (\phi_i(x)-\phi_j(x))+(\phi_i(y)-\phi_j(y))t)\in\overline{K}[t]\]
		Falls $1\leq i\neq j\leq m$, so gilt $\phi(x) \neq \phi_j(x)$ oder $\phi_i(y)\neq\phi_j(y)$, da $\phi_i$ und $\phi_j$ bereits eindeutig durch ihre Werte auf $x$ und $y$ bestimmt sind.
		
		Somit folgt $f\neq 0$. Da $|K|$ unendlich ist, existiert ein $c\in K$ mit $f(c) \neq 0$. Für $i<j$ gilt also $(\phi_i(x)-\phi_j(y))+(\phi_i(y)-\phi_j(y))c \neq 0$. Damit folgt $\phi_i(x) + c\phi_i(y)\neq \phi_j(x)+c\phi_j(y)$ für $i\neq j$. Da $c\in K$ ist, gilt $\phi_i(c) =\phi_j(c) = c$. Damit folgt $\phi_i(x+cy)\neq \phi_j(x+cy)$, da $\phi_i$ und $\phi_j$ Körperhomomorphismen sind. Setze $a := x +cy$. Dann sind die $\phi_i(a)$ für $1\leq i\leq m$ paarweise verschieden. Die $\phi_i(a)$ sind Nullstellen von $m_a(t)\in K[t]$. %hier verweist Stroppel auf 19.14, aber das beweisen wir ja gerade!?!?
		Folglich gilt
...
\begin{defi}
	Sei $L\sslash K$ eine Körpererweiterung. Dann ist \[\Gal(L\sslash K) := \{\phi\colon L\to L\mid \text{$\phi$ $K$-Isomorphismus} \}\]
	die Galoisgruppe von $L\sslash K$. Das ist eine Gruppe bezüglich der Komposition von Abbildungen.
\end{defi}
\begin{lem}\label{lem:20.1}
	$\Gal(L\sslash K)\subseteq  A := \{\phi\colon L\to L \mid \text{$\phi$ $K$-Homomorphismus} \}$. Ist $L\sslash K$ algebraisch, dann gilt sogar Gleichheit.
\end{lem}
\begin{proof}
	Falls $L\sslash K$ endlich ist, ist für $\phi\in A$ die Abbildung $\phi$ injektiv und, da $L\sslash K$ endlich ist, bijektiv.
	
	Falls $L\sslash K$ nicht endlich, aber algebraisch ist, betrachten wir $a\in L$ und das zugehörige Minimalpolynom $m_a(t)\in K[t]$. Dann ist $\phi(a)$ Nullstelle von $m_a(t)$ für alle $\phi\in A$. Sei \[L' = K(\{\text{Nullstelle von }m_a(t)\text{ in }L\})\subseteq L.\]
	Es gilt $a\in L'$, $\phi(a)\in L'$ für alle $\phi \in A$, $\phi\colon L'\to L'$ und $\phi(L') = L'$, weil $\phi$ Nullstellen permutiert.
	Folglich existiert ein Urbild von $a$ unter $\phi$; damit ist $\phi\in A$ surjektiv und automatisch auch injektiv. Also ist $\phi$ bijektiv und $\phi\in \Gal(L\sslash K)$.
\end{proof}

\begin{defi}
	Sei $G$ eine Gruppe, $X$ eine Menge und $G\operates X$ eine Operation von $G$ auf $X$. Diese heißt
	\begin{enumerate}
		\item treu, falls $(\forall x\in X: g.x = x)\Rightarrow g = e$ gilt.
		\item transitiv, falls $\forall x,y\in X:\exists g\in G : g.x = y$ gilt.
	\end{enumerate}
\end{defi}	
\begin{satz} \label{thm:grupOpTreuTransGaloisGrup}
	 Sei $K$ Körper und $f\in K[t]$ irreduzibel. Sei $L$ Zerfällungskörper von $f$ über $K$. Dann operiert $G := \Gal(L\sslash K)$ auf $X := \{\text{Nullstellen von }f\text{ in }L\}$ treu und transitiv durch $\phi.x = \phi(x)$ für $\phi\in G, x\in X$.
\end{satz}	
\begin{proof}
	\leavevmode
	\begin{description}
	\item[Wohldefiniertheit der Operation:] Für $y\in X$ gilt $\phi(y)\in X$ für alle $\phi \in G$, also ist $\phi.y$ wohldefiniert. Seien $\phi_1,\phi_2\in G$ sowie $x\in X$ beliebig. Dann gilt sowohl $(\phi_1\circ\phi_2).x = (\phi_1\circ\phi_2)(x) = \phi_1(\phi_2(x)) = \phi_1.(\phi_2.x)$ als auch $e.x = \id_L.x = \id_L(x) = x$; $G$ operiert also tatsächlich auf $X$.
	\item[treu:] Sei $\phi\in G$. Da $L = K(a_1,\dots, a_n)$ mit $a_i$ als den Nullstellen von $f$ gilt, folgt aus $\forall x\in X:\phi.x = x$ dann $\forall 1\le i\le n:\phi(a_i) = a_i$. Da aber $\phi\in G$ durch $\phi(a_i)$ für $1\leq i\leq n$ bereits eindeutig bestimmt ist, ist also $\phi = \id_L = e$.
	\item[transitiv:] Seien $x,y\in X$. Dann existiert genau ein $K$-Homomorphismus $\phi\colon K(x)\to\overline{K}$ mit $\phi(x) = y$. Nach dem Fortsetzungssatz existiert genau ein $K$-Homomorphismus $\hat{\phi}\colon L\to\overline{K}$ mit $\hat{\phi}|_{K(x)}  =\phi$. Da $L$ ein Zerfällungskörper, also $L \sslash K$ normal ist, ist $\hat{\phi}(L) = L$ und somit existiert genau ein $K$-Homomorphismus $\phi\colon L\to L$ mit $\phi(x) =y$. Daraus folgt $\phi.x = y$.
	\end{description}
\end{proof}


\begin{bem}
	Hat $f$ genau $n$ Nullstellen (in $L$), so gilt 
	\[\Gal(L\sslash K) < S_n\cong\{\text{Permutationen der Nullstellen von }f\}.\]
\end{bem}
...
\begin{defi}
	...
	In diesem Fall gilt nach \cref{lem:20.1}
	\[\Gal (L\sslash K) = \Aut (L\sslash K) =\{\phi\colon L\to L\mid \text{$\phi$ $K$-Homomorphismus}\}.\]
\end{defi}

\begin{satz}[Galoiserweiterungen]\label{thm:21.1}
	Sei $L\sslash K$ endlich. Dann sind äquivalent:
	\begin{enumerate}
		\item $L\sslash K$ ist galois.
		\item $[L:K] = |\Gal(L\sslash K)|$
		\item $K = L^{\Gal(L\sslash K)} := \{x\in L\mid \forall \phi\in \Gal(L\sslash K): \phi(x) = x\}$
		\item Für alle $a\in L$ ist \[m_a(t) = \prod_{\mathclap{b\in \Gal(L\sslash K).a}}(t-b).\]
	\end{enumerate}
\end{satz}


\lecture{18. Januar 2018}
\begin{proof}
	Wir setzen $G := \Gal(L\sslash K)$.
	\begin{description}
		\item[$1\Rightarrow2$:] Sei $L\sslash K$ galois, also insbesondere separabel. Dann folgt nach der \namereff{lem:separabel}: $[L:K]_{\sep} = [L:K]$. Deshalb gilt mit der Definition des Separabilitätsgrad und aufgrund der Normalität von $L \sslash K$ bereits
		\begin{align*}
			[L:K] &= [L:K]_{\sep} = |\{\phi\colon L\to \overline{K}\mid \text{$\phi$ $K$-Homomorphismus}\}| \\
			&= |\{\phi\colon L\to L\mid \text{$\phi$ $K$-Homomorphismus}\}| \\
			&= |\Gal(L\sslash K)|.
		\end{align*}
		\item[$2\Rightarrow 3$:] Offensichtlich ist $K\subseteq L^G\subseteq L$ ein Zwischenkörper. Dabei gilt nun die Ungleichungskette \begin{align*}[L:L^G] &\ge |\{\phi\colon L\to L\mid \text{$\phi$ $L^G$-Homomorphismus}\}| = |\Gal(L\sslash L^G)|\\&\ge |G| = [L:K]\ge [L:L^G],\end{align*} wobei die erste Ungleichung dem aufmerksamen Leser als Übung überlassen bleibt. Da nun überall Gleichheit folgt, gilt $[L:L^G] = [L:K]$ und somit $K = L^G$.
		\item[$3\Rightarrow 4$:] Sei $a\in L$. Dann ist $a$ Nullstelle des Minimalpolynoms $m_a(t)\in K[t]$ von $a$, und damit auch $\phi(a)$ für alle $\phi\in G$. Also teilt
		\[f(t) := \prod_{\mathclap{b\in G.a = \{\phi(a)\mid\phi\in G\}}}(t-b)\]
	 	dann $m_a(t)\in L[t]$. Sei $f(t) = \sum_{i\ge 0}c_it^i$. Für $\phi\in G$ definieren wir $\phi_*(f) := \sum_{i\ge 0} \phi(c_i)t^i$. Andererseits ist $\phi_*(f) = \prod_{b\in G.a}(t-\phi(b)) = \prod_{b\in G.a}(t-b) = f$. Also muss $\phi(c_i) = c_i$ für alle $i$ gelten (und zwar für alle $\phi\in G$). Damit ist $c_i\in L^G$ für alle $i$; nach Voraussetzung gilt $f\in K[t]$; $m_a(t)$ teilt folglich $f(t)$. Insgesamt gilt daher $m_a(t) = f(t)$.
		...
		Sei nun $f\in K[t]$ irreduzibel. Sei $f(a) = 0$ für ein $a\in L$. Da $f$ irreduzibel ist und $f(a)=0$, ist $f = \epsilon m_a(t)$ mit $\epsilon\in K^{\times}$. Da $m_a(t)$ über $L$ in Linearfaktoren zerfällt, zerfällt auch $f$; $L\sslash K$  ist also normal. Damit ist $L\sslash K$ galois.
		\qedhere
	\end{description}

\end{proof}

\begin{bsp}[Berechnung der Galoisgruppe]
	Sei $L$ der Zerfällungskörper von $f(t) = t^3-2\in \IQ[t]$. Sei $K\coloneqq \IQ$. $f$ hat 3 Nullstellen in $L$, nämlich $\sqrt[3]2, \sqrt[3]2\zeta, \sqrt[3]2\zeta^2$, wobei $\zeta = e^{\frac{2\pi i}3}$. Man beachte $\IQ[\sqrt[3]2]\subseteq \IR$. Nach \cref{thm:grupOpTreuTransGaloisGrup} ist $\Gal(L\sslash K)\subseteq S_3$. Nun betrachte den surjektiven Ringhomomorphismus 
	\begin{eqnarray*}
		\ev := \ev_{\sqrt[3]2} \colon \IQ[t] &\longto & \IQ[\sqrt[3]2]\\
		t &\longmapsto &\sqrt[3]2\\
		g(t) &\longmapsto &g(\sqrt[3]2)
	\end{eqnarray*}
	Er induziert einen Ringhomomorphismus $\overline{\ev}\colon \IQ[t]/(t^3-2)\to \IQ(\sqrt[3]2)$ nach Homomorphiesazt, welcher nachwievor surjektiv ist. Da $t^3-2$ irreduzibel ist, ist $\IQ[t]/(t^3-2)$ ein Körper. Damit ist $\overline{\ev}$ injektiv, also bijektiv und damit ein Körperisomorphismus. Folglich ist $[\IQ(\sqrt[3]2):\IQ] = \deg m_{\sqrt[3]2}(t) = 3$. Aber $L$ enthält echte komplexe Zahlen, also $[L:\IQ(\sqrt[3]2)]>1$. Das heißt, dass $[L:\IQ] = [L:\IQ(\sqrt[3]2)][\IQ(\sqrt[3]2):\IQ]\geq 2\cdot 3 = 6$. Da $L$ Zerfällungskörper von $f$ über $K$ ist, ist $L\sslash K$ also normal und separabel, da $\chr\IQ= 0$ (und $L\sslash K$ algebraisch) ist. Insgesamt ist $L\sslash K$ galois, und mit \cref{thm:21.1} folgt $6\leq [L:K] = |\Gal(L\sslash K)| \leq 6$, also $|\Gal(L \sslash K)| = 6$ und damit $\Gal(L\sslash K) \cong S_3$.
	
\end{bsp}

\subsection{Galoiskorrespondenz}
\begin{satz}[Galoiskorrespondenz]\label{thm:galoiskorrespondenz}
	Sei $L\sslash K$ eine endliche Galoiserweiterung. Wir setzen $G \coloneqq \Gal(L\sslash K)$. Dann existiert eine Bijektion
	\begin{eqnarray*}
		\Phi\colon \{\text{Untergruppen }H<G\}&\xlongleftrightarrow{1:1}&\{\text{Zwischenkörper }K\subseteq M\subseteq L\}\\
		H &\longmapsto & L^H := \{x\in L\mid\forall \phi\in H\phi(x) = x\}\\
		\{g\in G\mid\forall m\in M: g(m) = m\} =: H_M &\longmapsfrom& M.
	\end{eqnarray*}
Dabei entsprechen normale Untergruppen $N\nt G$ genau den Zwischenkörpern $K\subseteq M\subseteq L$ mit $M\sslash K$ normal. In diesem Fall gilt dann $\Gal(M\sslash K) \cong G/N$.
\end{satz}
\begin{bem}
	$\Phi$ ist verträglich mit Inklusion in folgender Weise: Sind $H_1, H_2<G$, so gilt \[H_1\subseteq H_2 \quad\Longleftrightarrow\quad \Phi(H_1)\supseteq \Phi(H_2).\]
\end{bem}
...
\begin{satz}\label{thm:22.2}
	Sei $L$ ein Körper sowie $G$ eine endliche Gruppe mit $G\subseteq \{\phi\colon L\to L\mid\text{$\phi$ Körperisomorphismus}\}$. Dann ist $L\sslash L^G$ galois und es gilt $\Gal(L\sslash L^G) = G$.
\end{satz}
\begin{proof}
	Sei $K := L^G$ und $n = |G|$. Sei nun $a\in L$. Dann ist $X := \{\phi(a)|\phi\in G\}$ eine endliche Menge, da $|G|$ endlich ist. Betrachte $f := f_a := \prod_{b\in X}(t-b)$. Jedes $\phi\in G$ definiert eine injektive Abbildung $\phi\colon X\to X$, welche aufgrund der Endlichkeit von $X$ schon bijektiv ist. Weiterhin sei $\phi_*(f) := \prod_{b\in X} (t-\phi(b)) =\prod_{b\in X}(t-b) = f$, weshalb $f\in L^G[t] = K[t]$ gilt. Somit ist $a$ Nullstelle von $f$, $f$ separabel (über $K$) und es liegen alle Nullstellen von $f$ in $L$; $L$ ist also Zerfällungskörper von allen $f_a$ mit $a\in L$.
	
	Nach der \namereff{thm:normalitaet} bzw. der nachfolgenden Bemerkung ist $L\sslash K$ normal und nach Konstruktion separabel. Also ist $L\sslash K = L\sslash L^G$ galois.
	
	Es bleibt $G = \Gal(L\sslash L^G)$ zu zeigen. Dabei ist die Inklusion $G \subseteq \Gal(L \sslash L^G)$ trivial. Weiterhin wissen wir, dass $[K(a): K] \le n$ ist, da $f_a$ von $m_a(t)$ geteilt wird und somit $[K(a) : K] = \deg m_a(t) \ge \deg f_a \ge \abs G = n$ gilt. Andererseits haben wir $n = |G|\le |\Gal(L\sslash L^G)| = [L:L^G] = [L:K]\leq n$. Folglich gilt überall Gleichheit und $|G| = |\Gal(L\sslash K)|$, woraus $G = \Gal(L\sslash K)$ folgt.
\end{proof}

Nun folgt der
\begin{proof}[Beweis der \namereff{galoiskorrespondenz}]
Sei zunächst $G\coloneqq \Gal(L\sslash K)$. Der aufmerksame Leser prüft geschwind nach, dass für eine Galoiserweiterung $L\sslash K$ die Körpererweiterungen $L\sslash M$ galois für alle Zwischenkörper $K\subseteq M\subseteq L$ sind. Sei $\Psi \colon M\mapsto H_M$ wie im Satz definiert.
\begin{description}
	\item[$\Phi\circ\Psi = \id$:] Sei $M$ ein Zwischenkörper von $K$ und $L$. Dann gilt $\Phi(\Psi(M)) = \Phi(H_M) = \Phi(\{g\in G\mid \forall m\in M:g(m) = m\}) = \Phi(\Gal(L\sslash M)) = L^{\Gal(L\sslash M)} = M$ nach \cref{thm:22.2}, weil $|G|$ und damit auch $|\Gal(L\sslash M)|$ endlich ist.
	\item[$\Psi\circ\Phi = \id$:] Sei $H<G$. Dann gilt $\Psi(\Phi(H)) = \Psi(L^H) =H_{L^H}$ mit $H_{L^H} = \{g\in G\mid \forall x\in L^H: g(x) = x\} = \Gal(L\sslash L^H) = H$ nach \cref{thm:22.2}.
\end{description}
Also ist $\Phi$ tatsächlich eine Bijektion.

Es bleibt die Korrespondenz zwischen normalen Untergruppen und normalen Körpererweiterungen zu zeigen. Dazu beweisen wir zunächst, dass für alle $g\in \Gal(L\sslash K)$ dann $g(L^H) =L^{gHg^{-1}}$ für $H<G$ gilt.
\begin{description}
	\item[\glqq$\subseteq$\grqq:] Sei $x\in g(L^H)$. Dann existiert ein $y \in L^H$ mit $x = g(y)$. Für alle $\phi \in H$ gilt $\phi(y)=y$, weshalb $(g \circ \phi \circ g^{-1})(x) = g(\phi(g^{-1}(x))) = g(\phi(y)) = g(y) = x$ für alle $\phi \in H$ gilt. Das bedeutet aber gerade $x \in L^{gHg^{-1}}$.
	\item[\glqq$\supseteq$\grqq:] Sei $x$ in $L^{gHg^{-1}}$; es gilt also $(g\circ\phi\circ g^{-1})(x) = x$ für alle $\phi\in H$, und damit $g^{-1}(x) = \phi(g^{-1}(x))$ für alle $\phi\in H$. Setzt man $y := g^{-1}(x) \in L^H$, so ist $x = g(y)\in g(L^H)$. 
\end{description}
Sei $H<G$. Dann ist $L^H\sslash K$ genau dann normal, wenn für alle $a\in L^H$ die Nullstellen von $m_a(t)$ in $K[t]$ enthalten sind. Das ist genau dann der Fall, wenn $g(a) \in L^H$ für alle $a \in L^H$ und $g \in G$ gilt, weil $G$ transitiv auf den Nullstellen operiert. Äquivalent dazu ist $g(L^H) = L^H$ für alle $g\in G$, weil $[L:K]$ endlich ist. Nach der Vorbemerkung wissen wir nun aber, dass genau dann $ L^H = L^{gHg^{-1}}$, also $H = gHg^{-1}$ für alle $g\in G$ gilt, da $\Phi$ bijektiv ist. Das bedeutet aber gerade, dass $H\nt G$ ein Normalteiler ist.

Schließlich zeigen wir $\Gal(L^H\sslash K) \cong G/H$ für $H\nt G$. Da $L\sslash K$ nach Voraussetzung separabel ist, ist insbesondere $L^H\sslash K$ separabel. Da $H\nt G$ gilt, folgt nach dem vorherigen Absatz $g(L^H) = L^H$ für alle $g\in G$, wir erhalten also eine wohldefinierte Abbildung
\begin{eqnarray*}
	\res\colon G &\longto & \Gal(L^H\sslash K)\\
	g & \longmapsto & g|_{L^H}.
\end{eqnarray*}
$\res$ ist offensichtlich ein Gruppenhomomorphismus mit \[\ker{\res} = \{g\in \Gal(L\sslash K)\mid g|_{L^H} = \id_{L^H}\} = \Psi(L^H) = H.\] Wir erhalten also einen induzierten Gruppenhomomorphismus $\overline{\res}\colon G/H\to \Gal(L^H\sslash K)$, der nach Konstruktion injektiv ist. Außerdem ist $|\Gal(L^H\sslash K)| = [L^H:K]$ nach \cref{thm:21.1} und $[L:K] = [L:L^H][L^H:K]$, wobei $[L:K] = |G|$ nach \cref{thm:21.1} und $[L:L^H] = |H|$ nach \cref{thm:22.2}; folglich muss $[L^H:K] = |G/H|$ gelten. Also ist $\overline{\res}$ ein Gruppenisomorphismus zwischen $G/H$ und $\Gal(L^H \sslash K)$.
\end{proof}

\lecture{22. Januar 2018}

\begin{bsp}
	\leavevmode
	\begin{itemize}
	\item Sei $\IC/\IR$ eine Galoiserweiterung vom Grad $[\IC:\IR] = 2$. Dann gilt $G = \Gal(\IC/\IR)$. Nach \cref{thm:21.1} ist $|G| = 2\Rightarrow G\cong \IZ/2\IZ$. 
...
Wir wollen nun die Galoiskorrespondenz verwenden, um den \namereff{thm:fundamentalsatz} mithilfe des Zwischenwertsatzes aus der Analysis zu beweisen.
\begin{satz}[Fundamentalsatz der Algebra]\label{thm:fundamentalsatz}
	$\IC$ ist algebraisch abgeschlossen.
\end{satz}
\begin{proof}
	Sei $L\sslash \IC$ endliche algebraische Körpererweiterung. Es ist $L = \IC$ zu zeigen. Da $L\sslash \IC$ endlich ist, ist $L\sslash \IR$ ebenfalls endlich. Weil $\chr \IR= 0$ ist, ist $L\sslash \IR$ separabel. Nach \cref{thm:existenz normaler oberkoerper} können wir $L\sslash \IR$ als normal voraussetzen. Damit ist $L\sslash \IR$ eine Galoiserweiterung.
	
	Sei $G:= \Gal(L\sslash \IR)$ und $S\subseteq G$ eine $2$-Sylowgruppe. $L^S\sslash R$ ist eine endliche, algebraische Körpererweiterung. Außerdem gilt $[L:\IR] = [L:L^S][L^S:\IR]$, wobei $[L:\IR] = |G|$ nach \cref{thm:21.1} und $[L:L^S] = |S|$ nach \cref{thm:22.2}, da $S\subseteq G$ eine Untergruppe der endlichen Gruppe $G$ ist. Nach \cref{thm:22.2} ist $L\sslash L^S$ außerdem galois. Nun ist $S$ aber eine $2$-Sylowgrppe; folglich ist $[L^S:\IR]$ ungerade. Nach dem \namereff{thm:primitives element} folgt $L^S = \IR(a)$ mit $\deg(m_a(t))$ ungerade ($m_a(t)\in\IR[t]$). Da $m_a(t)$ ungeraden Grad hat, hat $m_a(t)$ nach dem Zwischenwertsatz mindestens eine Nullstelle in $\IR$. Daraus folgt $\deg(m_a(t)) = 1$, weil $m_a(t)\in\IR[t]$ irreduzibel ist. Damit ist $L^S =\IR$ und $G =S$ insbesondere $2$-Gruppe. Da $G$ nun eine $2$-Gruppe ist, existiert nach \cref{thm:p-gruppe aufloesbar} eine Normalreihe $\{e\} = G_0\subseteq G_1\subseteq G_2\subseteq\dots\subseteq G_r = G = \Gal(L\sslash \IR)$ von Untergruppen mit $r \in \IN$, sodass $G_i/G_{i-1} \cong \IZ/2\IZ$ für alle $1 \le i \le r$ gilt.
	
	Mit der \namereff{galoiskorrespondenz} erhalten wir eine Sequenz von Körpererweiterungen $L = L^{\{e\}}\supseteq L^{G_1}\supseteq L^{G_2}\supseteq\dots \supseteq L^{G_r} = L^G = \IR$, wobei die letzte Gleichheit aus \cref{thm:21.1} folgt. Setze $L^{G_i} = L_i$. Damit ist $L_i\sslash L_{i+1}$ galois mit der Galoisgruppe $G_{i+1}/G_i$ nach der Korrespondenz von Normalteilern. Insgesamt erhalten wir $L$ also aus $\IR$ sukzessive durch endlich viele Körpererweiterungen von Grad $2$.
	
	Aber jede Körpererweiterung vom Grad $2$ ist durch Adjunktion eines Elementes $a$ mit $\deg(m_a(t)) = 2$ gegeben. Wir wissen, dass jedes quadratische Polynom in $\IC[t]$ eine Nullstelle (sogar $2$) in $\IC$ hat. Folglich kann $\IC$ nicht durch quadratische Erweiterungen echt vergrößert werden. Damit ist $L = \IC$ (starte mit $\IR\subseteq \IC = L^{G_{r-1}}$).
\end{proof}


\subsection{Zyklotomische Körper}
Sei $n \in \IZ_{>0}$ und $K$ ein Körper. Betrachte $f = t^n-1\in K[t]$. Frage: Was ist der Zerfällungskörper $L$ von $f$?

Im Fall $K = \IC$ sind die Nullstellen von $f$ genau die $n$-ten Einheitswurzeln in $\IC$. Für $K = \IR$ gilt also $L = \IR(\{\zeta\mid\text{$\zeta$ $n$-te Einheitswurzel}\})\subseteq \overline{\IR} = \IC$. Der Fall $K= \IQ$ funktioniert genauso. Was passiert für Körper $K$ mit $\chr K = p > 0$?
...
\begin{equation}
\chr K = 0\text{ oder }(\chr K = p\text{ und }p\nmid n) \label{eq:zyklo} \tag{*}
\end{equation}
\begin{description}
	\item[1. Behauptung:\label{beh:zyklo:1}] $f = t^n-1\in K[t]$ mit \eqref{eq:zyklo} ist separabel.
	\begin{proof}
		Der Fall $\chr K = 0$ ist trivial. Sei also $\chr K = p$ mit $p \nmid n$. Es gilt $f' = nt^{n-1}\in K[t]$ hat mit $(2)$ nur $0$ als Nullstelle; $f$ somit hat keine mehrfachen Nullstellen.
	\end{proof}
\end{description}
	Sei nun $Z = \{z_1,\dots, z_n\}$ die Menge der Nullstellen von $f\in K[t]$ in $\overline{K}$.
\begin{description}
	\item[2. Behauptung:\label{beh:zyklo:2}] $Z\subseteq \overline{K}^{\times}$ ist eine zyklische Untergrupe.
	\begin{proof}
		Offensichtlich ist $e = 1 \in Z$. Sind $z_i, z_j\in Z$, so folgt $(z_iz_j)^n-1 = z_i^nz_j^n-1 = 1\cdot 1-1 = 0$, also ist $z_iz_j\in Z$; und es gilt $(z_i^{-1})^n -1 = (z_i^n)^{-1}-1 = 1-1 = 0$; folglich ist auch $z_i^{-1}\in Z$. Damit ist $Z$ eine Untergruppe von $\overline{K}^{\times}$. Da $Z<\overline{K}^{\times}$ endlich ist, ist $Z$ nach \cref{thm:endlKoeZyklMult} zyklisch.
	\end{proof}
\end{description}
Folglich gilt $Z \cong \IZ/n\IZ$ als Gruppe, weil nach der 2. Behauptung $|Z| = n$ ist. Damit hat $Z$ genau $\phi(n)$ verschiedene Erzeuger, da $\phi(n) = |\{\overline{m}\in \IZ/n\IZ\mid\ggT (m,n) = 1\}|$ ist. Wir nennen diese Erzeuger von $Z$ die primitive $n$-ten Einheitswurzeln über $K$ (analog zum Fall $K = \IC$). Wähle nun eine solche $n$-te Einheitswurzel $\xi_n$.
\begin{lem}
	 Es gelte \eqref{eq:zyklo}. Der Zerfällungskörper von $f = t^n-1\in K[t]$ ist $K(\xi_n)$.
\end{lem}
\begin{proof}
	Sei $\xi  := \xi_n$. Es gilt $\xi^n = 1$ per Definition. Die $\xi^i$ sind für $1\leq i \leq n$ paarweise verschieden, $\xi^i = \xi^j$ mit $1\leq j<i\leq n$ in dem Widerspruch $\xi^{i-j} = 1$ resultiert, da $i-j<n$, aber $\xi$ eine primitive $n$-te Einheitswurzel ist.
	
	Außerdem gilt $(\xi^i)^n-1 = (\xi^n)^i-1 = 1-1 = 0$; $\xi^i$ ist also eine Nullstelle von $f$. $K(\xi)$ ist also Zerfällungskörper von $f$.
\end{proof}
\begin{defi}
	Es gelte \eqref{eq:zyklo}. Der Zerfällungskörper von $f = t^n-1\in K[t]$ heißt zyklotomischer Körper der Ordnung $n$ über $K$.
\end{defi}
\begin{lem}\label{lem:24.2}
	Es gelte \eqref{eq:zyklo}. Sei $f = t^n-1\in K[t]$. Sei $L$ Zerfällungskörper von $f$. Dann ist $G := \Gal(L\sslash K)$ abelsch und $|G|$ teilt $\phi(n)$.
\end{lem}
\begin{proof}
	Wir haben einen Gruppenhomomorphismus
	\begin{eqnarray*}
		\phi\colon G & \longto & \Aut(Z) =\{\phi\colon Z\to Z \mid \text{$\phi$ Gruppenisomorphismus}\}\\
		g & \longmapsto & g|_Z
	\end{eqnarray*}
	wobei $Z$ so wie in der 2. Behauptung oben definiert ist.
	
	Dieser Gruppenhomomorphismus ist injektiv, weil $g$ bereits durch $g|_Z$ eindeutig bestimmt ist, da $L$ der Zerfällungskörper von $f$ ist. Da $Z = \IZ/n\IZ$ (siehe 2. Behauptung), ist $\Aut(Z) \cong \Aut(\IZ/n\IZ)\cong(\IZ/n\IZ)^{\times}$. Folglich ist $\Aut (Z)$ und $\im\phi<\Aut(Z)$ abelsch; damit ist auch $G$ abelsch. Außerdem teilt $|G| = |\Gal(L\sslash K)|$ dann $|\Aut(\IZ/n\IZ)|$ nach dem \namereff{thm:lagrange}, und es gilt $|\Aut(\IZ/n\IZ)| = |(\IZ/n\IZ)^{\times}| = \phi(n)$.
	%falls irgendjemand noch das Übungsblatt findet, auf dem Aut(Z/nZ) = (Z/nZ)^\times bewiesen wird, bitte einfügen
\end{proof}
\begin{bsp}
	\leavevmode
	\begin{itemize}
	\item Seien $K = \IF_2$ und $f = t^3-1 = (t-1)(t^2+t+1)\in\IF_2[t]$, wobei $(t^2+t+1)$ irreduzibel in $\IF_2[t]$ ist. Jede Nullstelle $a\neq 1$ von $f$ in $\overline{\IF_2}$ ist eine primitive dritte Einheitswurzel. Dann ist $[\IF_2(a):\IF_2] = 2$.
	\item $f = t^7-1 = (t-1)(t^3+t^2+1)(t^3+t+1)$. Es existieren $6$ primitive $7$-te Einheitswurzeln; $(t^3+t^2+1)$ und $(t^3+t+1)$ treten als Minimalpolynome davon auf; im Gegensatz zu $K = \IQ$, wo jede primitive $n$-te Einheitswurzel dasselbe Minimalpolynom hat (siehe \cref{thm:24.4})
	\end{itemize}
\end{bsp}


\lecture{25. Januar 2018}
\begin{lem}\label{lem:24.3}%lohnt sich zu merken
	Sei $\xi$ eine primitive $n$-te Einheitwurzel über $K$ sowie $m\in\IZ$. Dann gilt \[\text{$\xi^m$ primitiv} \quad\Longleftrightarrow \quad \ggT(m,n) = 1.\]
\end{lem}

\begin{proof}
	Sei $f = t^n-1\in K[t]$ sowie $Z$ die Menge der Nullstellen von $f$ in $\overline{K}$. $Z$ ist eine zyklische Gruppe bezüglich Multiplikation; Erzeuger sind die primitiven $n$-ten Einheitswurzeln.
	...
	\end{eqnarray*}
	Nun ist $\xi^m$ primitiv genau dann, wenn $\ord(\xi^m) = n$, also wenn $\ord(\overline{m}) = n$ in $\IZ/n\IZ$, was äquivalent zu $\ggT(m,n) = 1$ ist.
\end{proof}
\begin{satz}\label{thm:24.4}
	Sei $\xi\in\overline{\IQ}$ eine primitive $n$-te Einheitswurzel \textup(über $\IQ$\textup). Dann ist $\IQ(\xi)\sslash\IQ$ galois und $[\IQ(\xi):\IQ] = \phi(n)$.
\end{satz}
\begin{proof}
	Nach \cref{lem:24.2} wissen wir, dass $\IQ(\xi)\sslash\IQ$ galois ist und $[\IQ(\xi):\IQ]\leq \phi(n)$ gilt. Es bleibt also zu zeigen,dass für das Minimalpolynom $f = m_{\xi}(t)\in\IQ[t]$ von $\xi$ dann $\deg(f) = \phi(n)$ gilt.
	
	Dafür reicht es nach \cref{lem:24.3} zu zeigen, dass jede primitive $n$-te Einheitswurzel eine Nullstelle von $f$ (über $\overline{\IQ}$) ist. Es ist nach der Definition des Minimalpolynoms klar, dass $f \mid t^n-1$ gilt. Folglich gilt $t^n-1 = f\cdot h$ in $\IQ[t]$ mit normiertem $h\in \IQ[t]$. Aus \cref{lem:24.5} folgt $f,h\in\IZ[t]$, da $f$ und $h$ normiert sind.
	
	Sei nun $p$ prim mit $p\nmid n$. Wir behaupten, dass $f(\xi^p) = 0$ gilt. Ist das nicht der Fall, so folgt $h(\xi^p) = 0$, weil $(\xi^p)^n-1 = 0$ ist. Dann ist $\xi$ eine Nullstelle von $h(t^p)$. Also teilt $f$ schon $h(t^p)$ aufgrund der Definition des Minimalpolynoms und der Normiertheit von $h$. Damit folgt $h(t^p) = f\cdot g$ für ein normiertes $g\in\IQ[t]$. Wieder mit \cref{lem:24.5} folgt $f,g\in\IZ[t]$. Betrachte den Ringhomomorphismus
	\begin{eqnarray*}
		\IZ[t] & \longto & \IF_p[t] = \IZ[t]/p\IZ[t]\\
		\sum_{i\geq0}c_it^i & \longmapsto & \sum_{i\geq 0} \overline{c_i}t^i.
	\end{eqnarray*}
	Damit folgt $\overline{h(t^p)} = \overline{f}\overline{g}$, wobei $\overline{h(t^p)} = \overline{h(t)}^p$ gilt. Folglich haben $\overline{f(t)}$ und $\overline{h(t)}$ eine gemeinsame Nullstelle, sind also nicht teilerfremd. Damit hat $t^n-1 = \overline{t^n-1} = \overline{f}\overline{h}$ eine mehrfache Nullstelle im Widerspruch zu $p\nmid n$. Es folgt sofort $f(\xi^p) = 0$.
	
	Sei nun $\xi'$ eine beliebige primitive $n$-te Einheitswurzel. Es ist $f(\xi') = 0$ zu zeigen. Sei $\xi' = \xi^m$ ($m\in \IN$, $\ggT(m,n) =1$ nach \cref{lem:24.3}). Wir erhalten die Darstellung $\xi'  =\xi^{p_1\dots p_r}$ mit $p_1\dots p_r = m$ und primen $p_i$ mit $p_i\nmid n$ für alle $i$. Wende nun obiges Argument wiederholt an und erhalte $f(\xi') = f(\xi^{p_1\dots p_r}) = 0$.
\end{proof}

\begin{lem}\label{lem:24.5}
	Seien $\beta, \gamma\in\IQ[t]$ und $\alpha\in\IZ[t]$ mit $\alpha = \beta\gamma$, wobei $\beta$ und $\gamma$ normiert sind. Dann folgt bereits $\beta, \gamma\in\IZ[t]$.
\end{lem}
\begin{proof}
	Seien
	...
	Damit gilt also $ab\alpha = \tilde{\beta}\tilde{\gamma}$. Wir zeigen nun $a = 1-b$. Sei dafür $p$ prim mit $p\mid ab$. Nach Reduktion modulo $p$ ergibt sich $0 = \overline{\beta}\overline{\gamma}$ in $\IF_p[t]$, also $\overline{\tilde{\beta}} = 0$ oder $\overline{\tilde{\gamma}} = 0$.
	
	Es folgt $\ggT(a_1,\dots,a_m)>1$ oder $\ggT(b_1,\dots, b_n)>1$, wobei $a_m = a$, $b_n = b$ nach Voraussetzung. Das ist ein Widerspruch zu obiger Annahme. Folglich existiert kein $p$ mit $p\mid ab$, also $ab = 1$ und $a = \frac 1b$, woraus die Aussage mit obiger Darstellung folgt.
\end{proof}

\begin{defi}
	Sei $K$ ein Körper, $n\in\IZ_{>0}$ und $\chr(K)\nmid n$. Dann sei
...
\begin{satz}\label{thm:24.6}
	Es gelte \eqref{eq:zyklo}. Dann gilt:
	\begin{enumerate}
		\item $\Phi_n(t)\in K[t]$ ist normiert, separabel und vom Grad $\phi(n)$. \label{enumi:24.6:1}
		\item Für $K = \IQ$ ist $\Phi_n(t)\in\IZ[t]$ und irreduzibel in $\IQ[t]$ und $\IZ[t]$.
		\item $t^n-1 = \prod_{d|n}\Phi_d(t)$\label{enumi:24.6:3}
...
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Normiertheit, Separabilität und $\deg \Phi_n(t)= \phi(n)$ sind klar.
		
		Es bleibt $\Phi_k(t)\in K[t]$ zu zeigen. Sei $L = K(\xi_1,\dots, \xi_{\phi(n)}) = K(\xi_1)$ nach \cref{lem:24.2}. Dann ist $\Phi_n(t)\in L[t]$. Jedes $\phi\in \Gal(L\sslash K)$ bildet primitive $n$-te Einheitswurzeln auf ebensolche ab und permutiert somit die primitiven Einheitswurzeln. Also gilt $\phi_*(\Phi_n(t)) = \Phi_n(t)$ für alle $\phi\in \Gal(L\sslash K)$. Daraus folgt $\Phi_n(t)\in L^{\Gal(L\sslash K)}[t] = K[t]$ nach \cref{thm:21.1}, da $L\sslash K$ galois ist.
		
		\item Sei $K  = \IQ$. Offensichtlich gilt $m_{\xi}(t)\in \IQ[t]$ und $m_{\xi}(t)\mid \Phi_n(t)$ für jede primitive $n$-te Einheitswurzel $\xi$ nach der Definition des Minimalpolynoms.
		
		Mit \cref{thm:24.4} folgt $\deg(m_\xi(t)) = [\IQ(\xi):\IQ] = \phi(n)$, nach Punkt \ref{enumi:24.6:1} dieses Satzes gilt also $m_{\xi}(t) = \Phi_n(t)$, da $\Phi_n(t)$ normiert ist. Daher ist $\Phi_n(t)$ irreduzibel in $\IQ[t]$, da $m_{\xi}(t)$ irreduzibel ist.
		
		Außerdem teilt $\Phi_n(t)$ das Polynom $t^n-1$, weil $\Phi_n(t) = m_{\xi}(t)$ ist. Wir erhalten $t^n-1 = \Phi_n(t)\cdot h(t)\in \IQ[t]$ mit normierten Polynomen $h(t), \Phi_n(t)\in\IQ[t]$. Aus \cref{lem:24.5} folgt nun $\Phi_n(t), h(t)\in\IZ[t]$ und damit die Irreduzibilität von $\Phi_n(t)\in\IZ[t]$, weil $\Phi_n(t)$ in $\IQ[t]$ irreduzibel und primitiv ist. 
		\item Sei $\mathcal{P}_d$ die Menge der primitiven $d$-ten Einheitswurzeln. Dann ist $Z = \bigcup\limits_{d|n}\mathcal{P}_d$. Also gilt \[t^n-1 = \prod_{d|n}\prod_{\xi\in\mathcal{P}_d}(t-\xi) = \prod_{d|n}\Phi(t).\]
		\item Es ist zu zeigen, dass die kanonische Abbildung
		\begin{eqnarray*}
			\alpha\colon \IZ[t] & \longto & K[t]\\
			\sum_{i \geq 0}b_it^i & \longmapsto & \sum_{i\geq 0}\can(b_i)t^i
		\end{eqnarray*}
		(mit dem Ringhomomorphismus $\can\colon \IZ\to K, 1\mapsto 1$) $\Phi_{n,\IQ}(t)$ auf $\Phi_{n, K}(t)$ abbildet, wobei $\Phi_{n, \IQ}(t)$ in $\IZ[t]$ liegt. Beweis per Induktion:
		\begin{description}
			\item[$n = 1$:] Offenbar ist $\IZ[t]\ni t-1 \mapsto t-1 \in K[t]$ erfüllt.
			\item[$n>1$:] Nach Induktionsannahme sowie Teil \ref{enumi:24.6:3} des Satzes folgt
			\begin{align*}
				t^n-1 & =\alpha(t^n-1) = \alpha(\Phi_{n,\IQ}(t))\prod_{\substack{d\mid n,\\d<n}}\Phi_{d, K}(t), \\\intertext{andererseits gilt}
				t^n-1 & = \Phi_{n,K}(t)\prod_{\substack{d\mid n,\\ d<n}}\Phi_{d, K}(t).
			\end{align*}
			Da $K$ nullteilerfrei ist, impliziert das bereits $\Phi_{n,K}(t) = \alpha(\Phi_{n,\IQ}(t))$.
			\qedhere
		\end{description}
	\end{enumerate}
\end{proof}
...
\subsection*{Hintergrund/Ausblick}
Fermats letzter Satz: Sei $n\in\IN, n\neq 0$. Existiert ein Tripel $(x,y,z)\in\IZ^3$ mit \[x^n+y^n = z^n\;?\]
...
\emph{Wie ist Fermat möglicherweise auf die Idee gekommen, das einfach bewiesen zu haben?}

\medskip

Seien $X,Y\in \IZ$ und $n\in\IZ_{>0}$, $n $ ungerade. Betrachte den Ausdruck $X^n+Y^n\in\IZ[\xi]$, wobei $\xi$ eine primitive $n$-te Einheitswurzel ist. Mit \cref{lem:24.7} und durch Nachrechnen erhält man  $X^n+Y^n = (X+Y)(X+\xi Y)\dots(X+\xi^{n-1}Y)$.

Falls nun $X^n+Y^n = Z^n\in\IZ$ gilt, können wir $X^n+Y^n = p_1\dots p_r$ als Produkt von Primzahlen schreiben, welche irreduzibel in $\IZ[\xi]$ sind. Wenn $\IZ[\xi]$ faktoriell wäre (was aber nicht der Fall ist), dann ist das ein Widerspruch zur eindeutigen Zerlegung in irreduzible Elemente.

\lecture{29. Januar 2018}
\subsection{Inverses Galoisproblem}
Wir wissen: Wenn $\xi$ eine primtitve $n$-te Einheitswurzel ist (über $\IQ$ in $\IC$), dann ist $\IQ(\xi)\sslash \IQ$ galois und $\Gal(\IQ(\xi)\sslash\IQ)\cong (\IZ/n\IZ)^{\times}$.

Falls $n_1,\dots, n_r\in\IZ_{>0}$ teilerfremd sind und $\epsilon_i$ eine primitive $n_i$-te Einheitwurzel ist, dann ist $\epsilon = \epsilon_1\dots\epsilon_r$ eine primitve $n = n_1\dots n_r$-te Einheitwurzel (nachprüfen!), also $\Gal(\IQ(\epsilon)\sslash \IQ)\cong (\IZ/n\IZ)^{\times}\cong (\IZ/n_1\IZ)^{\times}\times\dots\times(\IZ/n_r\IZ)^{\times}$, wobei die letzte Isomorphie noch nachzuprüfen ist.

Frage: Welche Gruppen treten als $\Gal(L\sslash \IQ)$ auf (mit $L\sslash \IQ$ etwa endlich oder galois)? Tritt jede endliche Gruppe auf?

\begin{satz}[Dirichlet] \label{thm:dirichlet}
	Sei $n\in\IZ_{>0}$. Dann existieren unendlich viele Primzahlen $p$ mit $p\equiv 1\pmod n$.
\end{satz}
\begin{proof}
	Wir nehmen an, es existieren nur endlich viele solche Primzahlen, sagen wir $p_1,\dots,p_r$. Sei $m\coloneqq p_1\dots p_r$. Dann ist \begin{equation}\Phi_m(t) \in \IZ[t]\text{ normiert und }\Phi(0)\neq 0 . \tag{*} \label{eq:25.1}
	\end{equation}
	Deswegen gilt $\lim\limits_{z\to\infty}\Phi_m(mz) = \infty$ (wobei $z\in\IN$). Daraus folgt, dass ein $z_0\in\IN$ existiert, sodass $\Phi_m(mz_0)>p_i$ für alle $i$ gilt. Mit \eqref{eq:25.1} folgt, dass eine Primzahl $p$ mit $p\mid \Phi_m(mz_0)$ und $p\neq p_i$ für alle $i$ existiert. Also gilt $p\mid (mz_0)^m-1$, aber $p\nmid (mz_0)^m$, weshalb $\ggT(m,p) = 1$ und $\overline{\Phi_m(mz_0)} = 0\in\IF_p$. Nach \cref{lem:25.2} folgt daraus $p\equiv 1\pmod m$ und mit der Definition von $m$ folgt $p\equiv 1\pmod n$.
\end{proof}
\begin{lem}\label{lem:25.2}
	Sei $m\in\IZ_{>0}$, $p$ prim mit $\ggT(m,p) = 1$, wobei $\overline{\Phi_m(t)}$ eine Nullstelle $\overline{a}$ in $\IF_p$ mit $a\in\IZ$ hat. Dann gilt $p\equiv 1\pmod m$.
\end{lem}
\begin{proof}
	$p$ teilt $\Phi_m(a)$, da $\overline{\Phi_m(\overline{a})} = 0$ in $\IF_p$. Daraus folgt $p\mid a^m-1$, also $a^m\equiv 1\pmod p$ und damit $\overline{a}^m = 1\in\IF_p^{\times}\subseteq \IF_p$, wobei $\IF_p^{\times}$ die zyklische Gruppe der Ordnung $p-1$ ist.
	
	Wir behaupten $\ord(\overline{a}) = m$ in $\IF_p^{\times}$. Da $\overline{a}^m = 1$ ist, gilt auf jeden Fall $\ord(\overline{a})\leq m$. Angenommen, $\ord(\overline{a}) = s<m$, wobei dann $s|m$ gilt. Nun folgt \[t^m-1 = \prod_{d|m}\Phi_d(t) = \Phi_m(t)\prod_{\substack{d|m,\\d<m}}\Phi_d(t) = \Phi_m(t) \left(\prod_{d|s}\Phi_d(t)\right) h(t) = \Phi_m(t)(t^s-1)h(t)\] für ein gewisses $h(t)\in\IZ[t]$.
		
	Also gilt $\overline{a}^m-1 = \overline{\Phi_m(\overline{a})}(\overline{a}^s-1)\overline{h(\overline{a})}$, wobei die ersten beiden Faktoren beide $0$ sind. Somit hat $f(t) = t^m-1\in\IF_p[t]$ eine mehrfache Nullstelle im Widerspruch zu $f'(t) = mt^{m-1}\in\IF_p[t]$, wobei $m\neq 0$ nach Voraussetzung.
	
	Folglich gilt die Behauptung; es ist $\ord(\overline{a}) = m$ und mit dem \namereff{thm:lagrange} erhalten wir $m\mid p-1$, also $p\equiv 1\pmod m$.
\end{proof}

\begin{satz}\label{thm:25.3}
	Jede endliche abelsche Gruppe $G$ ist isomorph zu $\Gal(L\sslash \IQ)$ für eine endliche Galoiserweiterung $L\sslash \IQ$.
\end{satz}
\begin{proof}
	Sei $G$ eine endliche abelsche Gruppe. Mit den \hyperreff[thm:sylow]{Sylowsätzen} und \cref{???} folgt, dass $n_1,\dots, n_r\in\IZ_{>0}$ mit $G\cong \IZ/n_1\IZ\times\dots \times \IZ/n_r\IZ$ existieren. Nach \namereff{thm:dirichlet} finden wir paarweise verschiedene Primzahlen $p_i$ ($1\leq i\leq r$) mit $p_i\equiv 1\pmod {n_i}$. Es gilt also $p_i-1 = m_in_i$ für gewisse $m_i\in\IZ$. %hier wurde auf 11.5 verwiesen, was allerdings nicht existiert. Bitte suchen!
	
	Es sei dem aufmerksamen Leser als Übung überlassen, die Existenz einer Untergrupe $H_i<(\IZ/p_i\IZ)^{\times}$ der Ordnung $m_i$ mit $(\IZ/p_i\IZ)^{\times}/H_i = \IZ/n_i\IZ$ zu zeigen.
	
	Seien nun $\epsilon_i$ für $1\leq i\leq r$ primitive $p_i$-te Einheitwurzeln. Sei $\epsilon = \epsilon_1\dots \epsilon_r$, also eine primitive $p_1\dots p_r$-te Einheitswurzel und $\Gal\coloneqq\Gal(\IQ(\epsilon)\sslash \IQ)\cong (\IZ/p_1\IZ)^{\times}\times\dots \times(\IZ/p_r\IZ)^{\times}$.
	Sei $H< \Gal(\IQ(\epsilon)\sslash \IQ)$ die Untergruppe, die $H_1\times\dots\times H_r$ entspricht.
	Sei $M = L^H$ mit $L = \IQ(\epsilon)$, wobei $M$ ein Zwischenkörper $\IQ\subseteq M\subseteq L$ ist. Da $\Gal$ abelsch ist, ist $H<\Gal$ automatisch normal. Die \namereff{thm:galoiskorrespondenz} sagt uns nun, dass $M\sslash \IQ$ normal ist. Da $M\sslash \IQ$ außerdem separabel ist (da $\chr\IQ=0$), ist diese Körpererweiterung galois mit
...
\[ \qquad f(t) = 0\]
für $f(t) = t^n+a_{n-1}t^{n-1}+\dots+a_1t+a_0\in\IQ[t]$ (oder $\IR[t],\IC[t]$).
\begin{description}
	\item[$n = 1$:] $t_1 = -a_0$ (einzige Lösung)
	\item[$n = 2$:] $t_{1,2} = \frac{-a_1\pm\sqrt{a_1^2-4a_0}}{2}\in\IC$. Schon die Babylonier konnten das, allerdings nur geometrisch und nur für bestimmte Werte, weil sie noch nicht so viele Skillz im Wurzelziehen hatten. Im 16. Jahrhundert wurde dann diese allgemeine Formel entdeckt.
...
\end{defi}
\begin{defi}
	Sei $L\sslash K$ eine endliche Körpererweiterung. Wir nennen $L\sslash K$ auflösbar, falls Körpererweiterungen $K\subseteq L\subseteq E$ existieren, sodass $E \sslash K$ galois und $\Gal(E \sslash K)$ als Gruppe auflösbar ist.
\end{defi}
\begin{lem}
	Sei $N$ die normale Hülle von $L\sslash K$ in $\overline{K}$. Dann ist $L\sslash K$ genau dann auflösbar, wenn $L\sslash K$ separabel und $\Gal(N\sslash K)$ auflösbar ist.
\end{lem}

\begin{proof}~
	\begin{description}
		\item[\glqq$\Rightarrow$\grqq:] Sei $E$ wie in der Definition; also ist insbesondere $L\sslash K$ separabel, weil $K\subseteq L\subseteq E$ gilt. Nach dem \namereff{thm:fortsetz} ist $\res\colon\Gal(E\sslash K)\to \Gal(N\sslash K), \phi\mapsto \phi|_N$ (wohldefiniert aufgrund der Normalität von $N$) surjektiv.
		Dabei ist $\Gal(E\sslash K)$ nach Voraussetzung auflösbar. Als Quotient einer auflösbaren Gruppe ist $\Gal(N\sslash K)$ auflösbar.
		\item[\glqq$\Leftarrow$\grqq:] Per Definition $N\sslash K$ endlich sowie galois mit $\Gal(N\sslash K)$ auflösbar. Man setze nun $E\coloneqq N$.
		\qedhere
	\end{description}
\end{proof}
Insbesondere gilt, wenn $L\sslash K$ endlich und galois ist, dass $L\sslash K$ genau dann auflösbar ist, wenn $\Gal(L\sslash K)$ auflösbar ist.
...
\end{kor}
\begin{proof}
	Nach dem \namereff{thm:primitives element} wissen wir $L = K(a)$ für ein $a\in L$. Sei $M$ der Zerfällungskörper von $m_a(t)\in K[t]$. Nun hat $m_a(t)\geq 4$ Nullstellen. Also ist $\Gal(M\sslash K)<S_n$, wobei $N$ die Anzahl der Nullstellen von $m_a(t)$ ist. Da Untergruppen von auflösbaren Gruppen auflösbar sind, reicht es also zu zeigen, dass $S_2, S_3, S_4$ auflösbar sind. Für $S_2, S_3$ wissen wir das; für $S_4$ ist $\{e\}\nt V_4\nt A_4\nt S_4$ eine Normalreihe mit abelschen Quotienten. Folglich ist auch $S_4$ auflösbar.
\end{proof}
\begin{satz}[Abel-Ruffini]
	Im Allgemeinen ist eine Gleichung 5. Grades $f(t) = 0$ nicht auflösbar. (Das heißt, dass der Zerfällungskörper von $f(t)$ über $\IQ$ nicht durch Radikale auflösbar ist.)
\end{satz}
\begin{proof}
	Eine \glqq allgemeine\grqq\ Gleichung $n$-ten Grades hat Galoisgruppe $S_n$, es existiert also ein $f(t)$ wie eben von Grad $n$ mit $\Gal(L \sslash \IQ) \cong S_n$, wobei $L$ Zerfällungskörper von $f(t)$ ist. Ein Beweis für primes $n$ findet man im Bosch.
...
\end{bem}
%Klausurrelevant: Alles außer zyklotomische Körper, vmtl auch Punkt 25,26 nicht.
%Wahr/Falsch
%Abfragen von Definitionen, Aussagen von Sätzen
%Beweisaufgaben
%Beispiele rechnen
