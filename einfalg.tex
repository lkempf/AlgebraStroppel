...
\begin{proof}
	...
	\end{align*}
	Nun gilt $p \nmid a_{p-1}$ und $p \mid a_i$ für $0 \leq i \leq p-2$ und $p^2 \nmid a_0$. Somit sind die Kriterien für das \textsc{\hyperref[thm:eisenstein]{Eisensteinsche Irreduzibilitätskriterium}} erfüllt. Folglich ist $f\left( \Phi_p(t) \right) \in \IZ[t]$ irreduzibel. Damit ist auch $\Phi_p(t)$ irreduzibel.
\end{proof}
...
Sei $L\sslash K$ eine Körpererweiterung sowie $a\in L$. Die Inklusion $K\to L, \lambda\mapsto\lambda$ ist ein Ringhomomorphismus. Nach der \textsc{\hyperref[thm:unieig_polyring]{universellen Eigenschaft des Polynomrings}} existiert genau ein Ringhomomorphismus
\begin{eqnarray*}
...
\begin{proof}
	...
	\end{description}
\end{proof}
\begin{kor} \label{kor:mehrfache nst aequivalenzen}
...
\begin{proof}
	...
	\begin{description}
		...
		\item[$\chr K = p>0$:] Sei $a\in L$ und $m_a(t)\in K[t]$ das zugehörige Minimalpolynom. Wir nehmen an, $m_a(t)$ habe eine mehrfache Nullstelle. Mit \cref{kor:mehrfache nst aequivalenzen} folgt $m_a'(t) = 0$, $m_a(t) = Q(t^p)$ für ein $Q(t)\in K[t]$. Sei $Q(t)  =\sum_{i = 0}^n b_it^i\in K[t]$. Weil $K$ perfekt ist, existieren $a_1, \dots, a_n$ mit $a_i^p = b_i$. Dann folgt $\left(\sum_{i = 0}^na_it^i\right)^p = \sum_{i = 0}^nb_i(t^p)^i = Q(t^p) = m_a(t)$. Das ist ein Widerspruch zur Irreduzibilität von $m_a(t)$; folglich hat $m_a(t)$ keine mehrfachen Nullstellen. Damit ist $a$ separabel über $K$ für alle $a\in L$; $L\sslash K$ ist folglich separabel.
		\qedhere
	\end{description}
\end{proof}
...
\begin{bem}\leavevmode\begin{enumerate}
		...
		\item Seien $L_1\sslash K$ und $L_2\sslash K$ Körpererweiterungen und $\phi\colon L_1\to L_2$ ein $K$-Isomorphismus. Dann gilt $[L_1:K]_{\sep} = [L_2:K]_{\sep}$.
...
\end{lem}
\begin{proof}
	Sei $L = K(a_1,\dots, a_n)$ für gewisse $a_1,\dots, a_n\in L$. Sei $L_0 = K$, $L_i = K(a_1,\dots, a_i)$, also $L_i = L_{i-1}(a_i)$. Wir wissen, dass für eine einfache algebraische Erweiterung $M(a)\sslash M$ für einen Körper $M$ nach \cref{thm:khomo <-> nullstellen} \[[M(a):M]_{\sep} = \left|\left\{\phi\colon M(a)\to \overline{M} \left| \substack{\text{$\phi$ $M$-Homo,}\\\deg \phi \le \deg m_a(t)\in M[t]}\right.\right\}\right|\] gilt, wobei $\deg m_a(t) = [M(a):M]$. Wir erhalten $[L:K]_{\sep} = \prod_{i = 1}^n[L_i:L_{i-1}]_{\sep} \leq \prod_{i = 1}^n[L_i:L_{i-1}] = [L:K]$.	
\end{proof}
...
\begin{bsp}
	\begin{itemize}
	\item Sei $\IC/\IR$ eine Galoiserweiterung vom Grad $[\IC:\IR] = 2$. Dann gilt $G = \Gal(\IC/\IR)$. Nach \ref{thm:21.1} ist $|G| = 2\Rightarrow G\cong \IZ/2\IZ$. 
