...
\lecture{30. November 2017}
	...
	\begin{proof}
		\leavevmode
		\begin{description}
			\item[\emph{Erzeugendensystem.}] Sei $k\geq 0$. Dann gilt $t^{d+k} = t^km_a(t)+Q$ in $K[t]$, wobei $Q$ eine Linearkombination von Polynomen mit Grad kleiner als $d+k$ ist. Also gilt in $K[t]/(p(t)) = K[t]/(m_a(t))$.
...
\begin{proof}[Beweis von \cref{thm:alg_ab}]
	...
	\begin{description}
		...
		Sei $I \coloneqq \left(f(X_f)\mid f\in J\right)\subseteq R$ nach der \hyperref[thm:unieig_polyring]{universellen Eigenschaft von $K[t]$}. Es ist klar, dass $I$ ein Ideal ist; wir behaupten, dass $I$ sogar ein echtes Ideal ist. Der Beweis hierfür folgt weiter unten.
...
\end{defi}
\begin{satz} \label{thm:korrespKHomNstMinPoly}
...
\begin{bsp}
	...
	\end{enumerate}
\end{bsp}
\begin{satz}[Normalität] \label{thm:normalKeÄquiv}
...
\end{satz}
\begin{kor} \label{cor:zwischenKoeImg}
	Sei $L\sslash K$ eine Körpererweiterung und $K\subseteq M\subseteq L$ ein Zwischenkörper mit $M\sslash K$ normal (und endlich, wobei man das nach Bemerkung auch weglassen kann). Dann gilt: $\phi\colon M\to L$ ist ein $K$-Homomorphismus. Dann ist $\phi(M) = M$.	
\end{kor}
\begin{proof}
	Betrachte $M\xrightarrow{\phi}L$ und $M\xrightarrow{\id}M\subseteq L$ $K$-Homomorphismen. Da $M\sslash K$ normal ist, gilt nach \cref{thm:normalKeÄquiv} $\im\phi = \im(\id) = M$.
\end{proof}

\begin{proof}[Beweis von Satz \ref{thm:19.7}]
	Da $L\sslash K$ endlich ist, gilt $L = K(a_1,\dots, a_n)$ für gewisse $a_1,\dots,a_n\in L$. Sei $f(t) = m_{a_1}(t)\dots m_{a_n}(t)\in K[t]$ mit $m_{a_i}(t)$ Minimalpolynom von $a_i$ Sei $M$ ein Zerfällungskörper von $f(t)$ über $L$, also auch ein Zerfällungskörper von $f(t)$ über $K$. Nach \cref{thm:normalKeÄquiv} ist $M\sslash K$ normal. Noch zu zeigen: $M\sslash K$ ist endlich. Behauptung: $[M:K]\leq \deg(f(t))$. Damit folgt der Satz, die Behauptung folgt aus nächstem Lemma.
\end{proof}
\begin{lem} Sei $f(t)\in K[t]$, $K$ ein Körper, $L$ ein Zerfällungskörper von $f(t)$. Dann gilt $[L:K]\leq\deg(f(t))$.
\end{lem}
\begin{proof}
	Sei $L = K(b_1,\dots, b_n)$, wobei $b_i$ die (verschiedenen) Nullstellen von $f(t)$ sind. Insbesondere teilt $m_{b_i}(t)$ das $f(t)$. Folglich gilt $[K(b_i):K] = \deg m_{b_i}(t)\leq \deg f(t)$. Damit folgt $[L:K] = [L:K(b_1)][K(b_1):K]$. $L = K(b_1)(b_2,\dots, b_n)$ ist ein Zerfällungskörper von $f(t)/(t-b_1)$. Mit Induktion ergibt sich
	$$[L:K(b_1)][K(b_1):K]\leq (\deg(f(t)/(t-b_1))!\cdot\deg(f(t)) = (deg(f(t)))!$$
\end{proof}

\begin{bem} Sei $L\sslash K$ eine normale Körpererweiterung; $K\subseteq M\subseteq L$ ein Zwischenkörper. Dann ist $L$ die normale Hülle von $M\sslash K$, falls es keinen Zwischenkörper $M\subseteq N\subseteq L$ mit $N\sslash K$ normal gibt. (Also ist $L$ der kleinste Körper, der $M$ enthält und normal über $K$ ist.) Man kann zeigen: die normale Hülle ist eindeutig bis auf $K$-Isomorphismus.
\end{bem}

\begin{defi} Sei $L\sslash K$ eine Körpererweiterung. \begin{enumerate}
		\item $a\in L$ heißt separabel, falls $m_a(t)\in K[t]$ keine mehrfachen Nullstellen hat in $\overline{K}$. %vllt auch in $L$, aber Stroppel weiß das gerade nicht so genau
...
\begin{proof}
	...
	\begin{description}
		...
		\item[2. $\chr K = p>0$:] Sei $a\in L$ und $m_a(t)\in K[t]$ Minimalpolynom. Annahme: $m_a(t)$ hat eine mehrfache Nullstelle. Mit \cref{cor:irredPolyÄquiv} folgt $m_a'(t) = 0$, $m_a(t) = Q(t^p)$ für ein $Q(t)\in K[t]$. Sei $Q(t)  =\sum_{i = 0}^n b_it^i\in K[t]$. Weil $K$ perfekt ist, existieren $a_1, \dots, a_n$ mit $a_i^p = b_i$. Dann gilt: 
		$$(\sum_{i = 0}^na_it^i)^p = \sum_{i = 0}^nb_i(t^p)^i = Q(t^p) = m_a(t)$$
		Das ist ein Widerspruch zu $m_a(t)$ irreduzibel; folglich hat $m_a(t)$ keine mehrfachen Nullstellen. Damit ist $a$ separabel über $K$ für alle $a\in L$; $L\sslash K$ ist folglich separabel.
	\end{description}
\end{proof}

\begin{defi}
	$L\sslash K$ sei eine algebraische Körpererweiterung. Dann definiere den Separabilitätsgrad
	$$[L:K]_{sep} \coloneqq \abs{\{\phi\colon L\to\overline{K}\mid\phi ~K\text{-Homomorphismus}\}}$$
	von $L\sslash K$.
\end{defi}

\begin{bem}\leavevmode\begin{enumerate}
		\item Der Separabilitätsgrad ist unabhängig von der Wahl von $\overline{K}$.
		\item $L_1\sslash K$, $L_2\sslash K$ seien Körpererweiterungen und es existiert ein $\phi\colon L_1\to L_2$ $K$-Homomorphismsu, der ein Isomorphismus ist. Dann gilt $[L_1:K]_{sep} = [L_2:K]_{sep}$.
...
\end{lem}
\begin{bem}
	Sei $L\sslash K$ endlich, normal. Dann ist $\phi(L) = L$ für jeden $K$-Homomorphismus nach \cref{cor:zwischenKoeImg}. Also ist $L\sslash K$ normal und es gilt $$[L:K]_{sep} = \#\{K-\text{Homos }\phi\colon L\to L\} = \#Aut_K(L)$$ weil jeder Homomorphismus injektiv ist, also bijektiv, weil $\dim_LK$ endlich.
\end{bem}
\begin{proof}[Beweis von \ref{lem:19.12}]
	Übungsblatt
\end{proof}
\begin{proof}[Beweis von \ref{lem:19.11}] Sei $L = K(a_1,\dots, a_n)$ für gewisse $a_1,\dots, a_n\in L$. Sei $L_0 = K$, $L_i = K(a_1,\dots, a_i)$, also $L_i = L_{i-1}(a_i)$. Wir wissen: für einfache algebraische Erweiterung $M(a)\sslash M$ für einen Körper $M$ ist 
$$[M(a):M]_{sep} = \abs{\{\phi\colon M(a)\to \overline{M}\mid\phi~ M\text{-Homo}\}}\leq \deg m_a(t)\in M[t]$$
nach \cref{thm:korrespKHomNstMinPoly}, wobei $\deg m_a(t) = [M(a):M]$. Somit gilt 
$$[L:K]_{sep} = \prod_{i = 1}^n[L_i:L_{i-1}]_{sep} \leq \prod_{i = 1}^n[L_i:L_{i-1}] = [L:K]$$
\end{proof}

\lecture{15. Januar 2018}
\begin{lem}[Charakterisierung von separabel]
...
\begin{proof}
	...
	\begin{description}
		...
		\item[Fall 2: $|K|  =\infty$:] Sei $N = K(x,y)$ und $\overline{K}$ der algebraische Abschluss von $K$. Sei $[N:K]_{sep} = m$ und $\{\phi_1,\dots,\phi_m\} = \{\phi\colon N\to \overline{K}|\phi\ K\text{-Homomorphismus}\}$. Es gilt (nach Lemma 19.13): $[N:K]_sep = [N:K]$. Sei %TODO: FIX THIS HÄSSLICHEN ZEILENUMBRUCH IM SEPARABILITÄTSGRAD
...
\begin{defi}
	$L\sslash K$ Körpererweiterung. Dann definiere die Galoisgruppe $$\Gal(L\sslash K) = \{\phi\colon L\to L \mid \phi\text{ bijektiver Körperhomomorphismus}, \phi|_K = \id_K\}$$
	von $L\sslash K$. Das ist eine Gruppe bezüglich Komposition von Abbildungen.
\end{defi}
\begin{lem}\label{lem:20.1}
	$\Gal(L\sslash K)\subseteq  A\eqqcolon\{\phi\colon L\to L\mid\phi\ K\text{-Homomorphismus}\}$. Ist $L\sslash K$ algebraisch, dann gilt sogar Gleichheit.
\end{lem}
\begin{proof}
...
\begin{defi}
	Sei $G$ eine Gruppe, $X$ eine Menge und $G\operates X$ eine Operation von $G$ auf $X$. Sie heißt
	\begin{enumerate}
...
\begin{satz} \label{thm:grupOpTreuTransGaloisGrup}
	 Sei $K$ ein Körper, $f\in K[t]$ irreduzibel. Sei $L$ ein Zerfällungskörper von $f$ über $K$. Dann operiert $G\eqqcolon \Gal(L\sslash K)$ auf $X\eqqcolon \{\text{Nullstellen von }f\text{ in }L\}$ treu und transitiv durch $\phi.x = \phi(x)$ für $\phi\in G, x\in X$. 
\end{satz}	
\begin{proof}
	\leavevmode
	\begin{description}
...
\begin{defi}
	...
	In diesem Fall gilt (\cref{lem:20.1}) 
	$$\Gal (L\sslash K) = \Aut (L\sslash K) =\{\phi\colon L\to L\mid K\text{-Homomorphismus}\}$$
\end{defi}
...
\end{satz}
\begin{proof}
	Sei $L\sslash \IC$ endliche algebraische Körpererweiterung. Zu zeigen: $L = \IC$. Da $L\sslash \IC$ endlich ist, ist $L\sslash \IR$ ebenfalls endlich. Weil $\chr \IR= 0$ ist, ist $L\sslash \IR$ separabel, o.B.d.A. $L\sslash \IR$ normal (\cref{thm:19.7}). Damit ist $L\sslash \IR$ eine Galoiserweiterung.
