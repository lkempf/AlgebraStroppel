...
\begin{proof}
	...
	Außerdem gilt:
	\begin{align*}
		&s_1+(-s_2) = \phi (r_1) + (-\phi (r_2)) \\ 
		&= \phi (r_1)+\phi (-r_2) = \phi (r_1+(-r_2))\in\im\phi \\
		&\Rightarrow s_1+(-s_2)\in \im\phi \\
		&\Rightarrow \text{(UR2)}
	\end{align*}
	Somit sind alle Unterringaxiome erfüllt.
\end{proof}
...
\begin{bsp}
	\begin{eqnarray*}
		\phi\colon \IR & \longhookrightarrow & \IC\\%TODO: langer Einbettungspfeil (longhookrightarrow)
		x & \longmapsto & x
	\end{eqnarray*}
...
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item \glqq$\Leftarrow$\grqq: $t-a$ teilt $p(t)\Rightarrow \exists q(t) \in R[t]$ sodass $p(t) = q(t)\cdot (t-a)\Rightarrow\ev_a (p(t)) = \ev_a(q(t))\cdot\ev_a(t-a) = 0 \Rightarrow a $ ist NST von $p(t)$.
		
		\glqq$\Rightarrow$\grqq: Sei $a$ NST von $p(t)$ Sei
		\begin{eqnarray*}
			\phi\colon R&\longto & R[t]\\
		...
		(Für alle $r\in R$ gilt $r1\mapsto r1; t\mapsto t+a\mapsto (t-a)+a = t$).
		...
		\item $m,n\in\IN_0$:
		$$p(t)q(t) = (\sum_{i = 0}^{m}a_it^i)(\sum_{i = 0}^{n}b_it^i) = a_nb_mt^{n+m}+\dots,$$ wobei $a_n,b_m \neq 0$ sind, also $a_mb_n\neq 0$, da $R$ Integritätsbereich ist. $\Rightarrow \deg(p(t)q(t)) = n+m$.
		
		\item Induktion nach Grad:
		
		$\deg(p(t)) = 1$: $p(t) = b_1t+b_0; b_0,b_1\in R$. Falls $a$ Nullstelle von $p(t)$, gilt nach 1: $\exists r\in R\setminus\{0\}$ sodass $r(t-a) = b_1t+b_0\Rightarrow r = b_1$ und $-ar = b_0$. Annahme: $a'$ weitere Nullstelle. Dann ist $-ar = -a'r' = -a'r$, da $r = b_1 = r'$, also $r(a'+(-a)) = 0$. Da $R$ Integritätsbereich und $r\neq 0$, muss $a' = a$ gelten.
		
		Sei nun $\deg(p(t)) = n>1$. Sei $a$ Nullstelle von $p(t)$. Dann folgt mit Teil 1 des Satzes: $(t-a) q(t) = p(t)$ für ein $q(t)\in R[t]$ mit $\deg(q(t)) = n-1$ (nach Teil 2 des Satzes).
		
		Also gilt nach der Induktionsvoraussetzung: $q(t)$ hat maximal $n-1$ Nullstellen.
		
		Behauptung: $b$ ist genau dann Nullstelle von $p(t)$, wenn $b$ Nullstelle von $q(t)$ ist oder $b = a$.
		
		\glqq$\Leftarrow$\grqq: $b$ Nullstelle von $q(t)$: $\ev_{b}(p(t)) = \ev_{b}(q(t))\cdot \ev_b{t-a} = 0\cdot \ev_{b}(t-a) = 0$ oder $b = a:$: $\ev_{b}(p(t)) = \ev_{b}(q(t))\cdot \ev_b{t-a} = \ev_{b}(q(t))\cdot 0 = 0$.
		
		\glqq$\Rightarrow$\grqq: $b$ Nullstelle von $p(t) \Rightarrow 0 = \ev_{b}(p(t)) = \ev_{b}(q(t))\cdot \ev_{b}(t-a)\xRightarrow{R \text{ Int-b.}} \ev_{b}(q(t)) = 0\Rightarrow b$ Nullstelle von $q(t)$ oder $\ev_b(t-a) = 0\Rightarrow b = a$.
		
		$\Rightarrow p(t)$ hat maximal $n$ verschiedene Nullstellen.
	\end{enumerate}
\end{proof}
...
\begin{bem}
	\leavevmode
	\begin{enumerate}
		\item $K$ algebraisch abgeschlossen; $p(t)\in K[t], \deg(p(t)) =n > 0\Rightarrow \exists c\in K^{\times}, a_1,\dots,a_n\in K$ nicht zwangsweise verschieden, sodass $p(t) = c(t-a_1)\dots (t-a_n)$. (Beweis: Teil 3 von Satz 7.14 induktiv anwenden)
...
\begin{defi}
	$S$ Ring, $R\subseteq S$ Unterring, $a_1,\dots,a_n\in S$ sodass $a_ix = xa_i\forall x\in R$, $a_ia_j = a_ja_i\forall 1\leq i,j\leq n$. Setze
	$$ R[a_1,\dots,a_n] = \bigcap\limits_{S'\subseteq S,R\subseteq S', a_1,\dots,a_n\in S'}S'$$ %TODO: mach das schöner
	Dann heißt $R[a_1,\dots,a_n]$ der von $a_1,\dots,a_n$ erzeugte Unterring in $S$ über $R$. 
\end{defi}


\begin{bem}
	\leavevmode$R[a_1,\dots,a_n]$ ist der kleinste Unterring von $S$, der $R$ und $a_1,\dots,a_n$ enthält.
		\begin{proof}
			\leavevmode
			\begin{itemize}
				\item $R[a_1,\dots,a_n]$ enthält $R$ und $a_1,\dots,a_n$ per Konstruktion.
				\item Ist $S'$ ein weiterer solcher Unterring: $S'\supseteq \cap_{S''}S'' = R[a_1,\dots,a_n]$.
			\end{itemize}
		\end{proof}	
\end{bem}

\begin{satz}
	Sei $S$ Ring, $R\subseteq S$ Unterring, $a_1,\dots,a_n\in S$ mit $a_ia_j = a_ja_i$ und $a_ix = xa_i$ für alle $x\in R$ udn $1\leq i,j\leq n$. Sei $\phi \colon R\to S$ die Inklusion und $R[t_1,\dots, t_n] := ((\dots(R[t_1])[t_2])\dots)[t_n]$. Dann ist $R[a_1,\dots,a_n]$ das Bild des Auswertehomomorphismus' 
	\begin{eqnarray*}
		\ev = \ev_{a_1,\dots,a_n} R[t_1,\dots,t_n] & \longto & S\\
		p(t_1,\dots t_n) & \longmapsto & p(a_1, \dots, a_n),
	\end{eqnarray*}
	wobei für 
	$$ p (t_1,\dots, t_n) = \sum_{(m_1,\dots,m_n)\in\IN_{0}^n}b_{m_1,\dots,m_n}\cdot t_1^{m_1}\dots t_n^{m_n}$$
	(fast alle $b_{m_1,\dots,m_n} = 0$) gesetzt wird
	$$ p(a_1,\dots, a_n ) = \sum_{(m_1,\dots, m_n)\in \IN_0^n} b_{m_1,\dots,m_n}a_1^{m_1}\dots a_n^{m_n}$$
...
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item $\ev$ ist wohldefinierter Ringhomomorphismus: 
		...
		$\Rightarrow \ev$ ist als Verknüpfung von Ringhomos wieder ein wohldefinierter Ringhomomorphismus.
		\item Es gilt für $r\in R$: $\ev(rt_1^0\dots t_n^0) = ra_1^0\dots a_n^0 = r\Rightarrow R\subseteq \im (\ev)$
		
		$\ev(1t_1^0\dots t_{i-1}^0t_1^1t_{i+1}^0\dots t_n^0) = 1a_i = a_i \Rightarrow a_i\in\im(\ev) \Rightarrow R[a_1,\dots, a_n]\subseteq \im(\ev)$.
		
		Sei $S'\subseteq S$ Unterring mit $R\subseteq S'$ und $a_1,\dots, a_n\in S'$.
		
		$\Rightarrow p(a_1,\dots,a_n) = \sum b_{m_1,\dots,m_n}a_1^{m_1}\dots a_n^{m_n}\in S'$, da $S'$ abgeschlossen unter $+$ und $\cdot$. $\Rightarrow R[a_1,\dots,a_n] = \im(\ev)$.
	\end{enumerate}
\end{proof}

\begin{defi}
	Sei $S$ kommutativer Ring, $R\subseteq S$ Unterring, $a_1,\dots, a_n\in S$. Dann heißen $a_1,\dots,a_n$ algebraisch unabhängig (über $R$), fallls $\ev = \ev_{a_1,\dots, a_n}$ (von Satz 7.15) injektiv ist.
	
	Falls $\ev$ nicht injektiv ist, so heißen $a_1,\dots, a_n$ algebraisch abhängig (über $R$).
\end{defi}

\begin{lem}
	Gleiche Voraussetzungen wie in der Definition oben.
	\begin{enumerate}
...
\begin{bsp}
	\leavevmode
	\begin{enumerate}
		\item $1,i\in\IC$ linear unabhängig über $\IR$, aber algebraisch abhängig, denn $p(t_1,t_2) = t_1+t_2^2; \ev_{1,i}(p(t_1,t_2)) = 1+i^2 = 0$.
		
		Schon $i\in \IC$ alg. abhängig über $\IR$, $\ev_{i}(1+t^2) = 1+i^2  = 0$.
		\item $a\in S$ beliebig ($S$ kommutativ) ist algebraisch abhängig über $S$: $\ev_a(t-a) = a-a = 0$.
		\item $S =R[t_1,\dots, t_n]$, dann sind $t_1,\dots t_n$ algebraisch unabhängig über $R$, weil $\ev_{t_1,\dots, t_n}\colon R[t_1,\dots,t_n]\to R[t_1,\dots, t_n] = id_{R[t_1,\dots t_n]}$ offensichtlich injektiv. %TODO: FIX THIS OVERFULL HBOX
		\item algebraisch über $\IQ$ unabhängige $x\in\IR$ heißen transzendente Zahlen.
	\end{enumerate}
\end{bsp}
...
\begin{defi} $R$ Integritätsbereich (d.h. $R\neq\{0\}$ und nullteilerfrei). Sei $a\in R$, $a\neq 0$, $a\notin R^{\times}$. 
\begin{itemize}
	\item $a$ heißt irreduzibel, falls: ($a = b\cdot c\Rightarrow b \in R^{\times}$ oder $c\in R^{\times}$).
	\item $a$ heißt prim, falls: ($a$ teilt $b \cdot c\Rightarrow a$ teilt $b$ oder $a$ teilt $c$), wobei jeweils $b,c\in R$ gilt.
	\item $R$ faktorieller Ring, falls zusätzlich für $b\in R, b\neq 0$ existiert Zerlegung
	$$\text{PZ}\qquad b = \epsilon p_1p_2\dots p_r \text{ für ein $r\in\IN$}$$
	mit $\epsilon\in R^{\times}$ und $p_1,\dots, p_r\in R$ irreduzibel und diese Zerlegung ist \glqq eindeutig\grqq, wobei \glqq eindeutig\grqq\ bedeutet eindeutig bis auf Einheiten und Reihenfolge der Faktoren. (d.h. falls $b = \epsilon'p_1'\dots p_s'$ für $\epsilon'\in R^{\times}$ und $p_1',\dots, p_s'$ irreduzibel, dann gilt $s = r$ und $\exists c_1,\dots, c_r\in R^{\times}$ und $\pi \in S_r$, sodass $p_i' = c_ip_{\pi(i)}$).
\end{itemize}
\end{defi}

\begin{bsp}
	$R = \IZ$. Sei $a\in\IZ, a\neq 0, a\notin\{\pm1\} = \IZ^{\times}$. Dann $a$ prim $\Leftrightarrow a = \pm p$, $p$ Primzahl $\Leftrightarrow a$ irreduzibel. $IZ$ ist faktoriell, denn jedes $b\in\IZ, b\neq 0$ hat Primzahlzerlegung $b = \epsilon p_1\dots p_r$ mit $p_i$ Primzahlen und $\epsilon \in\{\pm 1\} = \IZ^{\times}$. Dies ist \glqq eindeutig\grqq, z.B. $10 = 1\cdot 2\cdot 5 = (-1)(-2)\cdot 5 = (-1)2(-5)$ etc. 
\end{bsp}
\begin{bsp}
	$R = \IZ[\sqrt{-5}] = \{a+b\sqrt{5}i|a,b\in \IZ\}\subset \IC$ Unterring von $\IC$, genauer der von $\sqrt{5}i$ über $\IZ$ erzeugte Unterring in $\IC$. Dann sind $2,3,1+\sqrt{5}i, 1-\sqrt{5}i$ irreduzibel, insbesondere $1+\sqrt{5}i\notin R^{\times}$ (Übung!). Außerdem: $1+\sqrt{5}i\notin \{\epsilon 2,\epsilon3|\epsilon\in R^{\times}\}$ (Übung!).
	
	Aber: $ 6 = 2\cdot 3 = (1+\sqrt{5})(1-\sqrt{5})$, also ist $R$ nicht faktoriell.
\end{bsp}

\begin{lem}
	Sei $R$ Integritätsbereich, $a\in R$.
	\begin{enumerate}
		\item $a$ prim $\Leftrightarrow (a) \neq\{0\}, (a)$ Primideal
		\item $a$ irreduzibel $\Leftrightarrow (a)\neq\{0\},(a)\neq R$ und $\forall b\in R:$ falls $(a) \subseteq (b)\Rightarrow (a) = (b) $ oder $(b) = R$.
	\end{enumerate}
\end{lem}
\begin{proof}
	\leavevmode
...
\begin{satz} $R$ Integritätsbereich. Dann gilt für $a\in R$: \begin{enumerate}
		\item $a$ prim$\Rightarrow a$ irreduzibel.
		\item Falls $R$ Hauptidealring (HIR):
		\begin{enumerate}
			\item $a$ prim $\Leftrightarrow a$ irreduzibel.
			\item $\{0\}\neq I\subseteq R$ Ideal: $I$ Primideal $\Leftrightarrow I$ maximales Ideal.
		\end{enumerate}
	\end{enumerate}
\end{satz}
\begin{proof}
	\leavevmode
	\begin{enumerate}
		\item Sei $a = bc\xRightarrow{a\text{ prim}}a$ teilt $b$ oder $a$ teilt $c$.
		
		$\Rightarrow b = ar$ oder $c = ar$ für ein $r\in R$.  $\Rightarrow b = bcr$ oder $c = cbr$ für ein $r\in R\Rightarrow b(1-cr) = 0$ oder $c(1-br) =0$ für ein $r\in R$. Da $R$ nullteilerfrei ist und $b,c\neq 0$, weil $a\neq 0$, gilt $(1-cr )=0$ oder $(1-br) = 0$. Folglich ist $c$ eine Einheit oder $b$ eine Einheit. $\Rightarrow a$ irreduzibel.
		
		\item \begin{enumerate}
			\item Die eine Richtung folgt nach (1). Sei für die Rückrichtung nun $a$ irreduzibel. Nach Lemma 8.1 ist $(a)$ maximal unter allen Hauptidealen in $R$. Da $R$ ein Hauptidealring ist, ist $(a)$ sogar ein maximales Ideal. Aus Satz 7.10 folgt, dass $(a)$ ein Primideal ist. Nun ist $a$ prim nach 8.1, da $(a)\neq\{0\}$, weil $a$ irreduzibel, also insbesondere $a\neq 0$.
			
			\item \begin{description}
					\item[\glqq$\Leftarrow$\grqq:] nach 7.10
					\item[\glqq$\Rightarrow$\grqq:] Sei $\{0\}\neq I\subseteq R$ Primideal. Da $R$ ein Hauptidealring ist, folgt: $\exists a\in R$ mit $I = (a)$ und $a$ prim nach 8.1. Mit Teil (1) folgt: $I = (a)$ mit $a$ irreduzibel. Nach Lemma 8.1 ist $I = (a) $ maximal unter allen Hauptidealen; da $R$ HIR ist, ist $I$ maximales Ideal.
					\end{description}
			\end{enumerate}
	\end{enumerate}
\end{proof}

\begin{defi} $R$ Integritätsbereich. $R$ heißt euklidischer Ring, falls eine Abbildung
	$$\deg \colon R\setminus\{0\} \longto \IN$$
	existiert, sodass für alle $a,b\in R, b\neq 0$ gilt: $\exists q, r\in R$ mit $a = qb+r$, sodass $r = 0$ oder $\deg(r)<\deg(b)$.
\end{defi}
...
\begin{satz}
	$R$ Hauptidealring $\Rightarrow R$ faktoriell.
\end{satz}

\begin{proof} Sei $0\neq b\in R$. Zu zeigen: Primfaktorzerlegung (PZ) existiert. Wir nehmen an, dass (PZ) nicht existiert, also insbesondere ist $b$ nicht irreduzibel.
$\Rightarrow \exists b_1,c_1\neq R^{\times}: b = b_1c_1$ und nicht beide Faktoren haben eine Primfaktorzerleugn. OBdA hat $b_1$ keine (PZ), insbesondere $b_1$ nicht irreduzibel. Folglich existieren $b_2,c_2\notin R^{\times}: b_1 = b_2c_2$ und oBdA hat $b_2$ keine (PZ) etc.

$\Rightarrow (b_1)\subset (b_2)\subset (b_3)\subset (b_4)\subset \dots$ für gewisse $b_i\in R$ und $(b_i)\subsetneq (b_{i+1})$: denn sonst $b_i = b_{i+1}c_{i+1}$ für ein $c_{i+1}\in R$ und $b_{i+1} = b_ic_i'$ für ein $c_i'\in R \Rightarrow b_i(1-c_i'c_{i+1}) = 0$. Da $R$ nullteilerfrei ist, folgt $c_{i+1}\in R^{\times}$ für alle $i+1$, was im Widerspruch zur Konstruktion steht.

Setze nun $I : = \bigcup\limits_{i = 1}^{\infty}(b_i)$. Das ist Ideal (klar) und somit $I = (d)$ für ein $d\in R$, weil $R$ HIR ist. Nach Definition von $I$ existiert ein $b_i$ mit $d\in (b_i)$, also $(d)\subseteq (b_i)$, weil $(b_i)$ Ideal. $\Rightarrow (d) \subseteq (b_i)\subseteq I = (d)\Rightarrow (b_i) = (d)\subseteq (b_j) = (d) \forall j>i$. Das ist nun ein Widerspruch zu $(b_i) \subsetneq (b_{i+1})$. Folglich hat $b$ eine Primfaktorzerlegung. Dadurch haben wir nun die Existenz der (PZ); wir zeigen nun noch Eindeutigkeit:

Seien $b = \epsilon p_1\dots p_r$ und $b = \epsilon'p_1'\dots p_s'$ zwei Zerlegungen mit $\epsilon, \epsilon'\in R^{\times}, p_i, p_j'\in R$ irreduzibel ($1\leq i\leq r, 1\leq j\leq s$). Betrachte $p_1'$. Nach (2): $p_1'$ teilt $b$; also $p_1'$ teilt $p_i$ für ein $i$ ($1\leq i\leq r$) oder $p_1'$ teilt $\epsilon$ (denn $p_1'$ ist irreduzibel und damit prim, weil $R$ HIR). Fall $p_1'$ teilt $\epsilon$, dann gilt $p_1'\in R^{\times}$ im Widerspruch zu $p_1'$ irreduzibel.

Also: $\exists i: p_1'$ teilt $p_i$. OBdA sei $i = 1$ (evtl umnummerieren). Also ist $p_1 = p_1'c$ für ein $c\in R$. Da $p_1$ irreduzibel ist, ist $p_1'\in R^{\times}$ oder $c\in R^{\times}$. $p_1'$ ist aber irreduzibel und damit insbesondere keine Einheit; folglich ist $c\in R^{\times}$.

$\Rightarrow \epsilon cp_1'p_2\dots p_r = b = \epsilon 'p_1'p_2'\dots p_s'$. Da $R$ nullteilerfrei ist, darf man hier kürzen: $\epsilon c p_2\dots p_r = \epsilon 'p_2'\dots p_s'$. Iteriere dieses Argument nun und erhalte schließlich $r = s$ und $p_i = p_{\pi(i)}'c_i$ für $c_i\in R^{\times}$ und $\pi\in S_r\Rightarrow$ \glqq Eindeutigkeit\grqq.
\end{proof}
...
\begin{satz}
	$R$ kommutativer Ring, %whyever, sind Ringe nicht sowieso schon kommutativ?
	$R \neq \{0\}$
	\begin{enumerate}
		\item $R$ besitzt maximale Ideale (und damit auch Primideale nach 7.10)
		\item Jedes Ideal $I\subseteq R$, $I \neq R$ ist in einem (nicht notwenig eindeutigen!) maximalen Ideal enthalten.
	\end{enumerate}
\end{satz}
\begin{proof}
	Wir zeigen zunächst (2) unter der Annahme von (1): Sei $I\neq R\Rightarrow R/I \neq \{0\}$  und besitzt daher nach (1) maximales Ideal. Nach Satz 7.9 existiert ein maximales Ideal $J\subseteq R$ mit $J\supseteq I$.
\end{proof}
