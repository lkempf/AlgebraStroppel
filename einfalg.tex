...
\author{Alexander Esgen, Lukas Kempf, Luise Puhlmann}
...
\paragraph{Übung:} $f$ Gruppenhomomorphismus; $f$ ist injektiv genau dann, wenn $\text{Ker}(f) = \{e\}$
...
\begin{satz}[Homomorphiesatz]
	Sei $R$ Ring, $I\subseteq R$ Ideal. \begin{enumerate}
		...
		\item Sei $\phi\colon R\to S$ Ringhomo mit $I\subseteq \ker \phi$, dann $\exists!\overline{\phi}\colon R7I\to S$ Ringhomo, sodass $\overline{\phi}\circ can  = \phi$. Also:
		\begin{center}
		\begin{tikzcd}
...
\begin{defi}
	Sei $R$ kommutativer Ring, $a,b\in R$. \begin{enumerate}
		\item $(a) = \{ra|r\in R\}$ ist das von $a$ erzeugte Ideal
		...
		\item 
				 ...
				 $R$ ist nullteilerfrei, falls 0 der einzige Nullteiler ist
	 			 ...
	 			 $R$ heißt Integritätsbereich, falls nullteilerfrei und $R \neq \{0\}$
		\item $a$ heißt Einehit, falls $\exists r\in R$ mit $ar = 1 = ra$. (also $a$ hat Inverses bzgl der Multiplikation)
		
		$R^{\times} = \{c\in R|c \text{ Einheit in $R$}\}$ Einheitengruppe von $R$
	\end{enumerate}
\end{defi}
...
\begin{bsp}
	Betrachte $(\IZ,+,\cdot)$. Behauptung: Das ist ein Hauptidealring.\begin{itemize}
		...
		\item Sei $I$ Ideal, Falls $I = \{0\}$, dann $I = (0)$. Falls $I \neq \{0\}$. Dann $\exists x\in I, x\neq 0$. Wähle $n\in\IN$ minimal mit $n\in I$ (existiert weil $x\in I\Rightarrow -x \in I $ nach (I1)).
		
		Behauptung: $I = (n)$
		
		\begin{proof} \glqq $\supseteq$\grqq: nach (I2); \glqq $\subseteq$\grqq: Sei $y\in I$. Schreibe $y = bn+r$ mit $b,r\in\IZ$ mit $0\leq r< n\Rightarrow y+(-bn) = r\Rightarrow r\in I$; Widerspruch zur Minimalität von $n$ außer $r = 0$
		
		$\Rightarrow y = bn\Rightarrow I\subseteq (n)$
		\end{proof}
	\end{itemize}
\end{bsp}
...
\begin{lem}
	$R$ kommutativer Ring, $R\neq \{0\}$.\begin{enumerate}
		...
		\item Für $a,b,c\in R$ ist \begin{enumerate}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		Annahme: $R^{\times} = R\setminus\{0\}$. Sei $x\in R, x\neq 0\Rightarrow x\in R^{\times}\Rightarrow \exists r \in R$ mit $xr = 1 = rx\Rightarrow r = x^{-1}$ Inverse zu $x$ $\Rightarrow R$ Körper
...
\begin{bsp}
	$R = \IR [t], I = (t^2+1)$. Behauptung: $R/I$ ist Körper.
	\begin{proof}
		...
		$$ \overline{t ^j} = \overline{t^{j-2}(t^2+1)+(-t^{j-2})} \xeq[\text{in }R/I]{\text{Def von $+$ }} \overline{t^{j-2}(t^2+1)}+\overline{-t^{j-2}} \xeq{\text{Def von $\odot$}}$$
		$$ =  \overline{t^{j-2}}\odot \overline{t^2+1} + \overline{-t^{j-2}} = \overline{-t^{j-2}} \xeq{\text{Def von $+$ in $R/I$}} - (\overline{t^{j-2}})$$ TIM IST EINFACH NUR DOOF
		%TODO: FIX THIS
		Für $p = \sum_{i = 0}^{\infty}a_it^i\in\IR[t]$ gilt: $\overline{p} = \overline{b_01+b_1t}$ für gewissen $b_0,b_1\in R$. Also oBdA $x = \overline{b_01+b_1t}$ mit $b_0^2+b_1^2 \neq 0$.
		...
		Da $\overline{t^2+1} = \overline{0}$, also $\overline{-t^2} = \overline{1}$ gilt $xy = \overline{\frac{1}{b_0^2+b_1^2}(b_0^2+b_1^2)} = \overline{1}$. Also ist $x\in (R/I)^{\times}$ mit Inversem $y$.
...
\begin{defi}
	$R$ kommutativer Ring, $R\neq \{0\}$. $I\subseteq R$ Ideal heißt maximal, falls $I\neq R$ und $\not \exists J\subseteq R$ ideal mit $I\subset J\subset R$ (echte Teilmengen).
\end{defi}
\begin{lem}
	$R$ kommutativer Ring, $R\neq 0$. $R$ Körper $\Leftrightarrow\{0\}$ ist (einziges) maximales Ideal.
\end{lem}
\begin{proof}
	\leavevmode
	\begin{itemize}
		\item[\glqq $\Leftarrow$\grqq:] Sei $\{0\}$ maximales Ideal. Dann ist es einziges, denn falls $I\neq \{0\}$ maximales Ideal $\Rightarrow \{\}\subsetneq I\neq R\Rightarrow \{0\} = I$. Widerspruch!
		
		Sei $a\in R, a\neq 0\Rightarrow (0) \subset (a) \Rightarrow (a) = R\Rightarrow \text{Lemma 6 } R^{\times} = R\setminus\{0\}\Rightarrow R$ Körper.
		\item[\glqq $\Rightarrow$\grqq:] Sei $R$ Körper. $I\subseteq R$ Ideal, $I\neq \{0\}$. Dann $\exists a\in I, a\neq 0$. Weil $R$ Körper ist, ist $a\in R^{\times}\Rightarrow (a) = R \Rightarrow I = R\Rightarrow \{0\}$ ist einziges Ideal und maximal.
	\end{itemize}
\end{proof}

\begin{bsp}
	Betrachte $R = \IZ$. Behauptung: $I\subseteq \IZ$ Ideal. Dann $I$ maximal $\Leftrightarrow I = (n) $ mit $n = p$, $p$ Primzahl.
	\begin{proof}
		\leavevmode
		\begin{itemize}
			\item[\glqq $\Rightarrow$\grqq:] Sei $I = (a)$ (weil $\IZ$ Hauptidealring) maximal. Fall $a\neq \pm p$ Primzahl, existiert $b\in \IZ$ mit $b$ teilt $a$ und $b\neq a$, $b\neq  \pm 1$.
			
			$\Rightarrow$ $(a)\subsetneq (b)\subsetneq R$ (weil $b\neq \pm a$; $(b)\neq R$, weil $b\neq \pm 1$, also $b\neq \IZ^{\times}$) Widerspruch zu $I$ maximal, also $a  = \pm p$ $p$ Primzahl.
			
			\item[\glqq $\Leftarrow$\grqq:] Sei $I = (a)$ mit $a = \pm p$, $p$ Primzahl. Sei $I \subsetneq J\subset R$, $J$ Ideal. Dann gilt $J = (b)$ für ein $b\in\IZ$ (da $\IZ$ Hauptidealring) und nach Lemma 7.6 folgt $b$ teilt $a$ und $b\neq \pm a$ (weil $I\subsetneq J$) und $b \neq \pm 1$ (weil $(b)\neq R$). Widerspruch zu $a = \pm p$.
		\end{itemize}
	\end{proof}
\end{bsp}

\begin{satz} \label{satz8}
	$R$ kommutativer Ring, $R \neq\{0\}, I\subseteq R$ Ideal. Dann $R/I$ Körper $\Leftrightarrow I$ maximales Ideal.
\end{satz}

\paragraph{Als Vorbereitung:}

\begin{satz}
	Sei $\phi\colon R\to S$ Ringhomo. ($R,S$ beliebige Ringe) surjektiv. Dann gibt es eine Bijektion zwischen den Mengen
	$$\{\text{Ideale in $R$ mit }\ker \phi\subseteq I\}\longleftrightarrow \{\text{Ideale in $S$}\}$$
	durch:
	\begin{eqnarray*}
		I & \longmapsto &\phi(I)\\
		\phi^{-1}(J) &  & J\\ %TODO: add something linke longtomaps
		 = \{r\in R|\phi(r) \in J\}
	\end{eqnarray*}
\end{satz}

\begin{proof}
	$\phi(I)\subseteq S$ ist Ideal: $UG$ klar, weil Bild von Gruppe unter Gruppenhomo.\begin{itemize}
	...
	Sei $I\subset R$ Ideal mit $I\supset \ker\phi$. Zu zeigen: $\phi^{-1}(\phi(I)) = I$.
	\begin{itemize}
	...
	Sei $J\subset S$ Ideal. Zu zeigen: $\phi(\phi^{-1}(J)) = J$.
	\begin{itemize}
...
\begin{bem} Beachte: Die Bijektion von Satz 7.9 ist inklusionserhaltend, also 
	\begin{eqnarray*}
		I\subseteq I'&\Longleftrightarrow& \phi(I)\subseteq\phi(I')\\
		\text{Ideale von $R$, }&&\\
		\text{die $\ker\phi$ enthalten}
	\end{eqnarray*}
\end{bem}

\begin{proof} \cref{satz8} %TODO: Das hier sauber machen
	Setze $\phi = can\colon R\to R/I$; das ist surjektiv; sei $\ker\phi = I$.
	\begin{itemize}
...
\begin{defi}
	Sei $R$ kommutativer Ring, $I\subseteq R$ Ideal. Dann heißt $I$ Primideal, falls $I\neq R$ und $\forall x, y\in R$ gilt: falls $x\cdot y\in I\Rightarrow x\in I$ oder $y\in I$.
\end{defi}
\begin{bem}
	Falls $R$ Integritätsbereich, dann gilt: $\{0\}$ ist Primideal.
\end{bem}



\begin{bsp}
	$R = \IZ, a\in \IZ$. Dann ist $(a)$ Primideal $\Leftrightarrow a = \pm p$, $p$ Primzahl, oder $a = 0$.
	
	Denn sei $a = \pm p$, $p$ Primzahl. Seien $x,y\in R$ mit $xy\in (a)$.
	\begin{description}
		\item[$\Rightarrow$] $p$ teilt $xy$
		\item[$\Rightarrow$] $p$ teilt $x$ oder $p$ teilt $y$
		\item[$\Rightarrow$] $x\in (p) = (a)$ oder $y\in (p) = (a)$
		\item[$\Rightarrow$] $(a)$ ist Primideal.
	\end{description}

	Andersherum: Sei $(a) \neq (0)$ ein Primideal. Falls $a = \pm 1\Rightarrow (a) = R$ Widerspruch zu $(a)$ ist Primideal. Falls $a \neq \pm 1, a\neq pm p$ $p$ Primzahl: $\Rightarrow \exists n, m$ mit $1<\abs{n}, \abs{m}<\abs{a}$ sodass $a = nm$. 
	$\Rightarrow nm\in (a)$ und $n,m\notin (a)$ Widerspruch zu $(a)$ Primideal.
\end{bsp}

\begin{bem}
	Beachte: $(0)$ ist Primideal in $\IZ$, aber nicht maximal. In $\IZ$ gilt $I$ max $\Rightarrow I $ ist Primideal.
\end{bem}


\begin{satz}
	$R$ kommutativer Ring, $R\neq\{0\}, I\subseteq R$ Ideal. Dann gilt:
	$$ I \text{ Primideal} \Longleftrightarrow R/I \text{ Integritätsbereich}$$
	Insbesondere ist $\{0\}$ Primideal $\Leftrightarrow$ $R$ ist Integriätsbereich.
\end{satz}
...
\begin{proof}
	...
	\begin{description}
		...
		$\xRightarrow[\text{Int.-bereich}]{R/I} \overline{x} = \overline{0}$ oder $\overline{y} = \overline{0}\Rightarrow x\in I$ oder $y\in I\Rightarrow I$ ist Primideal.
	\end{description}
\end{proof}

\begin{kor}
	$R\neq\{0\}$ kommutativer Ring, $I\subseteq R$ Ideal. Dann gilt: $I$ maximal $\Rightarrow I $ Primideal.
\end{kor}
\begin{proof}
...
\begin{kor}
	\leavevmode
	\begin{enumerate}
	\item $R = \IZ$. Dann ist $\IZ/n\IZ$ Körper $\Leftrightarrow n = \pm p$ $p$ Primzahl.
	\item  $\IZ/n\IZ$ Integritätsbereich $\Leftrightarrow n = \pm p$, $p$ Primzahl, oder $n = 0$.
	\end{enumerate}
\end{kor}
...
\begin{satz}[Universelle Eigenschaft von Polynomringen 2]
	Sei $\phi\colon R\to S$ Ringhomomorphismus. Sei $a\in S$. $\Rightarrow \exists!$ Ringhomo:
	\begin{eqnarray*}
...
\begin{proof}
	Reicht Existenz zu zeigen: 
	\begin{itemize}
		\item $ev_a(1_{R[t]}) = ev_a(1t^0) = \phi(1)\cdot a^0 = 1$
		\item $ev_a(\sum_{i= 0}^{\infty}b_it^i + \sum_{i = 0}^{\infty}c_it^i) = ev_a(\sum_{i= 0}^{\infty}(b_i+c_i)t^i)  = \sum_{i = 0}^{\infty} \phi(b_i+c_i)a^i = \sum_{i = 0}^{\infty} \phi(b_i)a^i +\sum_{i=0}^{\infty}\phi(c_i)a^i = ev_a(\sum_{i=0}^{\infty}b_it^i) + ev_a (\sum_{i = 0}^{\infty} c_it^i)$
		\item $ev_a((\sum_{i = 0}^{\infty}b_it^i)(\sum_{i = 0}^{\infty}c_it^i)) = ev_a(\sum_{i = 0}^{\infty}(\sum_{k = 0}^{i} b_{i-k}c_k)t^i) = \sum_{i = 0}^{\infty} (\sum_{k = 0}^{i}\phi(b_{i-k})\phi(c_k))a^i = \sum_{i = 0}^{\infty} (\sum_{k = 0}^{i}\phi(b_{i-k})a^{i-k}\phi(c_k)a^k) = (\sum_{i = 0}^{\infty} \phi(b_i)a^i)(\sum_{i = 0}^{\infty}\phi(c_i)a^i) = ev_a(\sum_{i = 0}^{\infty}b_it^i)\cdot ev_a(\sum_{i = 0}^{\infty}c_it^i)$
	\end{itemize} %TODO: FIX THIS NICHT SEHR SCHÖNE SACHE
	$\Rightarrow ev_a$ ist Ringhomo.
\end{proof}

\begin{bsp}
	\begin{eqnarray*}
		\phi\colon \IR & \longto & \IC\\%TODO: injektiver Pfeil
		x & \longmapsto & x (+0\cdot i)
	\end{eqnarray*}
	$\Rightarrow \exists!$ Ringhomo
	\begin{eqnarray*}
		ev_i\colon \IR[t] & \longto & \IC\\
		t & \longmapsto & i\\
		p(t) & \longmapsto & p(i)
	\end{eqnarray*}
	Es gilt $ev_i(t^2+1) = i^2 +1 = 0$. $\Rightarrow t^2+1\in\ker ev_i\Rightarrow (t^2+1)\subset \ker ev_i$. Also folgt mit dem Homomorphiesatz: $\exists! \overline{ev_i}\colon \IR[t]/(t^2+1)\to \IC$ mit $\overline{ev_i}(\overline{b_1t+b_0}) = b_1i+b_0\Rightarrow \overline{ev_i}$ ist surjektiv.
	
	Da jedes $\overline{p(t)}\in\IR[t]/(t^2+1)$ sich eindeutig schreiben lässt als $\overline{p(t)} = \overline{b_1t+b_0}$ gilt:
	$$\overline{ev_i}(\overline{p(t)}) = \overline{ev_i}(\overline{b_1t+b_0}) = b_1i +b_0 = 0\Longleftrightarrow b_1 = b_0 = 0$$
	$\Rightarrow\overline{ev_i}$ ist injektiv.
	$\Rightarrow \overline{ev_i}$ ist Isomorphismus von Ringen (insbesondere von Körpern.)
\end{bsp}

\begin{defi}
	$R$ ein kommutativer Ring, dann heißt $a\in R$ Nullstelle von $p(t) \in R[t]$, falls gilt: $p(a) = ev_a(p(t)) = 0$ ($\phi\colon R\to R = id \colon R\to R$).
\end{defi}
