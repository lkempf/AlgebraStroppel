...
\begin{bem}
	Der Primkörper eines Körpers ist selbst ein Körper und zwar der kleinste Unterkörper von $K$. Es ist $0,1\in \langle\emptyset\rangle$ und damit auch $1+\dots+1$ für die $n$-fache Addition der $1$ ($n\in\IN$).
\end{bem}
...
\begin{proof}[Beweis Satz 14.2] %TODO: LABELS
...
\begin{description}
	\item[Fall 1:] $a$ ist transzendent: Dann gilt $\ev_a\colon  K[t]\to L$ injektiv und $\im\ev_a = K[a]\subseteq L$ (nach Satz 7.15). %TODO: LABELS
...
\begin{bsp}
	Betrachte $\IC\sslash\IR$ und $i\in\IC$. Dann: $t^2+1\in \IR[t]$ ist $m_i(t)$. Also $[\IR(i):\IR] = 2$. %TODO: FIX THIS HÄSSLICHEN ZEILENUMBRUCH
\end{bsp}
...
\begin{satz}[bzw Definition]\label{thm:algab}
...
\begin{proof}\ref{thm:alg_ab}
	...
	\begin{description}
		...
		Betrachte $R = K[X_f|f\in J]$ (saubere Definition siehe Übungsblatt 9).  %TODO: Link auf dieses Übungsblatt einfügen
		Das ist ein Polynomring über $K$ in unendlich vielen Variablen.
		
		Sei $I := (f(X_f)|f\in J)\subseteq R$ (universelle Eigenschaft von $K[t]$). Behauptung: $I$ ist ein echtes Ideal. Klar: $I$ ist Ideal.
		
		Behauptung 1: $I \neq R$ (Beweis unten)
		Nach Satz \ref{thm:9.1} existiert ein maximales Ideal $m\subseteq R$ mit $R\supseteq m \supseteq I$. Damit ist $R/M = L_1 $ Körper. Klar: $K \subseteq R$ und sogar $K \subseteq R/m$ (durch die Abbildung $K\to R\to R/m$). Also haben wir $L_1\sslash K$ Körpererweiterung. Sei nun $f\in J = K[t]\setminus K$; $f = \sum_{i = 0}^{\infty} b_i t^i$ mit fast allen $b_i = 0$. Zu zeigen: $f$ hat Nullstelle in $L_1$.
		
		Wir wissen $f(\can(X_f)) = \sum_{i = 0}^{\infty}b_i\overline{X_f}^i$, wobei $b_i\in K$ ist. Also $f(\can(X_f)) = \sum_{i = 0}^{\infty}\overline{b_i}{X_f}^i = \overline{\sum_{i = 0}^{\infty}g_iX_f^i} = can(f(X_f))$, wobei $f(X_f)\in I\subseteq m$, also $f(\can(X_f)) = 0$. Also ist $\can(X_f)\in L_1$ Nullstelle von $f(t)$.
		
		\item[2. Schritt:] Betrachte \glqq Turm\grqq\ von Körpererweiterungen $K : = L_0\subseteq L_1\subseteq L_2\subseteq\dots$, sodass $L_{i+1}\sslash L_{i}$ Körererweiterung und jedes $f\in L_i[t]\setminus L_i$ eine Nullstelle hat in $L_{i+1}$  (existiert nach Schritt 1). Setze
		$$ L : = \bigcup\limits_{i\geq 0} L_i$$
		Klar: $L$ ist Körper. Zu zeigen: Gegeben $g\in L[t]\setminus L$, dann hat $g$ Nullstelle in $L$.
		
		Sei dafür $g = \sum_{i = 0}^{\infty}a_it^i$, nur endlich viele $a_i\neq 0$. Folglich gibt es ein $n_0\in\IN$ mit $a_i\in L_{n_0}\forall i$. Das heißt aber, dass wir $g$ als Polynom in $L_{n_0}[t]$ auffassen können. Dadurch hat $g(t)$ eine Nullstelle in $L_{n_0+1}\subseteq L$.
		
		$\Longrightarrow$ Es gibt einen Körper $L$ mit $K\subseteq L$
	\end{description}
	Nun noch zum Beweis von Behauptung 1: Sei $I = R$. Also $1\in I$. Also $1 = \sum_{i =1}^{n}g_if(X_{f_i})$ für gewisse $g_i\in R$, $f_i\in J$. Mit Satz \ref{thm:15.4} (wiederholt anwenden!) existiert $L'\sslash K$ Köreprerweiterung, wobei $f_1,\dots, f_m$ Nullstellen in $L'$ haben, sagen wir $a_1,\dots, a_n$. Nach der universellen Eigenschaft für Polynomring $R$ existiert ein Ringhomomorphismus
	\begin{eqnarray*}
		\ev\colon R & \longto & L'[X_f|f\in J]
	\end{eqnarray*}
	mit $\ev(X_f) = \begin{cases} a_i & \text{ falls } f = f_i \text{ für ein }i\\
												X_f &\text{ sonst}
												\end{cases}$\\											
	für $\lambda\in K$: $\ev{\lambda}  = \lambda\in K\subseteq L'$. Dann gilt $\ev(f_i(X_{f_i})) = f_i(a_i) = 0$, weil $a_i$ Nullstelle von $f_i$ ist. Also gilt für $1\in R$: 
	$$1 = \ev(1) = \ev(\sum_{i = 0}^{m}g_if_i(x_{f_i})) = \sum_{i = 0}^m\ev(g_i)\ev(f_i(X_{f_i})) = 0$$
	Folglich gilt $1 = 0$ in $L'[X_f|f\in J]$; Widerspruch, also $1\notin I$.
\end{proof}
	
\lecture{7. Dezember 2017}	

\begin{satz}
	Sei $K$ Körper. Dann existiert eine algebraische Körpererweiterung $L\sslash K$ mit $L$ algebraisch abgeschlossen.
\end{satz}
\begin{proof}
	Nach obigem Satz existiert $L'$ Körper, sodass $K\subseteq L'$ Unterkörper ist und $L'$ algebraisch abgeschlossen. Betrachte
	$$K^{\text{alg}} = \{a\in L'| a\text{ algebraisch über } K\}$$
	$K^{\text{alg}}$ ist ein Körper (siehe Kapitel 15; Beweis wie für $K = \IQ$).
	
	1. Behauptung: $K\subseteq K^{\text{alg}}$ (also $K\subseteq K^{\text{alg}}\subseteq L'$ jeweils Körpererweiterungen). Denn sei $b\in K$. Dann ist $b$ Nullstelle von $f = t-b\in K[t]$.
	
	2. Behauptung: $K^{\text{alg}}\sslash K$ algebraisch. Klar nach Definition, weil jedes Element $a\in K^{\text{alg}}$ algebraisch über $K$ ist.
	
	3. Behauptung: $K^{\text{alg}}$ algebraisch abgeschlossen. Sei nämlich $f\in K^{\text{alg}}[t]\setminus K^{\text{alg}}$. Zu zeigen: $f$ hat Nullstelle in $K^{\text{alg}}$. Wir wissen dabei: $K^{\text{alg}}\subseteq L'$, $L'$ ist algebraisch abgeschlossen. Folglich hat $f$ eine Nullstelle $a\in L'$, also ist $a$ algebraisch über $K^{\text{alg}}$. Mit Satz 15.1 folgt $K^{\text{alg}}(a)\sslash K^{\text{alg}}$ ist algebraische Körpererweiterung. Aus der Transitivität von algebraischen Körpererweiterungen (Satz 15.2) und der 2. Behauptung folgt, dass $K^{\text{alg}}(a)\sslash K$ algebraische Körpererweiterung ist. Folglich ist jedes Element in $K^{\text{alg}}(a)$ algebraisch über $K$; insbesondere ist $a$ algebraisch über $K$, also $a\in K^{\text{alg}}$. Setze $L:= K^{\text{alg}}$.
\end{proof}

\begin{defi}
	Wir nennen $L$ wie in Satz 16.2 algebraischen Abschluss von $K$ und bezeichnen ihn oft mit $\overline{K}$.
\end{defi}
Wie sieht es nun mit der Eindeutigkeit von $\overline{K}$ aus (bis auf Isomorphismus)?
\begin{defi}
	Sei $K$ Körper und $L_1\sslash K$, $L_2, \sslash K$ Körpererweiterungen. Eine Abbildung $\phi\colon L_1\to L_2$ heißt $K$-Homomorphismus (kurz: $K$-Homo), falls $\phi$ Ring- (und damit Körper-)homomorphismus und $\phi|_K = \id_K$. $\phi$ heißt $K$-Isomorphismus, falls zusätzlich $\phi$ bijektiv. Falls $L_1 = L_2$ und $\phi\colon L_1\to L_2 = L_1$ $K$-Iso, dann nennen wir $\phi$ auch $K$-Automorphismus.
\end{defi}

\begin{bsp}
	$L_1 = L_2$, $\phi = \id$ ist $K$-Automorphismus.
\end{bsp}

\begin{defi}
	Die Menge $Aut(L\sslash K) = \{\phi\colon L\to L|\phi\ K\text{-Automorphismus}\}$ ist eine Gruppe bezüglich Komposition von Abbildungen (für jede Körpererweiterung $L\sslash K$). Wir nennen sie die Automorphismengruppe von $L\sslash K$.
\end{defi}

\begin{bsp}
	Betrachte $\IC\sslash \IR$. Behauptung: $Aut(\IC\sslash\IR) =\{\id, z\mapsto \overline{z}\}$.
\end{bsp}
\begin{proof}
	\leavevmode
	\begin{description}
		\item[\glqq$\supseteq$\grqq:] Klar.
		\item[\glqq$\subseteq$\grqq:] Sei $\phi\in Aut(\IC\sslash\IR)$. Dann ist $\phi(a+bi) =\phi(a)+\phi(b)\phi(i) = a+b\phi(i)$, wobei $a,b\in\IR$. Also ist $\phi$ eindeutig bestimmt durch $\phi(i)$. Es gilt: $-1 = \phi(-1) = \phi(i^2) = \phi(i)^2$. Daraus folgt, dass $\phi(i)$ eine Quadratwurzel von $-1$ sein muss, also $\phi(i )= i$ ($\Rightarrow \phi = \id$) oder $\phi(i) = -i$ ($\Rightarrow \phi$ ist die komplexe Konjugation).
	\end{description}
\end{proof}
...
\begin{satz}[Eindeutigkeit des algebraischen Abschlusses]\label{thm:16.3}
	Sei $K$ ein Körper und $L_1\sslash K$, $L_2\sslash K$ Körpererweiterung, sodass $L_1,L_2$ beide ein algebraischer Abschluss von $K$ sind. Dann existiert $\phi\colon L_1\to L_2$ $K$-Isomorphismus.
\end{satz}

\begin{bem}
	$\phi\colon L_1\to L_2$ $K$-Homo und $[L_1:K] = [L_2:K]<\infty$. Dann ist $\phi$ ein $K$-Isomorphismus. Denn: $\phi$ ist ein Ringhomo und $L_i$ ist Körper, also ist $\phi$ injektiv. Außerdem ist $\phi(x+y) = \phi(x)+\phi(y)$ und $\phi(\lambda x) = \phi(\lambda)\phi(x) = \lambda\phi(x)$ für $\lambda\in K$, $x,y\in L_i$. Also ist $\phi$ eine $K$-lineare Abbildung. Damit ist $\phi$ eine $K$-lineare injetivie Abbildung zwischen endlichdimensionalen $K$-Vektorräumen der gleichen Dimension, also ist $\phi$ bijektiv und damit ein $K$-Isomorphismus.
\end{bem}

\paragraph{Übersicht} Sei $L\sslash K$ eine Körpererweiterung. Dann gilt folgendes bezüglich Teilmengen von $\{\phi\colon L\to L\}$:
%TODO: Insert komisches "Diagramm"

\begin{lem}
	Sei $\phi\colon L_1\to L_2$ ein $K$-Homomorphismus. Sei $f\in K[t]$. Dann gilt: $a$ Nullstelle von $f\Leftrightarrow \phi(a)$ Nullstelle von $f$. 
\end{lem}
\begin{proof}
Sei $f = \sum_{i = 0}^{\infty}b_it^i\in K[t]$. Sei $a$ eine Nullstelle von $f(t)$, also $\sum_{i = 0}^{\infty}b_ia^i = 0$. \\
$\Rightarrow \phi(\sum_{i = 0}^{\infty}b_ia^i) = \phi(0) = 0$\\
$\Leftrightarrow \sum_{i = 0}^{\infty}	\phi(b_i)\phi(a)^i = 0$,  weil $\phi$ ein Ringhomomorphismus ist\\
$\Leftrightarrow \sum_{i = 0}^{\infty}b_i\phi(a)^i =0$ (weil $\phi$ ein $K$-Homo ist), also ist $\phi(a)$ eine Nullstelle von $f$.

Zur Rückrichtung: Da $\phi$ ein $K$-Homo ist, also insbesondere Ringhomo zwischen Körpern und damit injektiv ist, folgt: $\phi(\sum_{i = 0}^{\infty}b_ia^i) = 0\Rightarrow \sum_{i = 0}^{\infty}b_ia^i = 0$.
\end{proof}
...
\begin{proof}
	Sei $a\in L_1$ algebraisch über $K$. Also existiert ein $f\in K[t]\setminus K$ mit $f(a) = 0\Rightarrow a$ ist Nullstelle von $f \in K[t]$. Nach Lemma 16.4 ist $\phi(a)$ Nullstelle von $f$. Daher ist $\phi(a)$ algebraisch über $K$ und Teil 1 des Lemmas folgt.
	
	Sei $m_a(t) \in K[t]$ Minimalpolynom von $a$. Also ist $m_a(t)$ normiert und von minimalem Grad, sodass $a$ Nullstelle ist. Nach Lemma 16.4 ist $\phi(a)$ Nullstelle von $m_a(t)\in K[t]$. Folglich ist $m_{\phi(a)}$ Teiler von $m_a(t)$. Da analog auch $m_a(t)$ ein Teiler von $m_{\phi(a)}$ ist, gilt $m_a(t) = m_{\phi(a)} (t)$.
\end{proof}

\begin{bem}
	Beachte: $\phi$ bildet $a\in L_1$ algebraisch auf Nullstellen von $m_a(t)$ ab\textbf{!!!}
\end{bem}

\begin{lem}
	Sei $K$ Körper. Seine $L\sslash K$, $L'\sslash K$ algebraische Körpererweiterungen und $a\in L, a'\in L'4$ mit $m_a(t)=m_{a'}(t)\in K[t]$. Dann existiert ein einduetiger $K$-Isomorphismus $\phi\colon K(a)\to K(a')$, sodass $\phi(a) = a'$.
\end{lem}
\begin{proof}
	Betrachte den Ringhomo 
	\begin{eqnarray*}
		\ev_a\colon K[t]&\longto& L\\
		p & \longmapsto & p(a)
	\end{eqnarray*}
	Dann gilt $m_a(t)\to m_a(a) = 0\Rightarrow m_a(t) \in \ker(\ev_a)\Rightarrow (m_a(t))\subseteq \ker(\ev_a)$. Nach \ref{thm:homsatz_r} existiert genau ein Ringhomo: $\overline{\ev_a}\colon K[t]/(m_a(t))\to L$, sodass $\overline{\ev_a}\circ \can = \ev_a$.
	
	Klar: $\im\ev_a\subseteq K(a)$. Dann auch $im\overline{\ev_a}\subseteq K(a) \Rightarrow \overline{\ev_a}\colon K[t]/(m_a(t))\to K(a)$. Wir wissen (siehe Vergleich transzendent vs. algebraisch), dass $(m_a(t))$ ein maximales Ideal ist. Also ist $K[t]/(m_a(t))$ ein Körper. Damit ist $\overline{\ev_a}$ injektiv und $\im\overline{\ev_a}\subseteq K(a)$. Außerdem ist $\im\overline{\ev_a}$ ein Körper (weil Bild von Körper unter Ringhomo) und $\im\overline{\ev_a}$ enthält offensichtlich $K$ und $a$. $\Rightarrow \im(\overline{\ev_a}) = K(a)$. Also ist $\phi_1 = \overline{\ev_a}\colon K[t]/(m_a(t))\to K(a)$ Isomorphismus von Körpern. Es gilt nach Konstruktion: $\phi_1(\overline{t}) = a$ und $\phi_1|_K = \id_K$.
	
	Analog haben wir einen Isomorphismus von Körpern $\phi_2: K[t]/(m_{a'}(t))\to K(a')$ mit $\phi_2(\overline{t}) =a'$ und $\phi_2|_K  =\id_K$. Dabei ist $(m_a'(t)) = (m_a(t))$ (nach Voraussetzung). Sei $\phi = \phi_1\circ \phi_1^{-1}\colon K(a) \to K(a')$ ist ein Isomorphismus von Körpern und $\phi(a) = \phi_2(\overline{t}) = \phi(a')$ und $\phi|_K = \id_K$. Folglich haben wir einen $K$ Isomorphismus $\phi\colon K(a)\to K(a')$ mit $\phi(a) = a'$ ($\phi$ ist eindeutig, da $\phi|_K =\id_K$ und $\phi(a) =a'$; dadurch ist $\phi$ eindeutig bestimmt).
\end{proof}

\begin{satz}
	\leavevmode
	\begin{enumerate}
		\item $L\sslash K$ algebraische Körpererweiterung, $L'$ algebraisch abgeschlossener Körper; $f\colon K\to L'$ Ringhomomorphismus. Dann existiert $\hat{f}\colon L\to L'$ Ringhomomorphismus, sodass $\hat{f}|_K  = f$.
		%TODO: Diagramm
		\item Seien $K, K'$ Körper und $f\colon K\to K'$ Ringisomorphismus. Seien $\overline{K}, \overline{K'}$ algebraische Abschlüsse von $K$ beziehungsweise $K'$. Dann exsitert $\hat{f}\colon \overline{K}\to \overline{K'}$ Isomorphismus von Körpern mit $\hat{f}|_K = f$.
	\end{enumerate}
