...
$\begin{array}{ccccccc}
 G&\vartriangleright& [G,G]&\vartriangleright& [D_1(G), D_1(G)]&\vartriangleright&\dots\qquad(1) \\
 \shortparallel &&\shortparallel&&\shortparallel&&\\
 D_0(G) &&      D_1(G)     && D_2(G) 
 \end{array}$

\begin{satz}
	$G$ auflösbar $\Leftrightarrow \exists m\in\IN : D_m(G) = \{e\}$.
\end{satz}
\begin{proof}
	\leavevmode
	\begin{itemize}
		\item [\glqq $\Leftarrow$\grqq] Die abgeleitete Reihe (1) ist nach Definition eine Normalreihe und die Faktoren sind abelsch nach Bemerkung.
		\item [\glqq $\Rightarrow$\grqq] Sei $G$ auflösbar und $\{e\} = G_0\nt G_1\nt\dots G_n = G$ mit abelschen Faktoren. Nach Bemerkung gilt $G_n/G_{n-1}$ abelsch $\Rightarrow [G_n,G_n]\subseteq G_{n-1}$.
		
		Behauptung: $D_i(G) \subseteq G_{n-i}$. Das it klar für $i = 0,1$. Es gilt $D_{i+1}(G) = [D_i(G), D_i(G)]\subseteq [G_{n-i},G_{n-i}]\subseteq G_{n-i-1}$ nach Bemerkung. Also gilt $D_n(G) \subseteq G_0 = \{e\}$ und somit existiert ein $m:=n$ mit $D_m(G) =\{e\}$.	
	\end{itemize}
\end{proof}
...
\begin{defi}
	Gegeben $G\operates X$, $G\operates Y$. Eine Abbildung $f\colon X\to Y$ heißt $G$-Homomorphismus, falls $f(g.x) = g.f(x)$ für alle $g\in G$ und $x\in X$ gilt.
\end{defi}

\begin{defi}
	$G\operates X$, $x\in X$. Dann 
	\begin{enumerate}
		\item $G.x = \{g.x|g\in G\}$ Bahn von $x$
		\item $G_x = \{g\in G|g.x = x\}$ Stabilisator von $x$
		\item $X^G = \{x\in X|\forall g\in G\colon g-x = x\}$ Menge der Fixpunkte
	\end{enumerate}
\end{defi}

\begin{bem}
	$x\sim y$ falls $y\in G.x$ ist eine Äquivalenzrelation:\begin{itemize}
	...
	\end{itemize}
	Also $X = \dot\bigcup$ verschiedene Bahnen.
\end{bem}
...
\begin{bsp}
	\leavevmode
	\begin{enumerate}
	\item Sei $G$ Gruppe, $X:= \{H<G\}$, $G\operates X$ durch Konjugation: $g.H = gHg^{-1}$.
	
	Bahnen: Konjugationsklassen von Untergruppen
	
	Stabilisator von $H\in X$: $G_H = \{g\in G|gHg^{-1} = H\}$ Normalisator von $H$ in $G$, $N_G(H)$
	
	$X^G = \{H<G|gHg^{-1} = H ~ \forall g\in G\} = \{H\nt G\}$
	\item$G$ Gruppe, $H<G$, $X = G/H$. Dann $G\operates X = G/H$ durch $g.(aH) = gaH$ für alle $g\in G, a\in G$ (Linkstranslation), Operation ist transitiv; aber $\ker \Phi' = \bigcap\limits_{x\in G} xHx^{-1}$ (kleinster Normalteiler in $G$, der $H$ enthält)
	\end{enumerate}
\end{bsp}
...
\begin{proof}
	...
	\begin{enumerate}
		\item Übung
		\item Klar $f$ surjektiv; injektiv: Sei $f(g_1G_x) = g(g_2G_x)\Leftrightarrow g_1.x = g_2.x\Leftrightarrow g_1^{-1}g_2.x = x \Leftrightarrow g_1^{-1}g_2\in G_x \Leftrightarrow g_1G_x = G_2G_x$. Also $f$ wohldefiniert und bijektiv.
...
\begin{satz}[Bahnenformel]
	Sei $G\operates X$, $X$ endlich. Dann $$|X| = \sum_{i\in I}(G:G_{x_i}) = |X^G| + \sum_{i\in I, x_i\notin X^G}(G:G_{x_i})\enspace,$$ wobei $(x_i)_{i\in I}$ Elemente in $X$ sind, sodass die Bahnen ein Repräsentantensystem der Bahnen bilden.
\end{satz}

\begin{proof} $|X| = |\bigcup\limits_{i\in I} G.x_i| = \sum_{i\in I}|G.x_i| = \sum (G:G_{x_i}) \Rightarrow$ 1. Gleichung. Nun teile die Bahnen $G.x_i$ in solche auf mit genau einem Element ($\Leftrightarrow x_i\in X^G$) und solchen mit $\geq 2$ Elementen. Da $x_i\in X^G\Leftrightarrow G_{x_i} = G\Leftrightarrow (G:G_{x_i}) = 1$ folgt sofort die 2. Gleichung.
\end{proof}

\begin{satz}
	Sei $G$ endliche Gruppe. $G\operates G$ durch Konjugation. Sei $\{x_i\}_{i\in I}$ so gewählt, dass die Bahnen ein Repräsentantensystem für Konjugationsklassen sind. Dann: $$ |G|  = |Z(G)| + \sum_{i\in I}(G: C_G(x_i))\enspace ,$$ wobei $C_G(x_i) = \{g\in G|gx_ig^{-1} = x_i\}$ Zentralisatior von $x_i$ in $G$.
\end{satz}
