...
\begin{proof}
	...
	\begin{itemize}
		...
		Behauptung: $D^i(G) \subseteq G_{n-i}$ klar für $i = 0;1$. $D^{i+1}(G) = [D^i(G), D^i(G)]\subseteq [G_{n-i},{n-i}]\subseteq G_{n-i-1}$ nach Bemerkung. Also $D^n(G) \subseteq G_0 = \{e\}\Rightarrow \exists m:=n$ mit $D^m(G) =\{e\}$.	
	\end{itemize}
\end{proof}
...
\begin{defi}
	Gegeben $G\operates X$, $G\operates Y$, $f\colon X\to Y$ Abbildung heißt $G$-Homomorphismus, falls $f(g.x) = g.f(x)$ für alle $g\in G$ und $x\in X$.
\end{defi}
...
\begin{defi}
	$G\operates X$, $x\in X$. Dann\begin{enumerate}
		...
		\item $X^G = \{x\in X|\forall g\in G\enspace g-x = x\}$ Menge der Fixpunkte
	\end{enumerate}
\end{defi}
...
\begin{bsp}
	\leavevmode
	\begin{enumerate}
	\item $G$ Gruppe, $X:= \{H<G\}$, $G\operates X$ durch Konjugation: $g.H = gHg^{-1}$.
...
\begin{satz}
	$G$ endliche Gruppe. $G\operates G$ durch Konjugation. Sei $\{x_i\}_{i\in I}$ so gewählt, dass die Bahnen ein Repräsentantensystem für Konjugationsklassen sind. Dann: $$ |G|  = |Z(G)| + \sum_{i\in I}(G: C_G(x_i))\enspace ,$$ wobei $C_G(x_i) = \{g\in G|gx_ig^{-1} = x_i\}$ Zentralisatior von $x_i$ in $G$.
\end{satz}
