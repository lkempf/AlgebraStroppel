...
\begin{proof}
	...
	Noch zu zeigen: (1) Betrachte dafür $M := \{I\subseteq R| I\text{ Ideal,} I \neq R\}$. Betrachte nun die partielle Ordnung $\leq$ auf $M$ gegeben durch $I_1\leq I_2$ falls $I_1\subseteq I_2$. Sei $N\subseteq M$ total geordnete Teilmenge. Dann betrachte $\tilde{I} = \bigcup\limits_{I\in N} I$ ist Ideal (klar; das kannst du dir in einer ruhigen Minute selbst überlegen).
	
	$\tilde{I}\in M$, weil $1\notin \tilde{I}$, da $1\notin I\forall I\in N$ nach Definition von $N\subseteq M$.
	
	$\Rightarrow N$ hat obere Schranke in M. Nach dem Lemma von Zorn existiert ein maximales Element in $M$. Folglich besitzt $R$ ein maximales Ideal (im Allgemeinen nicht eindeutig).
\end{proof}
\begin{bsp}
	...
	\begin{eqnarray*}
		...
		p(t) & \mapsto & p(a)
	\end{eqnarray*}
	surjektiver Ringhomomorphismus. Es gilt $p(t) \in \ker\ev_a \Leftrightarrow p(a) = 0\Leftrightarrow a$ Nullstelle von $p(t)$. Nach Satz 7.13 ist das äquivalent dazu, dass $(t-a)$ das Polynom $p(t)$ teilt; also ist $p(t)$ Element des von $(t-a)$ erzeugten Ideals. Nach dem Homomorphiesatz für Ringe erhalten wir einen Isomorphismus
	$$ \IC[t]/(t-a) \longto \IC$$
	Da $\IC$ Körper ist, ist $I = (t-a)$ maximales Ideal (für jedes $a\in \IC$).
\end{bsp}


\subsection{Einschub: Chinesischer Restsatz}
Stroppel hat eine Pralinenschachtel. Wenn jemand errät, wieviele Pralinen sich darin befinden, bekommt derjenige alle Pralinen (da niemand nachprüfen kann, ob das, was Stroppel bzgl der Anzahl der Pralinen behauptet, stimmt, wird vermutlich niemand die Pralinen bekommen). Wir wissen über die Anzahl der Pralinen: Wenn man sie auf vier Leute aufteilt, bleiben zwei übrig; wenn man sie auf sieben Personen verteilt, bleiben drei Pralinen übrig. Also
\begin{eqnarray*}
	x &\equiv & 2 \pmod 4\\
	x &\equiv & 3 \pmod 7
\end{eqnarray*}
Eine Lösung ist zum Beispiel $x = 10$, aber laut Stroppel ist das nicht die Anzahl der Pralinen (die Pralinenschachtel sieht aber gar nicht so groß aus, als ob da noch wirklich viel mehr Pralinen darin sein könnten).

Behauptung: Genau die Zahlen der Form $10+28k, k\in\IZ$ sind Lösungen.

\begin{satz}[Chinesischer Restsatz]
	Sei $R$ kommutativer Ring, $R\neq\{0\}$. Seien $I_1,\dots,I_s$ Ideale in $R$, sodass $I_i+I_j = R\forall i\neq j$. Dann existiert ein surjektiver Ringhomomorphismus:
	\begin{eqnarray*}
		\Phi\colon R & \longto & R/I_1\times \dots R/I_s\\
		r & \longmapsto & (\overline{r},\dots, \overline{r})
	\end{eqnarray*}
	(wobei $\overline{r}$ jeweils die Nebenklasse von $r$ bezüglich dem passenden $I_i$ ist).
...
\begin{proof}
	$\Phi$ ist ein Ringhomomorphismus: nachrechnen.\\
	$\Phi$ ist surjektiv: Nach Voraussetzung existieren für $1\leq i,j\leq s$ Elemente $a_{ij}\in I_i, a_{ji}\in I_j$ mit $a_{ij}+a_{ji} = 1$. Definiere nun
	$$b_i = \prod_{j\neq i, 1\leq j\leq s} (1-a_{ij}) = \prod_{j\neq i, 1\leq j\leq s} a_{ji}$$
	Es gilt
	\begin{equation}
	 \overline{b_i} = \begin{cases} \overline{0} &\text{ in} R/I_j \text{ für } i\neq j\\
	 \overline{1} &\text{in } R/I
	 \end{cases}
	 \end{equation}
	 Sei nun $(\overline{r_1},\dots, \overline{r_s})\in R/I_1\times \dots \times R/I_s$ beliebig ($r_i\in R$). Setze
	 $$ x : = \sum_{i = 1}^{s} b_ir_i\in R$$
	 Dann $\Phi(x) = \sum_{i=1}^{s}\Phi(b_ir_i) = \sum_{i=1}^{s}\Phi(b_i)\Phi(r_i)$, weil $\Phi$ Ringhomomorphismus ist.
	 
	 Daraus folgt $\Phi(x) = (\sum_{i =1}^{s}\overline{b_i}\overline{r_i},\dots, \sum_{i = 1}^{s}\overline{b_i}\overline{r_i})$. Nach (2) ist das $(\overline{r_1},\dots, \overline{r_s})$. Folglich ist $x$ Urbild von $(\overline{r_1},\dots, \overline{r_s})$. Damit ist $\Phi$ ist surjektiv.\\
	 $\ker\Phi$: $r\in \ker\Phi \Leftrightarrow (\overline{r},\dots, \overline{r}) = (\overline{0},\dots, \overline{0})\in R/I_1\times\dots\times R/I_s$ 
	
	 $\Leftrightarrow \overline{r} = \overline{0}\in R/I_k\forall k (1\leq k\leq s) \Leftrightarrow r\in I_k\forall k\Leftrightarrow  r\in I_1\cap\dots\cap I_s$
\end{proof}


\subsection{Lokalisierung}
\begin{defi} $R$ kommutativer Ring, $R\neq \{0\}$. $S\subseteq R$ heißt multiplikativ abgeschlossen, falls $1\in S$ und $s_1,s_2\in S\Rightarrow s_1s_2\in S$.
	
Dann heißt $S^{-1}R$ Lokalisierung von $R$ an $S$. Dabei ist $S^{-1}R$ ein kommutativer Ring, wobei die Elemente Äquivalenzklassen in $R\times S$ bezüglich $\sim$ (vgl Übungsblatt 5).
Bezeichne mit $\frac rs$ die Äquivalenzklasse von $(r,s)\in R\times S$.
Es gilt $\frac rs = \frac r's'\Leftrightarrow \exists a\in S: a(rs'-r's) = 0$. Wir nennen $S^{-1}R$ auch den Ring der Brüche für $R$ bzgl. $S$.
\end{defi}

\begin{bem}
	\leavevmode
	\begin{enumerate}
		\item Falls $0_R\in S$ ist, ist $S^{-1}R$ bereits der Nullring, denn für $,s,s'\in S, r,r'\in R$ gibt es $a\in S$ (nämlich z.B. $a = 0$) mit  $a(rs'-r's) = 0$
		\item Falls $R$ Integritätsbereich ist, ist $S^{-1}R$ Integritätsbereich für alle $S$ mit $0\notin S$ (selber nachrechnen!)
	\end{enumerate}
\end{bem}

\begin{satz}[Universelle Eigenschaft der Lokalisierung]
	$R$ kommutativer Ring, $R\neq\{0\}, S\subseteq R$ multiplikativ abgeschlossen.
	\begin{enumerate}
		\item \begin{eqnarray*}
					\can\colon R & \longto & S^{-1}R\\
					r & \longmapsto \frac r1
				\end{eqnarray*}
			ist Ringhomomorphismus und $\can(S) \subseteq(S^{-1}R)^{\times}$
		\item Sei $\phi\colon R\to T$ Ringhomomorphismus, sodass $\phi(S) \subseteq T^{\times}$, dann $\exists! \hat{\phi}\colon S^{-1}R\to T$ Ringhomo, sodass
		$\hat{\phi}\circ \can = \phi$:
		\begin{center}
				\begin{tikzcd}
						R \arrow{rd}[swap]{\can} \arrow{r}{\phi} & T \\
						 	& Z\arrow{u}[swap]{\exists! \hat{\phi}\text{ Ringhomo}} \\
				\end{tikzcd}
		\end{center}
	\end{enumerate}
\end{satz}
...
\begin{proof}
	\begin{itemize}
		\item $\can$ ist ein Ringhomo: nachrechnen
		\item $\can(S)\subseteq \{\text{Einheiten}\}$: Sei $s\in S$. Dann ist $\can(s) = \frac s1$ und $\frac s1\cdot \frac1s = 1\Rightarrow \frac s1$ hat Inverses bezüglich der Multiplikation, also Einheit. Damit folgt Aussage (1).
		\item Zu Ausssage (2): Definiere $\hat{\phi}(\frac rs) = \phi(r)\phi(s)^{-1}$ (wobei das das Inverse bzgl $\cdot$ in $S^{-1}R$ darstellt, das nach Annahme existiert)
		
		Nachrechnen: $\hat{phi}$ ist Ringhomomorphismus. Es gilt $\hat{\phi}\circ \can(r) = \hat{phi}(\frac r1) = \phi(r)\phi(1)^{-1} = \phi(r)\cdot 1^{-1} = \phi(r) $. Damit folgt die Existenz von $\hat{\phi}$.
		\item Eindeutigkeit von $\hat{\phi}$: Es muss gelten $\hat{phi}(\frac rs) = \hat{\phi} (\frac r1\cdot \frac1s) = \hat{\phi}(\frac r1)\cdot \hat{\phi}(\frac1s) = \hat{\phi}(\frac r1)\hat{\phi}((\frac s1)^{-1}) = \hat{\phi}(\frac r1)\hat{\phi}(\frac s1)^{-1} = \phi(r)\phi(s)^{-1}$, weil $\hat{\phi}\circ\can = \phi\Rightarrow \hat{\phi}(\frac rs) = \phi(r)\phi(s)^{-1}$, also Eindeutigkeit.
	\end{itemize}
\end{proof}

\begin{defi}
	Falls $R$ Integritätsbereich, $R\neq\{0\}$. Dann $S = R\setminus\{0\}$ multiplikativ abgeschlossen und (Übungsblatt) $S^{-1}R =: Quot(R)$ ist Körper, man nennt ihn den Quotientenkörper zu  $R$.
\end{defi}
\begin{bem} Unterscheide von jetzt an $R/I$ Faktorring und $S^{-1}R$ Quotientenring bzw. Quotientenkörper.
\end{bem}
\begin{bsp}
	\leavevmode
	\begin{enumerate}
		\item $Quot(\IR[t]) = \left\{\frac{f(t)}{g(t)}| f(t), g(t)\in \IR[t], g(t)\neq 0\right\}$ Körper der rationalen Funktionen über $\IR$ in der Variable $t$.
		
		\item Was ist z.B. $Quot(\IZ[t])$? Gute Beschreibung? (Achtung: overkill incoming)
		
		Betrachte $R$ Integritätsbereich und dann die Abbildung
		...
		\end{array}$
		(beachte: $R'$ Integritätsbereich $\Rightarrow R'[t]$ Integritätsbereich)
		
		Nach universeller Eigenschaft des Polynomrings existerit genau ein Ringhomomorphismus
		\begin{eqnarray*}
		...
		Klar: $ p(t) \neq 0 \Rightarrow f(p(t)) \neq 0$, also Einehit, weil $T$ Körper. Nach Satz 11.1 existiert genau ein Ringhomomorphismus $\hat{f}\colon Quot(R[t])\to T$ mit $\hat{f}(\frac{g_1}{g_2}) = f(g_1)(f(g_2))^{-1}$. Man kann zeigen: $\hat{f}$ ist ein Ringisomorphismus (durch Angeben von inversen Abbildungen).
		
		Also: $Quot(R[t])\xrightarrow{\sim} Quot((Quot(R))[t]) = T$ Isomorphismus von Ringen. Im Beispiel hier wollen wir $R = \IZ$. Also
		\begin{eqnarray*}
...
Bezeichne ab jetzt mit $K(t_1,\dots,t_n) := Quot(K[t_1,\dots,t_n])$ für $K$ Körper.
