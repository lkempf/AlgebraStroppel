...
\begin{bem}
	Sei $G$ irgendeine Gruppe, $g\in G$. Dann gilt: $\ord(g) = \abs{\<g\>}$ und nach \namereff{thm:lagrange} dann $\ord(g)$ teilt $\abs G$, falls $\abs G$ endlich.
\end{bem}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		Behauptung: $G_{A'}<G$ mit $|G_{A'}| = p^k$. Dann folgt 1) mit $H = G_{A'}$. Klar: $G_{A'}<G$. Nach dem \namereff{thm:lagrange}: $|G| = |G_{A'}|(G:G_{A'})$, wobei $p^r$ die linke Seite der Gleichung teilt, und im Index auf der rechten Seite $p$ höchstens $r-k$-mal vorkommt.
