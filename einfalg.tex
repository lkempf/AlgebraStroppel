...
\usepackage{fontawesome, bbold, tikz-cd}
...
\end{defi}
Beachte: $\<\{g\}\> = \{e, g, g^{-1}, g^2, g^{-2},\dots\} = \{g^i|i\in\IZ\}$
...
\begin{enumerate}
	\item[(E1)] $f$ Gruppeniso $\Leftrightarrow$ $f^{-1}$ Gruppeniso: denn nach Definition existiert $f^{-1}$. Zu zeigen: $f^{-1}(g'\circ' h')= f^{-1}(g')\circ f^{-1}(h')$ für alle $g',h'\in G$. Sei $g', h' \in G' \Rightarrow \exists g, h\in G\ f(g) = g', f(h) = h'$. Also: $f^{-1}(g'\circ'h')= f^{-1}(f(g)\circ'f(h)) = f^{-1}(f(g\circ h))= g\circ h  = f^{-1}(g')\circ f^{-1}(h')$
...
\begin{kor}
	$f\colon G\to H$ Gruppenhomo. Dann gilt $G/\mbox{Ker}f \cong \mbox{im}f$.
\end{kor}
\begin{proof}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Betrachte 
		\begin{eqnarray*}
			f\colon H&\to& HN \overset{\mbox{can}}{\to} HN/N\\
			h&\mapsto &he
		\end{eqnarray*}
